...
- 결제/유료화 (토스 단건 기간권 + paywall 게이팅 + 친구초대 referral) → memory `payment-toss-monetization-2026-06-30` · UX개선 `ux-review-2026-07-01`. 두 스위치(`features/access/config.ts`): `PAYWALL_ENABLED`=정식 유료화(실 토스키 후 true) · **`LOCK_FOR_GUESTS`=true**(출시 전 게스트 소프트게이팅 — **미로그인=무료 3권(신데렐라·인어공주·백설공주, `isAccessibleForFree!==false`)만, 나머지 146권 잠금**; 로그인=전체 열람). entitlement 판정=`shared/utils/entitlement.ts`(`canReadBook`=isAccessibleForFree!==false OR isEntitled), 결제두뇌=`features/access`. **친구초대=코드 기반 + 양방향(드롭박스식)**: redeem RPC(`supabase-referral.sql`, 마이그 `referral_two_sided_reward`)가 **초대자·피초대자 둘 다 +7/최대28**(`inviterBonus`·`refereeBonus` 반환). `InviteButton`(내 코드 표시+복사, 공유창 X) + `RedeemCodeInput`(피초대자 로그인 후 코드 입력, 성공 시 본인 토스트 baseline 갱신+entitlement invalidate). **진입=사이드바 "친구 초대" 버튼(로그인 시)→`/invite-friends`**(큰 코드+사용법 4단계+받은코드 입력) + 온보딩 ProfileCreateStep "🎁 초대 코드가 있어요" + 부모설정 + `/invite/:code` 링크 랜딩(자동 redeem). `ReferralRewardToast`(초대자 앱 복귀 시 축하, entitlement staleTime 30s+focus refetch). **결제 위치=부모설정 💳 멤버십 섹션**(상태·체험일수·구독하기→/subscribe). LockBadge=`🔒 잠금`. PaywallNotice=표지+구독+초대+무료책 탈출로("회원 가입하면"). **배너**(PromoBanner)=게스트 "회원가입하면 7일 무료 체험"·서브 "친구를 초대하면 무료 기간이 서로 7일씩". **리포트 진입=사이드바 연속재생 아래 구분선+"학습 리포팅"(로그인 시만)→`/parent/reports`**. BookDetail 그림체 칩=`그림체 N`(이름 미노출).
- 연속재생 (`features/continuous/` — 동화책 여러 권 자동 이어재생/잠자리) → memory `continuous-play-2026-07-01`. **진입=사이드바 "연속재생" 버튼**(`authOnly` — **로그인 시만**; 게스트 사이드바는 동화책만) **+ LibraryPage "나의 재생 목록" 섹션**(로그인&저장세트≥1일 때만, 세계명작 위). **선택기는 나레이션 있는 책만**(`koCompletion.pagesTts` — 무음 책 제외; PageSubtitle은 TTS 재생 중일 때만 자막 그림). ViewerContainer 재사용(`playlist` prop: `key={bookId}` remount·reward 3곳 가로채기·**autoStart:index>0**(첫 책은 일반읽기처럼 "탭해서 시작하기" — autoplay 차단 브라우저 대비)·autoplay막힘 폴백게이트·속도·**paused**·stall가드). 컨트롤 **기본 숨김**(재생 화면 안 가림). `/continuous`·`/continuous/new`(빌더)·`/continuous/play`(풀스크린). 저장세트=Supabase `playlists`. LibraryPage 프로모배너=로그인 사용자도(무료 N일/친구초대).
- Learning Reports (부모 리포트 — **동화책 중심 재편**, 완독/읽은시간/연속일 KST 집계 `learning/lib/aggregate.ts`, "총페이지" 폐기, 파닉스/어휘/활동 탭 = dev-only) → [features/learning/CLAUDE.md](packages/client/src/features/learning/CLAUDE.md) · memory `learning-reports-parent-review-2026-07-01`(개선 완료).
