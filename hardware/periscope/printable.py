...
MU        = 33.0
PAD_TILT  = 80.0
# 🔴 **거울은 오스모 실물 치수**(가로 35 × 세로 20)를 쓴다. 사용자 실측이고,
#    그 오스모가 실제로 폰에서 잘 돈다.
#    ⚠ 내가 「20×35 를 9mm 에 두면 112° 라 폰 카메라가 못 낸다」고 했는데 **헛소리**였다.
#       그건 오스모 형상이 아니라 **오스모 거울을 우리 9mm 자리에 억지로 놓은** 값이다.
#       거울 크기를 정하면 **거리가 따라 정해진다** — 반대가 아니다.
# 🔴 방향도 이 계산이 정해 준다: 창 32×17 로 76°×41° 를 내려면 20.5 / 19.1mm 로
#    두 조건이 맞물린다. 뒤집어 놓으면(창 17×32) 10.9 / 35.9 로 전혀 안 맞는다.
FOV_H_REQ = 76.0                     # 판 315mm 를 담는 데 필요한 화각 (시뮬 검증)
FOV_V_REQ = 41.0
MIR_W, MIR_H, MIR_T = 35.0, 20.0, 1.1   # 창 + 사방 1.5mm 테두리
APER_W, APER_H = MIR_W - 3.0, MIR_H - 3.0
CAM_DROP  = 8.0
# 🔴 카메라~거울 거리는 **거울 크기와 필요한 화각에서 파생**된다. 둘 중 더 빡빡한
#    쪽(작은 값)을 쓴다 — 그래야 두 방향 다 화각이 모자라지 않는다.
CAM_GAP   = min((APER_W / 2) / math.tan(math.radians(FOV_H_REQ / 2)),
                (APER_H * math.cos(math.radians(MU)) / 2)
                / math.tan(math.radians(FOV_V_REQ / 2)))
...
# 🔴 폼 자리도 **혀 길이에 비례**로 잡는다 — 8mm 로 박아 두면 혀가 짧아졌을 때
#    끝 쪽으로 밀려난다(11mm 혀에 8mm 면 거의 끝이다).
FOAM_ARM_R = 0.40                    # 축에서 혀 길이의 이 비율만큼 아래
FOAM_W    = 12.0
FOAM_H    = 6.0
...
# ── 몸체
# 🔴 폭도 **거울에서 파생**한다 — 35mm 거울에 42mm 폭이면 벽이 3.5mm 씩밖에 안 남고,
#    거울을 옆으로 밀어 넣는 슬롯이 벽을 다 먹는다.
BODY_W    = MIR_W + 2 * WALL + 6.0
...
# 🔴 **거울 위 높이**가 이 부품의 키를 정한다. 20mm 로 뒀더니 몸체가 28.7mm 로
#    오스모(32)에 가깝게 커졌는데, 우리는 거울을 폰에 9mm 까지 붙여 얇은 게 이점이다.
#    절반으로 줄인다 — 혀 길이도 여기서 따라 줄어든다(손으로 안 적는다).
ABOVE_MIR  = 10.0                    # 거울 꼭대기 위로 올라가는 높이
...
ROOF       = MIR_TOP + ABOVE_MIR
TONGUE_TOP = ROOF - PIN_D / 2 - WALL   # 축
...
def foam():
    ...
    return (cq.Workplane("XY")
            ...
            .translate((0, CHANNEL - t, TONGUE_TOP - FOAM_ARM_R * (TONGUE_TOP - TONGUE_BOT))))
...
def foam_gap():
    ...
    y_face = PIVOT_Y - sag * math.sin(math.pi / 2 * FOAM_ARM_R)
...
def main():
    ...
    lo, hi = foam_squeeze()
    print(f"  폼 압축 {lo:.0f}~{hi:.0f}%  ({FOAM_W:.0f}×{FOAM_H:.0f}×{foam_gap():.1f}mm · 축에서 {FOAM_ARM_R*(TONGUE_TOP-TONGUE_BOT):.1f}mm)"
