...
🔴 조립 순서·설명은 reflector.py 의 STEPS 하나에서 온다. 영상용 목록을 따로 두면
   모델과 영상이 갈라진다.
"""
...
COLORS = {
    "core":         (0.88, 0.24, 0.14),
    "spring_shell": (0.48, 0.11, 0.07),
    "rocker_pad":   (0.11, 0.58, 0.44),
    "case_back":    (0.70, 0.18, 0.10),
    "mirror_glass": (0.80, 0.94, 0.98),
}
...
def build():
    ...
    stl = {}
    for n, f in C.PARTS.items():
        p = os.path.join(C.OUT, n + ".stl")
...
def main():
    ...
    for i in range(nframes):
        ...
        if t < INTRO:
            caption, sub = "탱고 잠망경 반사경 — 조립", "코어는 안 휜다 · 껍데기가 휜다 · 패드는 돈다"
        if t >= total - OUTRO:
