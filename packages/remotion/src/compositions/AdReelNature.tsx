...
//    (실사 한 벌). 대신 **분야의 넓이**가 축이다 — 공룡 21 · 육지 26 · 식물 18 · 바다 9 ·
//    곤충 9 · 하늘 8 · 우주 6 · 우리 몸 3 = 100권. 그래서 첫 씬이 카테고리 줄을 훑는다.
// 🔴 주인공 책이 티라노사우루스인 건 취향이 아니라 실측이다 — 채널에 배달되는 시청자군이
//    공룡 시청자이고 검색량도 종명이 압도적이다(memory `youtube-korea-redesign-2026-07-28`).
// 화면은 전부 실촬(`public/nature/`)이고 얹는 것은 자막·오디오·블록·CTA 뿐이다.

const { fontFamily } = loadFont('normal', { weights: ['700', '800'] });

export const NAT_FPS = 30;
export const NAT_WIDTH = 1080;
export const NAT_HEIGHT = 1920;
...
const LANGS = [
  { key: 'ko', label: '한국어', still: 'nature/n4-read-ko.png', beat: 111 + 12 },
  { key: 'en', label: 'English', still: 'nature/n4-read-en.png', beat: 95 + 12 },
];
...
// ── 규모 블록 ──────────────────────────────────────────────────────────────
const FACTS = [
  { big: '100권', small: '공룡 · 육지 · 바다 · 하늘 · 곤충' },
  { big: '8개 분야', small: '식물 · 우주 · 우리 몸까지' },
...
const SCENES: Scene[] = [
  ...
  {
    // ① 자연 카테고리 줄을 훑는다 — **넓이가 이 광고의 축**이다(명작의 그림체 3종에 해당).
    //    클립 173 · f104 부터 푸터(사업자정보)가 들어와 100 에서 끊었다(실측).
    key: 'n1-rows',
    durationInFrames: 100,
    Component: ({ dur }) => (
      <>
        <Clip src="nature/clips/n1-rows.mp4" />
        <Subtitle
          lines={[
            { text: '공룡 · 곤충 · 바다 · 하늘', at: 6 },
            { text: '식물 · 우주 · 우리 몸까지', at: 46, accent: true },
          ]}
          dur={dur}
        />
      </>
  ...
  },
  {
    ...
    Component: ({ dur }) => (
      <>
        <Clip src="nature/clips/n2-pick.mp4" />
        <Subtitle lines={[{ text: '공룡만 21권', at: 6 }]} dur={dur} />
      </>
    ),
  },
  {
    // ③ 책 상세 — **표지만**(선택 바 위·표지 아래를 잘라내고 앱 배경색으로 채웠다).
    //    자연엔 그림체 축이 없어 언어만 바뀐다(한국어 → English → 한국어).
    key: 'n3-cover',
    durationInFrames: 151,
    bgm: 0.7,
    Component: ({ dur }) => (
      <>
        <Clip src="nature/clips/n3-cover.mp4" />
        <Subtitle lines={[{ text: '읽어줄 말도 고르고', at: 6 }]} dur={dur} scrim={false} />
      </>
    ),
  },
  {
    // ④ 읽어주기 — 실제 나레이션 머리를 언어별로 얹는다(어절 경계에서 컷).
    key: 'langs',
    durationInFrames: LANGS_DURATION,
    bgm: 0.32,
    sfx: LANG_BOUNDS.map((l) => ({ at: l.from, src: `nature/audio/narr-${l.key}-head.mp3` })),
  ...
  },
  {
    ...
    bgm: 0.55,
    // 실측: f88 은 아직 0/4(카드 누르는 중), f94 에 1/4 + 초록 선 — 선이 붙는 f93 에 건다.
    sfx: [{ at: 93, src: 'nature/audio/correct.mp3' }],
    Component: ({ dur }) => (
      <>
        <Clip src="nature/clips/g1-match.mp4" />
        <Subtitle lines={[{ text: '그림과 낱말을 이어 보고', at: 6 }]} dur={dur} until={95} />
      </>
    ),
  },
  {
    // ⑥ 단어 그림 그리기(칠하기) — 원본 f425 완성, 270~445 로 잘라 로컬 155.
    key: 'g-draw',
    durationInFrames: 176,
    bgm: 0.55,
    sfx: [{ at: 155, src: 'nature/audio/correct.mp3' }],
    Component: ({ dur }) => (
      <>
        <Clip src="nature/clips/g3-draw.mp4" />
  ...
  },
  {
    ...
    bgm: 0.55,
    sfx: [{ at: 101, src: 'nature/audio/correct.mp3' }],
    Component: ({ dur }) => (
      <>
        <Clip src="nature/clips/g5a-write.mp4" />
  ...
  },
  {
    ...
    // 🔴 리빌은 낱말을 가린다: `resolve-scene.ts` 가 그 낱말이 **본문에 실제로 나오는 쪽**만
    //    인정하고, 없으면 엉뚱한 장면 대신 아무것도 안 띄운다. 티라노 책은 10개 중 4개
    //    (숲·야자수·나뭇잎·코)가 본문에 없어서 두 번 연속 리빌 없이 찍혔다 —
    //    촬영 스크립트가 본문에 나오는 낱말이 걸릴 때까지 다시 뽑는다.
    // 소리: 띵동(f4) → 쉼 0.4s → 그 쪽 나레이션(f46, 3.66s=110f) → 꼬리.
    key: 'g-reveal',
    durationInFrames: 184,
    bgm: 0.3,
    sfx: [
      { at: 4, src: 'nature/audio/clear.mp3' },
      { at: 46, src: 'nature/audio/narr-reveal-head.mp3' },
    ],
    Component: ({ dur }) => (
      <>
        {/* 리빌 화면은 정지라 뒤 2초는 마지막 프레임 복제다 — 아주 느린 줌으로 살린다 */}
        <Clip src="nature/clips/g5b-reveal.mp4" zoom={[1, 1.05]} dur={dur} />
        <Subtitle
  ...
  {
    // ⑨ 공룡 21권 격자 — 클립 125 · f88 부터 푸터라 80 에서 끊었다.
    key: 'n5-list',
    durationInFrames: 80,
    Component: ({ dur }) => (
      <>
        <Clip src="nature/clips/n5-list.mp4" />
        <Subtitle lines={[{ text: '공룡만 이만큼', at: 6 }]} dur={dur} />
      </>
    ),
  },
...
export const NAT_DURATION = total;
...
export const AdReelNature: React.FC = () => (
  <AbsoluteFill style={{ backgroundColor: CREAM }}>
    <Audio src={staticFile('nature/audio/bgm.mp3')} volume={bgmVolume} />
    {TAIL_LEN > 30 && (
      <Sequence from={TAIL_FROM} durationInFrames={TAIL_LEN}>
        <Audio src={staticFile('nature/audio/bgm.mp3')} volume={bgmTailVolume} />
      </Sequence>
    )}
