/** 호리네 생활동화 릴스 전용 씬 — **실제 책 미리보기**(그 페이지 글 + 그 페이지 그림 1:1).
 *
 * 🔴 명작·자연도감이 쓰는 `StoryScene` 과 분리한 이유:
 *  - 그쪽은 한 씬에 삽화 여러 장을 **타이머로 순환**시킨다(자막 1~2문장 + 페이지 버킷 N장).
 *    여기서는 자막 세그먼트에 그림을 1:1로 묶기 때문에, 같은 컴포넌트를 쓰면 문장 수를 넘는
 *    삽화가 아예 안 나온다 — 명작·자연 릴스를 조용히 망가뜨린다.
 *  - 브랜드 프레임(헤더 제목 상시 + 로고 푸터)·풀블리드 16:9 도 생활동화만 검수된 디자인이다.
 */
...
export const LifeStoryScene: React.FC<Props> = ({
  title,
  body,
  imageUrls,
  hero,
