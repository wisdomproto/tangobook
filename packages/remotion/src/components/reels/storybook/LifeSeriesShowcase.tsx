/** 호리네 생활동화 릴스 시리즈 씬 — 45편 표지를 전부 깔고 위로 흘린다.
 *
 * 🔴 자연도감이 쓰는 `SeriesShowcase`(8테마 정사각 타일 + 라벨) 와 분리한 이유: 자연은 8개
 *  테마 라벨 자체가 정보라 타일+라벨이 맞고, 생활동화는 45편이라 라벨을 달면 글자 벽이 된다.
 *  한 컴포넌트에 두 모드를 넣으면 한쪽 손질이 다른 쪽을 조용히 바꾼다.
 */
...
export const LifeSeriesShowcase: React.FC<Props> = ({ headline, covers, headerTitle }) => {
  const frame = useCurrentFrame();
  return (
    <AbsoluteFill
      style={{
