...
export interface LineMatchingItem {
  ...
  subLabel?: string;
  /**
   * 우측 **글자 카드 아래** 낱말. 파닉스 복습처럼 우측이 글자(ㄱ·ㄹ)이고 그림이 애매할 수 있을 때만 넣는다
   * (오리 그림이 무엇인지 못 알아보면 짝을 지을 수가 없다). 동화책 어휘 게임은 안 넣으므로 무변경.
   * 🔴 이름은 `imageLabel`(그 그림의 낱말)이지만 **그림 밑이 아니라 글자 밑에 그린다**(2026-07-29) —
   *    그림 밑에 두면 아이가 그림을 안 보고 글자만 읽어 짝을 지어버린다.
   */
  imageLabel?: string;
}
