// 유치원동화 로컬 TTS(voicebox mp3, 바탕화면) → R2 업로드 → editor2 storybook page.ttsUrl 세팅.
// gen-yuchiwon.mjs 가 바탕화면 voicebox_samples/유치원_{Yxx}_{title}/pNN.mp3 로 뽑은 걸 붙인다.
// 책은 folder="호리 유치원" + (번호 제거) title 로 매칭. 멱등(같은 키로 덮어쓰기).
//
// 사용:
//   node packages/server/scripts/attach-yuchiwon-tts.mjs --only=Y01 --apply
//   node packages/server/scripts/attach-yuchiwon-tts.mjs --apply           # Y01~Y20 전부
//   (--apply 없으면 dry-run)
...
for (const e of INDEX) {
  ...
  if (m) EPS.push({ code: `Y${m[1]}`, num: Number(m[1]), title: e.title });
}
...
function findDeskDir(code) {
  ...
  const d = fs.readdirSync(DESK).find((n) => n.startsWith(`유치원_${code}_`));
...
async function main() {
  ...
  for (const ep of eps) {
    ...
    if (!dir) { warn.push(`${ep.code} "${ep.title}": mp3 폴더 없음 (TTS 미생성?)`); continue; }
