...
import { BookGroupsService } from '../src/services/book-groups.service.js';
import {
  resolveOwnerUserId,
...
async function main() {
  ...
  // 🔴 한 책 = 한 그림체(2026-09-14) — 그림체 모핑은 같은 그룹의 책들을 모아서 만든다.
  const { groups } = await BookGroupsService.load();
  ...
  for (const id of ids) {
    try {
      ...
      const group = groups.find((g) => g.kind === 'style' && g.bookIds.includes(id));
      const styleBooks = group
        ? await Promise.all(
            group.bookIds.map((bid) => (bid === id ? storybook : fetchStorybook(bid)))
          )
        : [storybook];
      const props = buildReelProps({ storybook, styleBooks, storyboard, genreMap, captions });
      if (!props) {
