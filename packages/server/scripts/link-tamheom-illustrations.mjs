// 세상 탐험 기획서(HTML, 완성 대본) → editor2 storybook 생성/재생성 (멱등).
//
// 각 회차 HTML(tamheom-{slug}.html)이 완성 대본:
//   <p class="ko">        = 독자용 나레이션 글 (page.text)
//   <pre class="scene">   = SCENE 콘티 (page.scene_description + scene_structure)
//   고정 캐스트 8인(호리) + window.SH_GUESTS(직업 게스트: 소방관·기관사 등)
//   R2 comic-assets/saenghwal-plan/{key}  = 호리 8인 확정 레퍼런스 (생활동화와 공유)
//   R2 comic-assets/tamheom-{slug}/{key}  = 회차 직업 게스트 레퍼런스
//   R2 comic-assets/tamheom-{slug}/p{n}   = 페이지 삽화 (아직 미생성 — 있으면 자동 연결)
//
// editor2 에 책이 아직 없어 전부 신규 생성. folder="호리 세상 탐험" 으로 사이드바 그룹핑.
// 재실행 시 title 로 기존 책을 찾아 in-place 재생성(id 유지). 삽화 없어도 글·캐릭터만으로 생성.
//
// 사용:
//   node packages/server/scripts/link-tamheom-illustrations.mjs            # dry-run
//   node packages/server/scripts/link-tamheom-illustrations.mjs --apply
//   node packages/server/scripts/link-tamheom-illustrations.mjs --only=sobangcha [--apply]
...
const INDEX_JSON = path.join(PUBLIC_DIR, 'tamheom-index.json');
const FOLDER = '호리 세상 탐험'; // editor2 사이드바 폴더 그룹핑 키 (= category)

const args = parseArgs(process.argv.slice(2));
const APPLY = args.flags.has('apply');
const ONLY = args.only ? String(args.only) : null;

// ── 고정 캐스트 8인 (saenghwal-core.js SSOT 사본 — 호리 앙상블, 양 선생님 없음) ──
const FIXED_CHARS = [
...
function parseEpisode(docId) {
  const html = fs.readFileSync(path.join(PUBLIC_DIR, `tamheom-${docId}.html`), 'utf-8');
...
async function main() {
  ...
  let docIds = index
    .filter((e) => e.file && e.file !== 'tamheom-plan.html')
    .map((e) => e.file.replace(/^tamheom-/, '').replace(/\.html$/, ''));
  ...
  for (const ep of parsed) {
    const assets = await loadComicAssets(`tamheom-${ep.docId}`);
    ...
    console.log(
      `[${ep.docId}] "${ep.title}"  ${existingId ? `→ 재생성 (${existingId})` : '→ 신규'}\n` +
        `   pages ${pages.length} · 삽화 ${illoN} · 캐스트 ${chars.map((c) => c.name).join('')} (레퍼런스 ${refN}) · 게스트 ${ep.guests.length}` +
