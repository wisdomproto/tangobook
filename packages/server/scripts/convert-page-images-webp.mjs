#!/usr/bin/env node
/**
 * 전체 동화책의 페이지 일러스트(jpg/png)를 webp 로 변환 + storybook JSON 의 이미지 URL 교체.
 * - 대상 = pages[].illustrationUrl + styleAssets[*].pageIllustrations[*].illustrationUrl (표시되는 이미지).
 *   히스토리(illustrationHistory)·표지는 제외(표지는 convert-covers-webp.mjs).
 ...
 * 기본 dry-run / --apply (원본 JSON 백업 scripts/_backup-page-webp).
 */
import {
  S3Client,
...
function collectPageUrls(sb) {
  const urls = new Set();
  const add = (u) => {
    if (isRaster(u)) urls.add(u);
  };
  for (const p of sb.pages ?? []) add(p.illustrationUrl);
  for (const a of Object.values(sb.styleAssets ?? {})) {
    if (!a) continue;
    const pi = a.pageIllustrations ?? {};
    for (const v of Object.values(pi)) add(v?.illustrationUrl);
  }
  return [...urls];
}
...
const backupDir = path.join(__dirname, '_backup-page-webp');
...
if (!APPLY) console.log(`\n실제 적용: WIDTH=${WIDTH} QUALITY=${QUALITY} node scripts/convert-page-images-webp.mjs --apply`);
else console.log(`\n✅ 완료 (원본 JSON 백업: scripts/_backup-page-webp/)`);
