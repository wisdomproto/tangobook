...
declare v_inviter uuid; v_referred uuid; v_bonus int;
begin
  ...
  update public.entitlements set referral_bonus_days = least(28, referral_bonus_days + 7), updated_at = now()
    where account_id = v_inviter returning referral_bonus_days into v_bonus;
  return jsonb_build_object('ok', true, 'inviterBonus', v_bonus);
end; $$;
