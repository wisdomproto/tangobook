...
(function () {
  ...
  function composeBatchPrompt(pages) {
    ...
    var head = [
      ...
      legend,
      '🔴 아이마다 몸 한 부위가 자연 비례를 크게 넘고, 그 부위가 곧 캐릭터다 — 실루엣 질량의 절반쯤을 차지하고 연필이 가장 촘촘히 들어가며, 화면 밖으로 잘리거나 무엇에 가려지지 않고 그 아이 머리보다 작아지지 않는다. 나머지 몸은 평범하게 두고 얼굴은 작다.',
      '🔴 눈은 짧은 연필 획 두세 개를 겹쳐 만든 작은 매듭이고 점을 찍지 않는다 — 놀라면 획이 늘어 매듭이 커지고 진해지며, 획 하나만 눕히면 감은 것이다. 입 곡선과 눈썹 두 획이 기분을 만든다. 어떤 눈도 그 아이의 과장된 부위보다 크지 않다. 얼굴을 사물이 가로지르지 않는다.',
  ...
  function sheetPrompt(charOrKey) {
    ...
    return [
      ...
      '[캐릭터] ' + g.name + ' — ' + g.desc,
      '🔴 이 아이의 과장된 부위(귀·꼬리·가시 등)를 실루엣 질량의 절반쯤으로 크게 그린다. 눈은 짧은 연필 획 두세 개를 겹친 작은 매듭이고 점이 아니다. 입 곡선과 눈썹 두 획이 기분을 만든다.',
