...
(function () {
  ...
  // ── 🔴 앵커 넷 — 권 번호로 고른다. SSOT = docs/art-direction/mei-anchor.md ──
  var ANCHORS = {
    A: { name: '색연필 · 비탈 풀밭', slug: 'mei-pencilslope', vols: ['01','07','09','12','13','15','24'],
      ko: [
        '[스타일] 메이네 산마을 — 4~6세 유아 그림책. 앵커 A `mei-pencilslope`.',
        '거친 온회색 종이(#EDE9E1) 위에 색연필 두 자루로만 그린다. 모든 면은 한 방향으로 그은 평행 획으로 채우고, 종이 결이 획을 끊어 어떤 면도 꽉 차지 않는다.',
        '두 색이 교차해 그은 자리에서만 어두운 색이 생긴다 — 이것 말고 다른 어두움은 없다. 칠하지 않은 종이가 하늘이고 눈이고 빛이다.',
        '음영·번짐·문지름·그라데이션·드리운 그림자·하이라이트가 전부 없다.',
        '색 두 가지: 풀 초록 #7A8B5A, 흙 갈색 #8A7358. 교차한 자리 = 이끼 #4E5840.',
        '🔴 눈 오는 권(01·13)은 비탈을 아예 안 긋는다 — 끝에서 끝까지 맨 종이이고, 두 색은 그 위에 선 것만 그린다. 흰 색연필도 파란 그림자도 없다.',
        '🔴 주황 #D4622A 은 다섯 아이가 하나씩 지닌 물건에만 쓴다 — 메이 목도리·루디 앞치마 주머니·삐노 신발·소소 리본·레오 목깃. 그 밖에는 절대 쓰지 않는다.',
        '획 방향은 한 면 안에서 절대 안 바뀌고, 방향이 바뀌는 자리가 곧 경계다.',
        '화면비 16:9 스프레드. 사실적 렌더링·수채·3D·펠트 아님. 그림 안에 글자·말풍선·문자를 절대 넣지 않는다.',
      ].join(' ') },
    B: { name: '크레용 · 산장 안', slug: 'mei-crayonchalet', vols: ['02','10','14','18','21','22','25'],
      ko: [
        '[스타일] 메이네 산마을 — 4~6세 유아 그림책. 앵커 B `mei-crayonchalet`.',
        '거친 온회색 종이(#EDE9E1) 위에 크레용 두 자루로만 그린다. 크레용이 종이 결의 높은 데만 걸려 낮은 데를 비우므로, 모든 면은 회색 결이 비쳐 보이는 끊긴 밀랍 면이다.',
        '누르는 힘은 어디서나 같다 — 끊기는 것은 손이 아니라 종이가 하는 일이다. 두 색이 겹친 자리에서만 어두운 색이 생긴다.',
        '방 안은 같은 마크를 반복해 짓는다: 바닥=널 획 반복, 벽=짧은 점선 반복, 천=지그재그 반복. 마크 모양은 매번 똑같고, 화면 밖으로 잘릴지언정 새로 그리지 않는다.',
        '🔴 찾고 있는 물건만 제 윤곽으로 그리고 나머지는 반복 마크다 — 그래야 눈이 그것을 찾는다.',
        '색 두 가지: 나무 #7A5E42, 그림자 #4A4038. 겹친 자리 = 나무껍질 #33302A. 불빛은 칠하지 않은 종이이고 화면에서 가장 밝다.',
        '🔴 주황 #D4622A 은 다섯 아이가 하나씩 지닌 물건에만.',
        '화면비 16:9 스프레드. 문지름·긁어내기 없음. 그림 안에 글자·말풍선·문자를 절대 넣지 않는다.',
      ].join(' ') },
    C: { name: '과슈 · 산길·개울', slug: 'mei-gouachebrook', vols: ['04','05','11','19','20'],
      ko: [
        '[스타일] 메이네 산마을 — 4~6세 유아 그림책. 앵커 C `mei-gouachebrook`.',
        '거친 온회색 종이(#EDE9E1) 위에 과슈 두 색으로만 그린다. 납작 붓 자국이 그대로 보이고, 획이 윤곽에 못 미쳐 멈추므로 형태 둘레에 맨 종이 테가 남는다.',
        '물감은 불투명하고 무광이며 절대 묽게 풀지 않는다(수채 번짐 금지). 두 색이 겹친 자리에서만 어두운 색이 생긴다.',
        '물은 가로로 한 번 쓸어 칠한 한 면이다 — 안에서 밝아지거나 어두워지지 않고, 물결도 반짝임도 0이다. 물에 잠긴 것은 그 면 안에 겹침색으로 놓이고, 물 위의 것은 윤곽이 다 보이게 얹힌다.',
        '색 두 가지: 물 #5C7D82, 바위 #6E6A63. 겹친 자리 = 깊은색 #38484B. 길은 맨 종이가 이어진 띠다.',
        '🔴 숲 권(04)에는 개울이 없다 — 바위색이 나무 줄기를(줄기당 한 획, 아홉 그루 이하, 전부 수직) 그리고 물색은 줄기 뒤에 가로로 한 번 쓴 그늘이 된다. 깊이는 줄기가 촘촘해지는 것으로만 나타내고 어두워지지 않는다.',
        '🔴 주황 #D4622A 은 다섯 아이가 하나씩 지닌 물건에만.',
        '화면비 16:9 스프레드. 부드럽거나 번진 가장자리 없음. 그림 안에 글자·말풍선·문자를 절대 넣지 않는다.',
      ].join(' ') },
    D: { name: '숯 · 마을 광장', slug: 'mei-charcoalsquare', vols: ['03','06','08','16','17','23'],
      ko: [
        '[스타일] 메이네 산마을 — 4~6세 유아 그림책. 앵커 D `mei-charcoalsquare`.',
        '거친 온회색 종이(#EDE9E1) 위에 숯 하나와 흙색 하나로만 그린다. 숯을 얹고 손가락으로 문질러 고르게 편 결이고, 종이 결이 알갱이로 비친다.',
        '🔴 빛은 그리는 것이 아니라 지우개로 빼내는 것이다 — 숯을 걷어낸 맨 종이가 화면에서 가장 밝다. 흰색을 칠하는 일은 절대 없다.',
        '한 면은 문질러 있거나 빼내 있거나 둘 중 하나이고, 그 사이의 단계는 없다.',
        '색: 숯 #3A3733(바닥·군중·나무·동물 등), 지붕 흙색 #9A6B52(기와·종탑·덧문 — 광장의 유일한 따뜻한 덩어리).',
        '군중은 납작한 실루엣 일곱 이하이고 얼굴도 손도 없다. 창은 빼낸 사각형 여덟 이하로 이웃과 같은 모양을 복제하지 않는다.',
        '🔴 눈 오는 권(16)은 광장을 아예 안 문지른다 — 끝에서 끝까지 맨 종이이고 숯은 서 있는 것과 군중에만 나온다. 흰 분필은 어디에도 쓰지 않는다.',
        '🔴 숯 가장자리는 모두 부슬부슬하되, 그 쪽의 주인공 아이 윤곽만 눌러 그은 한 줄로 또렷하다.',
        '🔴 주황 #D4622A 은 다섯 아이가 하나씩 지닌 물건에만 — 숯 화면에서 유일한 순색이라 군중 속에서 다섯을 찾는 표식이 된다.',
        '화면비 16:9 스프레드. 그림 안에 글자·말풍선·문자를 절대 넣지 않는다.',
      ].join(' ') },
  };

  var VOL = ((location.pathname.split('/').pop() || '').match(/mei-(\d+)/) || [])[1] || '';
  var ANCHOR_KEY = Object.keys(ANCHORS).find(function (k) { return ANCHORS[k].vols.indexOf(VOL) !== -1; }) || 'A';
  ...
  window.MEI = { copyText: copyText, anchors: ANCHORS, cast: FIXED_CHARS, mountPasteSlots: mountPasteSlots };
