/*
 * yuchiwon-core.js — 호리네 유치원동화 회차 공용 스크립트 (saenghwal-core.js fork: 고정캐스트에 양 선생님 추가, index=yuchiwon)
 ...
 * 기능: 상단 탭, 쪽별/전체 묶음 이미지 프롬프트(스타일 1회 + @image1..9 고정캐스트 + @image10~ 단역 + 쪽별 [등장]),
...
(function () {
  ...
  var STYLE_PROMPT = [
    '[스타일] 호리네 유치원동화 — 3~5세 유아 그림책. 그림체 = 니들펠트(양모 인형) 스톱모션 룩 (needle-felted wool plush, handmade felt stop-motion diorama).',
  ...
  // ── 고정 캐스트 9인 — 배치에서 항상 @image1..9 (순서 고정, 전 회차 공통) ──
  var FIXED_CHARS = [
    ...
    { key: 'teacher', name: '양 선생님', aliases: ['teacher', 'sheep teacher', 'Teacher', '양 선생님', '선생님'],
      desc: '무지개반 담임 양(羊) 선생님 — 니들펠트 양모라 온몸이 폭신한 크림색 양털 뭉게구름, 둥근 치비 비율(어른이지만 아이들보다 살짝만 큼 1.4배·각진 데 없이 포근), 도톰한 둥근 안경과 부드러운 미소, 연두색 카디건, 늘 따뜻하고 차분한 담임 인상.' },
  ];

  var GUESTS = (window.SH_GUESTS || []).slice(); // 화별 단역 → @image10~
  var ALL = FIXED_CHARS.concat(GUESTS);
  ALL.forEach(function (c, i) { c.img = i + 1; }); // 1..9 고정, 10~ 단역
  var IS_GUEST = {}; GUESTS.forEach(function (g) { IS_GUEST[g.key] = true; });
  // 레퍼런스 이미지 없을 때 폴백 얼굴 (고정 캐스트)
  var FACE = { hori: '🐯', mom: '🐯', dad: '🐯', hoya: '🐯', toto: '🐰', bori: '🐻', kongi: '🐿️', dubu: '🐶', teacher: '🐑' };
  ...
  function composeBatchPrompt(pages) {
    ...
    var head = [
      STYLE_PROMPT,
      '',
      '[캐릭터 레퍼런스] 아래 @imageN 순서대로 레퍼런스 이미지를 첨부하세요. 얼굴·헤어·비율·색은 @imageN 시트와 100% 동일하게 유지합니다. @image1~9 = 고정 캐스트(항상 이 순서), @image10~ = 이 화 단역.',
      legend,
  ...
  (async function () {
    ...
    var idx = [];
    try { var ri = await fetch('/yuchiwon-index.json'); idx = await ri.json(); } catch (e) {}
    var eps = idx
  ...
  (async function () {
    ...
    if (anchor) {
      ...
      bar.innerHTML =
        ...
        '<div class="bhint">버튼을 누르면 <b>스타일(1회) + 캐릭터 레퍼런스(@image1~) + ' + pages.length + '개 쪽 장면</b>이 하나로 복사됩니다. GPT에 <b>@image1부터 순서대로 레퍼런스 이미지를 첨부</b>하세요(1~9=고정 캐스트, 10~=이 화 단역). 각 쪽 [등장]이 그 컷에 넣을 캐릭터를 지정합니다.</div>' +
    ...
    (function () {
      ...
      Promise.all([
        fetch('/api/comic-assets/saenghwal-plan').then(function (r) { return r.json(); }).catch(function () { return {}; }),
        fetch('/api/comic-assets/yuchiwon-plan').then(function (r) { return r.json(); }).catch(function () { return {}; })
      ]).then(function (arr) {
        planAssets = Object.assign({}, (arr[0] && arr[0].data) || {}, (arr[1] && arr[1].data) || {});
    ...
    if (GUESTS.length) {
      ...
      sec.innerHTML =
        ...
        '<div class="ghint">이 화에만 나오는 단역입니다(@image10~). ① [시트 프롬프트 복사] → GPT에 넣어 니들펠트 시트 생성 → ② 아래 박스에 붙여넣어 확정하세요. 그러면 전체 프롬프트의 @image 번호와 이 시트가 1:1로 맞습니다. <b>@image1~9 고정 캐스트 시트는 📘 기획서 탭에서.</b></div>' +
        cards;
