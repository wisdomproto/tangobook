...
(function () {
  ...
  var FIXED_CHARS = [
    ...
    { key: 'mole', name: '두더지 할아버지', aliases: ['두더지 할아버지', '할아버지', 'Mole grandfather', 'Mole'],
      desc: '첫 단골 — 낮고 둥근 몸, 볼에 파묻히다시피 한 눈, 이 빠진 큼직한 발톱, 팔꿈치가 늘어난 뜨개 조끼. 🔴 길눈이 어둡다 — 안내한다며 앞장서면 헤맨다. 빨강 없음.' },
  ...
  function composeBatchPrompt(pages) {
    ...
    var head = [
      ...
      legend,
      '🔴 아무도 예쁘게 그리지 않는다. 인물마다 「못난 구체 하나」가 매 등장에 그대로 있고 정리하지도 빼지도 않으며, 인물이 작게 나와도 온전한 압력으로 찍는다 — 코코=한 번 꺾인 꼬리 / 엄마=팔뚝에 눌어붙은 밀가루와 털이 닳아 빠진 자리 / 두더지 할아버지=볼에 묻힌 눈과 이 빠진 발톱, 팔꿈치가 늘어난 조끼 / 까치 아줌마=한 귀퉁이 떨어져 나간 부리 / 돼지 아저씨=앞치마 밖으로 늘어진 배와 갈라진 장화.',
      '🔴 눈은 점이 아니라 고리다 — 판이 테두리만 물어 눈 한가운데로 종이가 비친다. 입 곡선과 눈썹 두 획이 기분을 만들고, 고리는 뜨거나 감기만 하며 크기는 안 변한다. 얼굴을 사물이 가로지르지 않는다. 🔴 웃음은 얼굴에 있다 — 페파형이라 표정이 사건의 절반이다.',
  ...
  function sheetPrompt(charOrKey) {
    ...
    return [
      ...
      '[캐릭터] ' + g.name + ' — ' + g.desc,
      '🔴 이 인물의 「못난 구체 하나」를 온전한 압력으로 그린다. 눈은 점이 아니라 고리다 — 판이 테두리만 물어 한가운데로 종이가 비친다. 입 곡선과 눈썹 두 획이 기분을 만든다.',
