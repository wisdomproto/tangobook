/*
 * jeonrae-core.js — 탱고북 전래동화 공용 스크립트 (경량: 회차 네비게이션 전용)
 * 기획서·회차 HTML 하단에 <script src="/jeonrae-core.js"></script> 로 include.
 * 기능: 좌측 슬라이드 사이드바(☰ 회차) — jeonrae-index.json 기반 회차 목록 + 완성/진행 상태 배지(R2) + 메모(R2).
...
(function () {
  ...
  (function injectCss() {
    if (document.getElementById('jr-core-style')) return;
    var css = [
      ...
      // ── 회차 전체 프롬프트 바 ──
      '.jr-batch{background:var(--paper,#fffdf7);border:1.5px solid var(--mint,#2fa38f);border-radius:14px;padding:14px 18px;margin:16px 0 24px;}',
      '.jr-batch .bhead{font-size:14px;font-weight:900;margin-bottom:4px;}',
      '.jr-batch .bhint{font-size:12px;color:var(--ink-soft,#6d645a);margin-bottom:12px;line-height:1.6;}',
      '.jr-batch .brow{display:flex;gap:8px;flex-wrap:wrap;}',
      '.jr-batch-btn{background:var(--jade,#1f7a6d);color:#fff;border:none;border-radius:999px;padding:9px 20px;font-weight:800;font-size:13px;cursor:pointer;}',
      '.jr-batch-btn:hover{background:var(--jade-dark,#145c52);}',
      '.jr-batch-btn.done{background:var(--vermilion,#cf4b34);}',
    ].join('');
  ...
  (async function () {
    ...
    side.innerHTML =
      '<div class="ep-head"><b>전래동화 회차</b><span id="ep-summary"></span>' +
    ...
    var idx = [];
    try { var ri = await fetch('/jeonrae-index.json'); idx = await ri.json(); } catch (e) {}

    // 📘 기획서 링크 (항상 상단)
    var planEntry = idx.filter(function (e) { return e.file === 'jeonrae-plan.html'; })[0];
    if (planEntry) {
    ...
    var eps = idx
      .filter(function (e) { return e.file && e.file !== 'jeonrae-plan.html'; })
      .map(function (e) {
  ...
  // ── 회차 이미지 프롬프트 도구 (window.JR_EPISODE 정의된 회차에서만) ──
  //   회차 파일은 <script>window.JR_EPISODE={style:'...',cast:[{token,name,desc,aliases}]}</script> 를
  //   이 core include 앞에 두기만 하면, 전체 프롬프트 바 + 쪽별 복사 + 카드 복사 + 붙여넣기가 자동 생성된다.
  (async function () {
    var ep = window.JR_EPISODE;
    if (!ep || !ep.style) return;
    ...
    function compose(subset) {
      ...
      var head = [
        ...
        legend,
        '※ 각 쪽 [등장]에 적힌 @imageN 인물만 그 컷에 그린다. 사물(박·지게 등)은 SCENE 지시대로 배경·소품에 그린다.',
        '',
        '[출력 규칙]',
    ...
    if (anchor && anchor.parentNode) {
      ...
      bar.innerHTML =
        ...
        '<div class="brow"><button type="button" class="jr-batch-btn" id="jr-copy-all">📋 전체 프롬프트 복사 (' + pages.length + '장)</button></div>';
      anchor.parentNode.insertBefore(bar, anchor.nextSibling);
      document.getElementById('jr-copy-all').addEventListener('click', function () { copyText(compose(pages), this); });
    }
