/*
 * abc-tree-core.js — 「ABC 나무 · 호리네 영어 파닉스 동화」 공용 스크립트 (hangeul-tree-core 포크)
 * 기획서·회차 HTML 하단에 <script src="/abc-tree-core.js"></script> 로 include.
 * 기능:
 *   ① 좌측 슬라이드 사이드바 — abc-tree-index.json 기반 유닛 목록 + 완성/진행 배지(R2) + 메모(R2).
 *      상태·메모는 saenghwal 라우트 공유(docId=파일명이라 무충돌): /api/saenghwal-{status,memo}
 *   ② window.AT_EPISODE={cast:[...]} 정의된 회차 → 전체/쪽별 이미지 프롬프트 복사 + 컷 붙여넣기(R2).
 *      🔴 그림체·캐스트는 한글 나무와 **같은 세계관·같은 니들펠트**다 (STYLE_PROMPT 만 영어 학습용으로 조정).
 */
(function () {
  'use strict';
  ...
  var STYLE_PROMPT = [
    '[스타일] ABC 나무 · 호리네 영어 파닉스 동화 — 4~7세 영어 파닉스 그림책. 그림체 = 니들펠트(양모 인형) 스톱모션 룩 (needle-felted wool plush, handmade felt stop-motion diorama).',
    ...
    '🔴 중심 배경 = 호리네 마을의 **ABC 나무** — 한글 나무 옆에 선 두 번째 나무다(같은 마을, 같은 캐스트). 잎과 열매에 알파벳이 열린다.',
    '화면비 16:9 스프레드. 평면 2D 아님·그림물감 아님·매끈한 CG/클레이 아님 — 양모 섬유 질감이 반드시 보여야 함 (visible wool-felt fibers).',
    '🔴 이 책은 영어 학습책이라, 오늘의 알파벳과 타겟 단어는 SCENE 지시대로 나무 열매 표면·나뭇잎 글자판 위에 또렷한 로마자(니들펠트 자수/펠트 글자 느낌)로 그린다. 대문자·소문자를 함께 보이게 한다. 그 외의 임의 문자·말풍선·설명 캡션은 넣지 않고, 상단에 캡션용 여백을 남긴다.',
  ].join(' ');

  (function injectCss() {
    if (document.getElementById('at-core-style')) return;
    var css = [
      ...
      '.at-batch{background:var(--paper,#fff);border:1.5px solid var(--mint,#3f8f52);border-radius:14px;padding:14px 18px;margin:16px 0 24px;}',
      '.at-batch .bhead{font-size:14px;font-weight:900;margin-bottom:4px;}',
      '.at-batch .bhint{font-size:12px;color:var(--ink-soft,#6b5b4a);margin-bottom:12px;line-height:1.6;}',
      '.at-batch .brow{display:flex;gap:8px;flex-wrap:wrap;}',
      '.at-batch-btn{background:var(--leaf,#3f8f52);color:#fff;border:none;border-radius:999px;padding:9px 20px;font-weight:800;font-size:13px;cursor:pointer;}',
      '.at-batch-btn:hover{background:var(--leaf-dark,#2f6b3d);}',
      '.at-batch-btn.done{background:var(--apple,#e0533b);}',
  ...
  (async function () {
    ...
    side.innerHTML =
      '<div class="ep-head"><b>ABC 나무 유닛</b><span id="ep-summary"></span>' +
    ...
    var idx = [];
    try { var ri = await fetch('/abc-tree-index.json'); idx = await ri.json(); } catch (e) {}

    var planEntry = idx.filter(function (e) { return e.file === 'abc-tree-plan.html'; })[0];
    if (planEntry) {
    ...
    var eps = idx
      .filter(function (e) { return e.file && e.file !== 'abc-tree-plan.html'; })
      .map(function (e) {
  ...
  // ── ② 회차 이미지 프롬프트 도구 (window.AT_EPISODE 정의된 회차) ──
  (async function () {
    var ep = window.AT_EPISODE;
    if (!ep) return;
    ...
    if (anchor && anchor.parentNode) {
      ...
      bar.innerHTML =
        ...
        '<div class="brow"><button type="button" class="at-batch-btn" id="at-copy-all">📋 전체 프롬프트 복사 (' + pages.length + '장)</button></div>';
      anchor.parentNode.insertBefore(bar, anchor.nextSibling);
      document.getElementById('at-copy-all').addEventListener('click', function () { copyText(compose(pages), this); });
    }
