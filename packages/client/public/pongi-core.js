...
(function () {
  ...
  // ── 🔴 STYLE SSOT — 앵커 a45 changjak-flatplate 재사용. 변경 시 이 블록만 교체하면 전 회차 반영 ──
  var STYLE_PROMPT = [
    '[스타일] 퐁이네 운하 마을 — 4~6세 유아 그림책. 그림체 = 하드에지 평면 색면(hard-edge flat colour, zero shading).',
    '모든 형태는 단색 도형 하나로 그린다. 음영·그라데이션·질감·윤곽선이 전부 없고, 형태는 두 색이 맞닿는 자리에서만 읽힌다.',
    '팔레트 네 색 고정: 짙은 솔잎 초록 #21372E, 흙 갈색 #8C7C68, 따뜻한 미색 #F6F4EE, 운하 초록 #2C4A3C.',
    '🔴 다섯째 색인 붉은 벽돌색 #A8442F 은 오직 퐁이의 목끈에만 쓴다 — 화면에서 유일하게 따뜻한 것이 주인공이다. 다른 인물·소품에는 절대 쓰지 않는다.',
    '화면비 16:9 스프레드. 사실적 렌더링·수채·3D·펠트 아님 — 평면 색면이어야 함.',
