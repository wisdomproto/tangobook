...
export function VocabularyStudyPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-cream-50 to-peach-100">
      ...
      {/* 🔴 남는 세로는 위아래로 나눈다. **늘리기는 시도했다가 되돌렸다** — 1150px 창에서 카드가
          436×990 이 되어 더 이상했다(2026-09-01). 이 화면은 원래 내용이 적어서, 답은 공간을
          채우는 게 아니라 **덩어리를 제 비율로 만들고 가운데 두는 것**이다.
          `justify-center` 가 아니라 `my-auto` 인 이유: 내용이 창보다 길면 자동 마진은 0 으로
          접혀 위가 잘리지 않는다(가운데 정렬은 위를 스크롤 시작점 위로 밀어 못 보게 만든다). */}
