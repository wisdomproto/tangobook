...
import { ASSET_BASE, SPRITE_KEYS, SFX_KEYS, SFX_BASE } from '../config';
...
export class PreloadScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.audio(SFX_KEYS.hurt, `${SFX_BASE}/hurt.mp3`);
    this.load.audio(SFX_KEYS.gameover, `${SFX_BASE}/gameover.mp3`);
    this.load.audio(SFX_KEYS.bgm, `${SFX_BASE}/bgm.mp3`);
    this.load.audio(SFX_KEYS.coin, `${SFX_BASE}/coin.mp3`);
    this.load.audio(SFX_KEYS.powerup, `${SFX_BASE}/powerup.mp3`);
    this.load.audio(SFX_KEYS.milestone, `${SFX_BASE}/milestone.mp3`);
  }
