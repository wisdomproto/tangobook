...
- **붙인 자리 = `resolveSceneFromWord` 의 파닉스 가드 한 곳.** 파닉스 책이 *자기 쪽*을 보여주면
  안 된다는 규칙(2026-07-29)은 그대로고, 그 자리에서 **다른 동화책**을 돌려준다. 호출부(플레이어 8개)를
  안 건드리므로 한글 파닉스 4종(블록·낱말쓰기·낱말그리기·그림짝)이 전부 자동 적용.
...
- ⚠️ 인덱스는 **한글 파닉스 전용**. 영어 파닉스는 낱말이 인덱스에 없어 자연히 낱말만 읽는다(영어판 미구축).
