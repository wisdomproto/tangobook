...
- 🔴 **타일 크기는 열 수가 아니라 폭이 정한다 (2026-07-28)**: 한글 트레이가 `grid-cols-11` 고정이라 **375px 폭에서 타일이 6.7px**, 폰 가로(812×375)에서도 27px 였다(도입 때부터 그랬고 회귀 아님 — `2363ae0e` 커밋 메시지에 이미 "23~27px"로 기록만 돼 있었다). → `[grid-template-columns:repeat(auto-fill,minmax(2.75rem,1fr))]` = **44px 바닥을 잡고 몇 칸 들어갈지는 패널 폭이 정함**(영어 블록의 `flex-wrap` + `w-11 h-12` 와 같은 원리). 🔴 **브레이크포인트(`sm:`)로는 못 고친다** — 뷰포트 기준인데 실제 제약은 **패널 폭(뷰포트의 절반)**이라 667px 가로화면이 `sm` 이상이라 11칸 그대로다. 🔴 **가로 스크롤 트레이도 안 된다** — 이 게임은 `onTouchMove` 로 드래그해서 스크롤 제스처와 충돌. 세로 화면은 `landscape:flex-row`(=세로면 트레이 위아래 스택)로 6칸 확보. **정사각 강제 X**(짧은 가로화면은 줄 높이가 폭보다 먼저 모자람) + 트레이 `shrink-0`(줄이 늘면 flex 비중으론 화면 밖으로 밀린다). 실측 375×812 44.8×22.4 · 812×375 44.8×22.2 · 1440×900 46.5×48(기존 룩 유지). ⚠️ **세로 375px 가로화면은 높이가 산술적으로 모자란다**(자모 40개 3줄=148px 필요 vs 트레이 몫 92px) — 높이까지 44px 원하면 **자모 개수를 줄이는 수밖에**(쌍자음·어려운 모음을 그 단어에 필요할 때만) 없고 그건 정답을 흘리는 게임성 변경이라 보류. 🔴 **드롭존도 같은 병이었다** — `w-[clamp(2.5rem,8vh,5.5rem)]` 이 375 세로에서 65px 인데 열은 44px 라 **칸끼리 15쌍 겹쳐 엉뚱한 칸에 떨어졌다** → `min(8vh,11vw)` 로 폭 한계를 물림(가로화면·데스크톱 수치는 불변). 🔴 **레이아웃 실험은 인라인 style 말고 소스 편집+HMR 로** — 인라인 style 이 Tailwind 클래스에 먹히는 케이스가 있어(`!important` 를 줘도) 잘못된 결론을 여러 번 냈다.
