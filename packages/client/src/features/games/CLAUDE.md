...
파닉스 단어 카드·동화책 핵심낱말 삽화를 흑백 도안으로 만들어 아이가 **탭한 칸을 칠한다**.
라우트 `/coloring-demo`, 목록 `public/coloring/manifest.json` = **붙여넣은 도안 2,379장**
(굽기 = `build-coloring-manifest.mjs`). 갈래(16) → 단원·책 → 도안 세 단계로 들어간다 —
🔴 **한 화면에 다 깔지 않는다**(장당 200KB라 격자에 전부 걸면 480MB를 받는다).

- 🔴 **도안·원본은 `/api/r2-proxy` 를 거친다.** R2 공개 호스트(`pub-*.r2.dev`·`assets.tangobook.co.kr`)가
...
- 🔴 **흰 물감은 내지 않는다**(2026-09-15 사용자: 개구리 왕자 왕관) — 종이가 흰색이라 칠해도 티가 안 나고,
  대개는 진짜 흰색이 아니라 **어긋난 자리에 걸린 원본 배경**이다(금색 왕관 몸통이 `#fefefb` 로 읽혀 첫 물감이 흰색이었다).
  흰색이 1등인 칸은 흰색·검은 선이 아닌 색이 4% 만 넘어도 그 색으로 읽고, 그래도 없으면 **칠할 칸에서 뺀다**
  (`requiredRef` 도 팔레트가 가진 칸만 — 안 그러면 영영 안 끝난다). 그림 전체가 흰색뿐일 때만 흰 물감을 남긴다.
...
- 🔴 **그냥 겹쳐 읽으면 흰 고양이가 나온다**(2026-09-08). 붙여넣은 도안은 원본을 **다시 그린** 그림이라
  크기·자세가 안 맞는다 — 고양이 몸통 칸에 원본의 흰 배경이 가장 많이 걸린다. 실측 **표본 200장 중 81장**이
  칸 절반 이상을 배경색으로 읽었다. 고친 것 둘: ①원본에서 **그림이 차지한 사각형**을 도안의 **선이
  차지한 사각형**에 맞춰 늘려 놓고 읽는다(`readColorSource` + `boundsOf`) ②칸의 1등 색이 배경색이면
  **배경 아닌 색 중 15% 이상 차지하는 것**으로 바꿔 읽는다(`buildPalette(…, background)`).
  → 81 → 28장(138장 좋아지고 18장 나빠짐). 하얀 백조는 대신 쓸 색이 15%를 못 넘어 그대로 하얗다.
  🔴 `auto-color.mjs` 도 **같은 처리를 한다** — 안 그러면 오프라인 검사가 앱과 다른 답을 낸다.
  ⚠️ 그래도 **347장(14.7%)이 남는다** — 도안이 원본과 아예 다른 것을 그린 경우다(라푼젤 「머리카락」
  원본은 금발 땋은 머리 하나인데 도안은 공주 얼굴 전체). 다시 그리기 전엔 안 낫는다.
...
- 목록 생성 = `node packages/server/scripts/build-coloring-plan.mjs` → `public/coloring-plan-data.json`(파닉스 + 동화책 핵심낱말 **2,387장**). 붙여넣기는 `/api/comic-assets/coloring-plan`. ✅ **2,386장 붙임**(2026-09-08).
...
2,000장을 눈으로 다 보지는 못하므로 **재고, 걸린 것만 작업판에 표시**한다.

```bash
node packages/server/scripts/check-coloring-sheets.mjs   # 도안 픽셀 (≈15분)
node packages/server/scripts/check-coloring-colors.mjs   # 원본에서 색이 읽히는지 (≈25분)
node packages/server/scripts/build-coloring-flags.mjs    # 두 결과 → 딱지 + 게임 제외 목록
node packages/server/scripts/build-coloring-manifest.mjs # 게임 목록
```

- 🔴 **기준은 `build-coloring-flags.mjs` 한 곳에만** 적는다(`RULES`). 작업판·게임 제외 목록·검사기가
  각자 숫자를 들면 같은 도안을 두고 서로 다른 말을 한다.
- 🔴 **칸 나누기는 앱과 같은 코드**(`@tangobook/shared` flood-fill)로 한다. 다만 `auto-color.mjs` 의
  칸 하한은 60px, 앱·검사기는 넓이 비 0.003 이라 **칸 수가 서로 다르게 나온다**(알고 쓸 것).
- 🔴 **「칸이 하나」와 「칠할 게 없다」는 다른 말이다**(2026-09-08). `regions <= 1` 을 「못 칠한다」로 읽어
  23장을 게임에서 뺐는데, 하트·별·숟가락·뼈는 **닫힌 덩어리 하나라 한 번 탭하면 칠해진다**(별로 직접
  플레이해 확인). 진짜 못 칠하는 건 **선이 테두리로 열려 안팎이 이어진 것**이고, 그건 칸 수가 아니라
  **남는 칸이 티끌**로 드러난다(물개 「귀」 0.5% · 「강」 0개). 🔴 그렇다고 **「가장 큰 칸 5% 미만」만
  보면 안 된다** — 칸이 스무 개인 빽빽한 그림까지 314장이 걸린다. **칸 수와 넓이를 같이** 봐야 한다
  → 제외 = `regions === 0 || (regions <= 2 && biggest < 0.05)`.
- 작업판 표시 = `public/coloring-plan-flags.json`(`{key:{level,why}}`). 세 층 — `bad`(다시 뽑을 것) ·
  `warn`(살펴볼 것) · `color`(원본에서 색을 못 읽음). 🔴 **색 층을 ⚠ 숫자에 안 섞는다** — 그게 345장이라
  손볼 것 9장이 그 속에 묻힌다.
- 🔴 **딱지는 잰 그때의 사진이다.** 다시 붙여도 저절로 안 사라지므로 위 세 줄을 다시 돌린다.
  두 판을 견줄 땐 `diff-coloring-check.mjs <이전 coloring-check.json> [이전 flags.json]` —
  **파일 크기로 실제 교체 여부부터 가른다**(딱지만 비교하면 「고쳤는데 여전히 걸린 것」과 「안 고친 것」이
  구분이 안 된다).
- 실적(2026-09-08 2판): 87장 다시 붙여 **색·회색 남은 것 8 → 0 · 빽빽함 39 → 6 · 가는 칸 21 → 7**.
  남은 것 = 다시 뽑을 것 9(칠할 게 없음 7 + 검게 칠한 면 2) · 살펴볼 것 44 · 색이 안 맞는 것 345.
...
- **인터랙션 = 드래그 + 탭 둘 다**. 🔴 **판 위에 놓인 조각도 끌어서 옮긴다**(2026-09-16 사용자) — 잘못 놓으면 되돌리기로 지웠다 다시 놓아야 했다. 끌면 옮기고 그냥 누르면 돌아간다(끈 직후의 click 은 `draggedRef` 로 한 번 삼킨다). 겹치거나 판 밖이면 제자리 — `canPlace` 를 **자기 자신을 뺀** 목록으로 본다. 조각을 끌면 유령이 손가락을 따라오고, 그냥 누르면 선택 후
