...
function reasonMessage(t: TFunction, reason?: string): string {
  switch (reason) {
    case 'self':
      return t('redeem.reasonSelf');
    case 'already_referred':
      return t('redeem.reasonAlreadyReferred');
    case 'race':
      return t('redeem.reasonRace');
    case 'invalid_code':
      return t('redeem.reasonInvalidCode');
    default:
      return t('redeem.reasonDefault');
  }
}

// cap(28일) 도달 시 거짓 "+7일" 축하를 하지 않도록 실제 증가분으로 문구 결정.
function successMessage(t: TFunction, res: RedeemResult): string {
  const mine = res.refereeDelta;
  const friend = res.inviterDelta;
  if (typeof mine !== 'number' || typeof friend !== 'number') {
    return t('redeem.successBoth');
  }
  if (mine > 0 && friend > 0) return t('redeem.successBoth');
  if (mine > 0) return t('redeem.successMine', { days: mine });
  return t('redeem.successCapped');
}
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setState('err');
        setMsg(reasonMessage(t, res.reason));
      }
    } catch {
      setState('err');
      setMsg(t('redeem.requestFailed'));
    }
  };
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex gap-2">
        ...
        >
          {state === 'busy' ? t('redeem.applying') : t('redeem.apply')}
        </button>
      </div>
