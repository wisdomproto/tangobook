...
- `consonant-blend-listen` → `${consonant}+모음 1` / `2` (2026-07-26 단축) — 🔴 `ㄱ + 모음 배우기 1` 은 카드에서 **두 줄로 접힌다**. 카드 제목은 한 줄에 들어가는 길이로.
- `consonant-write` → `${consonant} 써보기`. **한 번 쓸 때마다 슬롯에 낱말 그림이 열린다**(무작위) + 글자 TTS → **그 낱말 TTS** → 띵동. 슬롯 자리는 처음부터 잡아둬 그림이 열려도 레이아웃이 안 흔들린다.
