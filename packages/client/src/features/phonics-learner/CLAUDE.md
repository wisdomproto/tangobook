...
- **`낱말 그리기`(connect-dots) = keypoints 자동 추출**(2026-07-26, `extract-word-card-keypoints.mjs`) — 127/128 단어, **32 단원 전부 가능**. 손으로 점 찍지 않는다.
  - 이 게임은 2026-05-25 부터 "점 순서대로 잇기"가 아니라 **폴리곤 안 색칠**이라, 필요한 건 정확한 점 순서가 아니라 실루엣을 감싸는 폐곡선 하나다 → 배경 제거 → 최대 연결요소 → Moore 경계 추적 → 호 길이 균등 18점.
  - 🔴 **색거리 하나로는 「크림 배경 위의 흰 사물」과 「옅은 그림자」를 못 가른다** — 임계를 올리면 우유·구름·휴지·컵·찐빵이 잘리고 내리면 그림자가 딸려 온다. 그래서 밝기를 **비대칭**으로 보고(밝으면 사물 / 어두우면 많이 어두워야 사물), 그래도 남는 흰 사물은 **결(3×3 표준편차)** 로 건진다 — 배경은 완전히 평평하고 니들펠트는 섬유 결이 있다.
  - 🔴 **눈으로 확인하고 넘길 것**: `--preview` 가 폴리곤을 얹은 PNG 를 뽑는다. 임계 조정은 이 그림을 보고 하는 것이지 숫자로 하는 게 아니다.
  - 마스크가 화면의 5% 미만·90% 초과면 배경 분리 실패로 보고 건너뛴다(전면을 흙이 덮은 `쓱싹` 1장이 그랬다 — 단원에 3장 남아 게임은 정상).
