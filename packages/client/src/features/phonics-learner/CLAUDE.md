...
- **`낱말 그리기`(connect-dots) = keypoints 자동 추출**(2026-07-26, `extract-word-card-keypoints.mjs`) — 127/128 단어, **32 단원 전부 가능**. 손으로 점 찍지 않는다.
  - 이 게임은 2026-05-25 부터 "점 순서대로 잇기"가 아니라 **폴리곤 안 색칠**이라, 필요한 건 정확한 점 순서가 아니라 실루엣을 감싸는 폐곡선 하나다.
  - 🔴 **분할은 픽셀 임계가 아니라 「테두리에서 배경 물 흘리기」** — 픽셀마다 배경색과의 거리로 재던 방식은 못 쓴다. 크림 배경 위의 흰 사물·작가가 그린 옅은 그림자·카드마다 다른 비네팅이 **전부 같은 거리**에 있어서, 임계를 만질 때마다 다른 카드가 깨졌다(밝기 비대칭 → 결 → 모서리 보정을 차례로 붙였지만 매번 새 실패). 배경의 진짜 성질은 색이 아니라 **액자에 닿아 있고 평평하다**는 것. 물이 흐르는 조건 = **평평(기울기) ∧ 배경색 ∧ 테두리 연결**, 셋 다 필요하다 — 색만 보면 흰 사물이 배경이 되고, 기울기만 보면 경계가 흐린 흰 토끼 머리 위로 물이 넘어가고, 연결성이 없으면 사물 안쪽에 구멍이 뚫린다.
  - 🔴 **열림(erode→dilate)을 닫힘보다 먼저** — 김(연기)·수염 같은 실오라기가 남으면 닫힘이 그걸 몸통에 이어붙여 폴리곤이 빈 배경까지 부푼다(고기 카드).
  - 🔴 **폴리곤은 윤곽이 아니라 볼록 껍질** — 윤곽을 18점으로 줄이면 점이 돌기 끝에 앉아 스파이크가 생기고 그 반작용으로 물체가 잘린다. 색칠 게임에선 **잘리는 게 넘치는 것보다 나쁘다**. 그리고 껍질을 리샘플할 땐 꼭짓점으로 스냅하지 말고 **변 위를 보간**할 것(스냅하면 변이 통째로 건너뛰어져 큰 현이 생긴다).
  - 🔴 **큰 덩어리는 여러 개 남긴다**(최대 대비 25% 이상) — 하나만 남기면 주인공이 둘인 카드(얘기의 곰+토끼, 눈의 두 눈)에서 한쪽이 폴리곤 밖으로 나간다.
  - 🔴 **눈으로 확인하고 넘길 것**: `--preview` 가 폴리곤을 얹은 PNG 를 뽑는다. 손잡이 조정은 이 그림을 보고 하는 것이지 숫자로 하는 게 아니다. **160px 콘택트 시트로 훑지 말 것** — 그 크기에선 김이 딸려 들어간 고기 카드를 놓친다(실제로 놓쳐서 사용자가 잡아냈다).
  - 마스크가 화면의 5% 미만·90% 초과면 배경 분리 실패로 보고 건너뛴다(배경이 아예 없는 전면 삽화 `쓱싹`·`가을` — 단원에 다른 카드가 남아 게임은 정상).
