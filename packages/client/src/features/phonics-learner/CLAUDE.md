...
- `consonant-tap` → `${consonant} 배우기` (이전 `${consonant} 누르기`). **카드마다 타겟 단어 그림+낱말**(무작위, 2026-07-26) — 세 번째 탭에서 **그 카드의 낱말**을 읽는다(`ㄱ ㄱ 고기` 리듬 그대로). 🔴 375px 카드는 104px 뿐이라 글자를 크게 두면 `flex-1` 인 그림이 **0px 로 눌린다** — 작은 화면 타이포를 먼저 줄일 것.
