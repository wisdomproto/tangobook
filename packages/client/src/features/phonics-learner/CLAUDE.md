...
- **`낱말 그리기`(connect-dots) = keypoints 자동 추출**(2026-07-26, `extract-word-card-keypoints.mjs`) — 127/128 단어, **32 단원 전부 가능**. 손으로 점 찍지 않는다.
  ...
  - **영어도 같은 스크립트로 완료**(2026-07-27) — 카드 연동 `link-abc-tree-word-cards.mjs` **362/388**(남은 26 = Book 5 u06~u08 미업로드) + keypoints **490장**(한글 128 + 영어 362, 실패 0). 유닛 목록을 손으로 적지 않고 **커리큘럼에서 파생**하므로 영어가 자동으로 딸려 온다.
