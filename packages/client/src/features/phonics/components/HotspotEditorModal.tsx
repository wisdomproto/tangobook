...
interface HotspotEditorModalProps {
  ...
  aspectRatio?: string;
  onSave: (hotspots: (WordHotspot | undefined)[]) => void;
  onClose: () => void;
}
...
export function HotspotEditorModal({
  ...
  // 클릭한 위치에 기존 핫스팟이 있는지 확인
  const hitTest = useCallback(
    (nx: number, ny: number): number | null => {
      for (let i = hotspots.length - 1; i >= 0; i--) {
        const h = hotspots[i];
        if (!h) continue;
        if (nx >= h.x && nx <= h.x + h.w && ny >= h.y && ny <= h.y + h.h) return i;
      }
      return null;
    },
    [hotspots]
  ...
  const handleSave = () => onSave(hotspots);
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-auto">
        ...
        <div className="flex gap-2 px-4 pt-3 flex-wrap">
          {words.map((w, i) => {
            const c = color(i);
            const hasHotspot = !!hotspots[i];
            return (
              <button
                key={i}
                onClick={() => setSelectedIdx(i)}
                className={`px-3 py-1.5 rounded-lg text-sm font-medium border-2 transition-all ${
                  i === selectedIdx ? 'ring-2 ring-offset-1 shadow-md' : ''
                }`}
                style={{
                  borderColor: c.border,
                  backgroundColor: i === selectedIdx ? c.bg : 'transparent',
                  color: c.text,
                  ...(i === selectedIdx
