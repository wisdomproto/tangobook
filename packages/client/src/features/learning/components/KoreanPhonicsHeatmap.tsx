    ...
    <div className="space-y-3">
      {KOREAN_PHONICS_LEVELS.filter((lv) => !levelIds || levelIds.includes(lv.id)).map((lv) => {
        ...
        for (const c of grid.cells) {
          cellByKey.set(`${c.row} ${c.col}`, c);
          const s = syllableStats.get(c.statKey);
        ...
        return (
          <div key={lv.id} className="rounded-2xl bg-white/80 p-4 shadow-sm">
            ...
            >
              <div>
                {/* 색을 안 주면 상속받아 옅게 깔린다 — 레벨 제목은 이 카드에서 제일 큰 글자다. */}
                <div className="font-bold text-ink-900">{lv.name}</div>
              ...
              <div className="flex shrink-0 flex-col items-end gap-0.5 text-xs">
                ...
                {grid.cells.length > 0 && (
                  // 🔴 `text-success`(#5CC99F)는 흰 배경 대비 2:1 이라 작은 글씨로 못 쓴다 →
                  //    같은 계열의 mint-600(4.9:1). 색 뜻(익힘=초록)은 그대로 이어진다.
                  <span className="font-semibold text-mint-600">{masteredPct}% 익힘</span>
                )}
              </div>
            </button>
            {grid.cells.length > 0 && (
                      ...
                      {grid.rows.map((c) => (
                        <tr key={c}>
                          ...
                          {grid.cols.map((v) => {
                            ...
                            const s = syllableStats.get(cell.statKey);
                            const m = s ? computeMastery(s, now) : 0;
                            return (
