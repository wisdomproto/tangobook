  ...
  return (
    <div>
      <h3 className="mb-2 text-base font-bold text-ink-900">
        ...
        <span className="ml-1.5 text-sm font-black text-coral-500">모두 {details.length}개</span>
      </h3>
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="truncate text-[11px] text-ink-500">
                  {primary ? primary.title : '동화책'}
                  {extra > 0 && <span className="text-ink-400"> 외 {extra}권</span>}
                </div>
      ...
      {details.length > INITIAL && (
        ...
        >
          {expanded ? '접기' : `전체 ${details.length}개 보기`}
        </button>
      )}
    </div>
  );
