...
- 🔴 **부모 화면에 마스터리 % 금지** — `computeMastery` 가 `exp(-days/30)` 을 곱해 **아무것도 안 해도
  ...
  대신 **`안 봄 / 봄 / 연습 중 / 익힘`** 네 단계. 숫자는 dev 탭에만.
  🔴 **말은 한 벌이다** — `lib/mastery.ts` 의 `PARENT_LABEL`(배지)과 `MasteryDistributionBar` 의
  `SEGMENTS`(막대 범례)가 같은 말·같은 색을 써야 한다. 여기서 **두 번 어긋났다**: ①「안 봄」·「봄」을
  둘 다 「아직」 으로 묶어 **회색 「아직」과 코랄 「아직」이 나란히** ②배지만 「아직·배우는 중」 으로
  바꿔 **막대 범례와 딴 말**. 한쪽만 보고 고치면 반드시 어긋난다.
- 🔴 **색 뜻은 표 위에서 한 번 설명한다**(`MasteryLegendCard`) — 색 4칸만 두면 부모가 회색과 연한
  코랄의 차이를 스스로 추론해야 한다. **"한동안 안 하면 조금씩 내려간다"까지 적는다**: 이걸 모르면
  지난주 「익힘」이 이번 주 「연습 중」일 때 "애가 까먹었다"로 읽는다.
...
- 🔴 **자음×모음 칸은 단어에서 만난 음절도 센다**(2026-07-28) — 예전엔 음절 활동만 세서 **표가
  실제보다 비어 보였다**(사용자 지적: "타겟 단어 공부하면서 각 음절 노출되자나"). `groupBySyllable`
  이 한글 단어 이벤트를 `decomposeWord`(shared) 로 쪼개 칸에 얹는다. 🔴 **맞고 틀림은 약하게**
  센다(`WORD_CREDIT = 0.25` = **단어 4번이 글자 1번**). 처음엔 노출만 올렸는데 사용자가
  **"아예 점수 안 주는 게 더 이상하지"** — 단어를 맞히는 아이는 그 글자를 어느 정도 읽고 있다.
  단어 한 번은 「봄」을 못 넘고(0.19), 그 글자를 따로 한 번 맞히면 「연습 중」(0.36).
  `correct/wrong` 이 소수가 되므로 **툴팁 같은 표시부는 반올림**(`round1`)해야 `0.25/0.25` 가 안 뜬다.
  영어 단어는 `decomposeWord` 가 빈 배열이라 저절로 걸러진다.
  - 🔴 **음절을 직접 쏘는 게임의 단어는 부분점수에서 뺀다** — 한글 블록·낱말 쓰기는 **플레이어가
    이미** `decomposeWord` 로 음절 이벤트를 정식 1점으로 보낸다(아이가 글자를 실제로 조작하므로
    그게 맞다). 모르고 단어 점수까지 얹어 **같은 판정을 1.25번 세고 있었다**.
    🔴 **그림 짝은 여기서 뺐다**(2026-07-28) — 그림과 `고기` 를 이었다고 `고` 를 읽은 게 아니고,
    게다가 **완료가 곧 전원 정답**이라 판정이랄 것도 없었다. 이제 단어만 보내고 부분점수를 받는다.
    **"음절 1점 = 아이가 그 글자를 직접 만들거나 썼을 때"** 가 기준이다.
    🔴 게임 이름을 박지 말 것 — **그 판에서 음절 이벤트가 실제로 왔는지를 이벤트에서 뽑는다**
    (`syllableGames`). 플레이어가 바뀌어도 집계는 안 건드린다.
  - ⚠️ **동화책 어휘는 진작부터 들어오고 있었다** — `groupBySyllable`·`PhonicsReportSection` 둘 다
    `source` 를 안 가린다. `useGameLogger` 에 "connect-the-dots 만 연동" 이라는 **낡은 TODO** 가
    남아 있어 반대로 읽었다(실제 호출부 9곳). 주석은 지웠다. 이 규칙은 **`MasteryLegendCard` 에
    한 줄로 적어 둔다** — 안 적으면 부모가 「고기만 했는데 왜 고·기 칸이 켜졌지」 를 묻는다.
- 🔴 **표의 축은 레벨마다 다르다**(2026-07-28) — 한글1·3 은 `자음 × 모음`, **한글2 받침은 `받침 × 초성`**
  (모음 `ㅏ` 고정이라 축에서 뺀다). 받침 `blending` 은 `[초성, 중성, 받침, 결과]` **4원소**인데 3원소로
  읽어서 ①받침이 결과음절 자리에 들어가고 ②셀 자음(`ㄱ`)이 행 헤더(`받침ㅇ`)와 안 맞아 **격자가 통째로
  비어 있었다**. 🔴 게다가 집계 키가 `ㄱㅏ` = 기본 음절 `가` 라 **「연습 중 35」가 한글1 진도를 빌려온
  숫자**였다 — 빈 칸보다 나쁜 거짓말이다. 이제 `buildKoreanPhonicsGrid` 가 **결과 음절을 쪼개**
  `{row, col, statKey}` 를 만들고(`decomposeHangul`), 키는 `초성+중성+받침` 이라 `강`(ㄱㅏㅇ)과
  `가`(ㄱㅏ)가 갈린다. 받침은 98칸이 실제로 다 채워지므로 **한글4처럼 축을 줄이지 않는다** —
  줄이는 건 데이터가 **없을 때** 하는 일이다.
  - 🔴 **격자 키와 집계 키가 같은 규칙이어야 한다** — 각자 테스트가 통과해도 이 이음매는 안 잡힌다.
    `korean-phonics-grid.test.ts` 의 「격자 키 ↔ 집계 키」가 그 자리를 지킨다.
  - 🔴 **또 다른 키 이음매 = 표 렌더의 `cellByKey`**(2026-08-05). `KoreanPhonicsHeatmap` 표는
    `cellByKey.get(\`${row} ${col}\`)`(공백)로 칸을 찾는데, `set()`템플릿의 "공백"이 소스에 **보이지
않는 NULL(U+0000)**로 들어가 있어 **전 칸이 빈 칸**이었다. 분포 막대는`statKey`로 집계해 정상이라
**은폐됐고 실제 부모 리포트에도 있던 버그**다. 가드`KoreanPhonicsHeatmap.test.tsx`(격자에만 있는
    음절 '갸'가 실제로 뜨는지 — 이벤트 없이도 격자엔 음절이 다 떠야 한다).
  - 🔴 **받침 활동도 이제 기록한다** — 예전엔 "표에 자리가 없다"며 `if (!isCoda)` 로 건너뛰었는데
    자리를 만들었으니 건너뛸 이유가 없다. `useLogSyllable(unitId)`(phonics-learner) 가 **완성된 글자를
    쪼개** 넘기므로 자음·받침 모드가 같은 코드다(받침 모드의 `pair.first` 는 자음이 아니라 음절 `가`라
    그대로 넘기면 칸이 안 맞는다).
