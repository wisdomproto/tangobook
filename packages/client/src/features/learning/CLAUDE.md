...
- 🔴 **자음×모음 칸은 단어에서 만난 음절도 센다**(2026-07-28) — 예전엔 음절 활동만 세서 **표가
  ...
  이 한글 단어 이벤트를 `decomposeWord`(shared) 로 쪼개 칸에 얹는다. 🔴 **노출만 올리고 맞힘으로는
  안 센다** — `고기` 를 맞혔다고 `고` 를 읽는다는 근거는 없다. 그래서 그 칸은 `computeMastery` 의
  `attempts === 0` 가지를 타고 **「봄」에서 멈춘다**(위로 가려면 그 음절을 실제로 판정받아야 한다).
