...
| 발행 (Phase 3 + 실발행)          | 완료 | `PublishDashboard` + **Meta 연동/멀티채널 실발행**(2026-07-10): IG/FB/Threads/YouTube 쇼츠. 발행 큐=dflo 보드(언어pills+채널컬럼). 🔴 발행 파이프라인 hardening(2026-07-12): getBundle 캐시+재시도 · IG 릴스 폴링 ~3분·5s 간격(앱 요청한도 `(#4)` 회피). 상세 → memory `meta-connect-publish-2026-07-10` · `marketing-ad-tab-publish-2026-07-12`                 |
| 콘텐츠 정규/광고 탭 (2026-07-12) | 완료 | `ContentListPanel` 상단 정규/광고 탭(`mkt_contents.content_kind` regular\|ad, `ui-store` 영속). 광고 콘텐츠=`ContentTabs`가 릴스+카드뉴스만(기본 릴스) → 릴스 업로드+`ReelsPublishDialog` 발행. 광고 릴스 시드=`scripts/upload-ad-reel-assets.ts`(R2)+`mkt_publish_records` 직접삽입→프로덕션 스케줄러 발행. 상세 → memory `marketing-ad-tab-publish-2026-07-12` |
