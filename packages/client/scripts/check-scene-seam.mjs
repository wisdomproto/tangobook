...
// 🔴 **한국어는 음절 단위라 어간만 적으면 활용형이 통째로 샌다**(2026-09-07, pipo). 「벌리」로는
// 「벌**린**·벌**려**·벌**렸**」이 한 글자도 안 걸려서, SCENE 에 「부리를 딱 **벌린** 채」·「입을 크게
// **벌려** 웃는다」가 **적혀 있는 쪽**이 후보로 올라왔다(오검출 넷). 활용형을 다 적는다.
// 🔴 **꾸미는 말이 앞에 오는 어순도 받는다** — 「**벌어진 부리** 끝이」(pipo 03 p3).
const SAYS = /(?:벌어진|벌린|벌려|열린|헤벌어진)\s*(?:입|부리|주둥이|입술)|(?:입|부리|입가|입술|주둥이)[^.,]{0,14}(?:벌어|벌리|벌린|벌려|벌렸|열리|열린|열려|열었|크게 열|헤벌|쩍)|말한다|말하는|외친다|외치|묻는다|물어본|소곤|속삭|노래(?:한다|해|를 부)|중얼|읊|턱이 벌/;
