    ...
    for (const p of bk.pages) {
      for (const s of p.ko.split(/(?<=[.!?])\s+/)) {
        ...
        const SIG = ['꼬리가 붕', '목이 쭉', '꼬리가 쫑', '두 발로 쭉', '목을 살랑', '날개를 활짝', '종이 한 번', '종이 땡', '칙칙폭폭 들어와', '꼬리지느러미를 탁', '"배다!"',
          ...
          // 미오(시리즈 07) = 다섯 아이가 저마다 하나씩, 권마다 한 번
          '가방 끈을 두 손으로 꽉', '모자 챙을 앞으로', '두 손으로 귀를 덮', '목을 쭉', '손수건 매듭을 잡아당',
          // 미나(시리즈 15) = 해냈을 때 코를 하늘로 번쩍 (p9~p10, 호리편 25권이 권마다 한 번)
          '코를 하늘로 번쩍', '코가 하늘로 번쩍', '코를 하늘 높이', '코를 하늘을 향해', '코가 하늘을 향해', '코를 번쩍'];
