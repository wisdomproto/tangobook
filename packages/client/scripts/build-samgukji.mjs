...
const CAST = {
  ...
  diaochan: { token: 'Diaochan', name: '초선', desc: '왕윤이 딸처럼 기른 열여섯 살 소녀 · 🔴 이 책에서 가장 곱게 그리는 사람이다 — 여포와 동탁이 둘 다 넘어가는 것이 이 권의 줄거리라 그게 그림으로 보여야 한다 · 🔴 이 책에서 얼굴에 화장을 한 유일한 인물(눈가와 입술) · 옅은 옥색 옷 · 손에 둥근 부채 · 🔴 어른스럽게 그리지 않는다.', aliases: ['Diaochan', '초선'] },
  ...
  huatuo: { token: 'Huatuo', name: '화타', desc: '이름난 의원 · 흰 머리를 뒤로 묶었다 · 🔴 무기 대신 늘 나무 약함을 든다 · 조조 앞에서도 눈을 안 내리는 얼굴.', aliases: ['Huatuo', '화타'] },
...
const EN = {
  ...
  diaochan: 'DIAO CHAN — a sixteen-year-old girl, THE MOST BEAUTIFUL FACE IN THIS BOOK - the story turns on two warlords both losing their heads over her, so it has to show; the only painted face in the story (eyes and lips), pale jade robe, holding a round fan. Beautiful but still a girl, never drawn as a grown woman',
  ...
  huatuo: 'HUA TUO — the physician of the story, white hair drawn back and tied, a small wooden medicine case always in one hand and never a weapon, steady-eyed',
 ...
 */
const ROLE_WORDS = /(라는|이라는|이름은|스승|선생|고을|마을|고장|나라|도읍|성|산|장수|벼슬아치|문관|신하|선비|땅|상인|장사꾼|호걸|청년|사내|황제|관리|사람|아우|형|아들|아내|어머니|아버지|누이)/;
