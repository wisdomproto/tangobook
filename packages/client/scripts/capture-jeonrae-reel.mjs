// 전래 동화 광고 릴스 — 라이브러리 여정 · 표지 · 읽어주기 스틸 · 20편 격자.
//   node scripts/capture-jeonrae-reel.mjs <baseUrl> <outDir> [clip...]
//
// 🔴 전래동화의 축은 **우리 것**이다 — 낱말이 전부 전통 사물(박·지게·톱·엽전·짚신)이고,
//    그림체도 이 라인 전용 「점눈이」다. 넓이(자연)나 그림체 3종(명작)이 아니라 **문화**가 팔 거리.
// ⚠️ 공개는 **시즌1 20편뿐**(시즌2 20편은 표지 미완이라 비공개) — 광고에 40편이라 쓰면 거짓말이다.
// ⚠️ 번역도 **ko·en 둘뿐**이다(20/20 en). 「5개 언어」는 이 라인에 쓸 수 없다.
...
const BOOK = '1784529056876'; // 흥부와 놀부 — 낱말 5개(박·지게·톱·엽전·짚신) **전부 본문 등장**
const TITLE = '흥부와 놀부';
...
try {
  ...
  // ── n1-rows — 전래 동화 줄을 **가로로** 흘려 익숙한 제목을 보여준다 ──────
  // 🔴 자연판은 카테고리 8줄을 세로로 훑었지만 전래는 **카테고리가 하나뿐**이라 그 방식이
  //    성립하지 않는다. 대신 그 줄의 가로 캐러셀을 흘린다 — 흥부·콩쥐·오누이 같은
  //    **아는 제목이 지나가는 것** 자체가 이 라인의 훅이다.
  if (want('n1-rows')) {
    console.log('[n1-rows]');
    await page.evaluate(() =>
    ...
    await page.evaluate((l) => {
      ...
      if (r) window.scrollBy({ top: r.getBoundingClientRect().top - 150, behavior: 'instant' });
    }, CATEGORY);
    await sleep(900);
    const found = await page.evaluate(
      (l) => !!document.querySelector(`[role=region][aria-label="${l}"]`),
      CATEGORY
    );
    if (!found) throw new Error(`「${CATEGORY}」 줄을 못 찾았다`);
    await record(page, OUT, 'n1-rows', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        await page.evaluate((l) => {
          ...
          r?.scrollBy({ left: 470, behavior: 'smooth' });
        }, CATEGORY);
        await sleep(680);
      }
      await sleep(500);
