...
const FLAGS = [
  ...
  ['설정', '아이가 규칙을 외워야 굴러간다', [
    /(할 때마다|하면 반드시|하기 전엔|해야만|한 번만|세 번|하루에 한 번|절대 .{0,6}면 안)/,
    /(규칙|약속|법|주문|마법)[이가은는을를]?\s*(있|생기|깨|어기|외우)/,
  ]],
  ['추론', '원인·대가·연쇄를 머리로 따라가야 한다', [
    /(대신|바꾸는 대신|그 대가로|대가|잃는다|잃게)/,
    /(하나씩 건너|차례로 옮겨|돌고 돌아|거슬러 올라|되짚)/,
    /(왜 그런지|이유를 알아내|알아채야|추리|비밀을 밝)/,
  ]],
  ...
  ['판타지', '세계가 현실과 다르게 작동한다', [
    ...
    /(달빛|별빛|별|바람|소리|노래|냄새|꿈|그림자)[을를]\s*\S{0,6}(담|모아|모으|배달|판다|팔아|병에|주머니에)/,
    /(투명해|투명하게)/,
  ]],
];
