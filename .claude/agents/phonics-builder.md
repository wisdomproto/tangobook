...
11. 🔴 **게임 목록을 형제 레벨/권과 대조** — 새 레벨 generator 를 백지에서 짜면 형제가 가진 게임을
    빠뜨린다. **Book 3/4/5(`makeWordUnitPlan`)가 Book 2(`makeBook2UnitPlan`)의 「낱말 쓰기」를 빼고
    3종만** 두어 사용자가 "왜 낱말쓰기 없어?" 로 잡았다(2026-08-04). 낱말 쓰기 게임은 pattern 없이
    호출하면 호스트가 단원 전체 단어로 굴린다(Book 2 와 같은 경로) — plan 에 한 줄 빠진 것뿐이었다.
    새 레벨을 만들면 **형제 레벨의 play 섹션 kind 목록을 나란히 놓고** 빠진 게 없는지 볼 것.
...
- 🔴 **`[소리][낱말]` 음원은 Gemini 통문장 말고 라이브러리 concat 으로** — Gemini 에 "ay cave" 를 통째로
  주면 프리픽스를 들쭉날쭉 읽는다(실측: "ay"→/oʊ/·"cave"에선 앞소리 거의 누락, 1.08초). concat 은
  pre-recorded 클립을 이어붙여 일관되다("A cave"=에이 케이브). Book 4/5 블렌드도 concat 이었다. 낱말이
  라이브러리에 없을 때만 Gemini 폴백. (Book 3 매직-e = `scripts/regen-magice-word-tts.ts`, 2026-08-04.)
- 🔴 **배우기 음원은 낱말만 읽지 말고 그 권의 파닉스를 가르쳐야** — 매직-e 인데 "cave" 만 읽으면
  매직-e 를 안 가르친다(사용자: "그냥 케이브라고만 읽어주는데?"). 장모음=모음이 이름을 말함
  (a_e→"A", i_e→"I"…). 새 권 음원은 **그 권이 가르치는 소리**가 낱말 앞에 붙는지 확인.
