...
저장소·공통 코드·빌드/배포 담당. 기능 데이터의 의미는 해당 영역 담당 지침과 대조한다.
...
shared 런타임 상대 import는 .js 확장자를 유지한다. 로컬 서버는 DISABLE_PUBLISH_SCHEDULER=1 설정을 확인한다.

## 검증·인계

변경에 맞는 typecheck/test/build와 실행 경로를 확인한다. tsc 통과와 Node ESM 실행 성공은 구분한다.
