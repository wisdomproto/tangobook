# 마케팅·채널 운영 기억

갱신: 2026-09-21. 초기 분류 기준: main의 e681e945 및 위 문서들. 이 초기화 작업은 기능 동작 재검증이 아니다.

## 현재 상태

[BRIEF](BRIEF.md)에 기존 코드·전문 지침을 연결했다. 네이버 블로그는 [2026-09-21 재개 기록](tasks/20260921-naver-blog-resume.md)을 확인한다. DB 이력은 8월 21일까지 갱신되어 개인 메모리보다 최신이며, 실제 네이버 예약 잔량과는 구분한다. 발행기는 `.worktrees/naver-blog`의 작업을 먼저 확인한다. 나머지 진행 작업은 재개 시 worktree의 task 기록과 변경 내용을 확인한다.

자체 블로그 활동 글은 [2026-09-21 한글·영문 작성·발행 기록](tasks/20260921-activity-blog-ko-en.md)에 있다. 색칠공부·숨은그림찾기·한글/영어 블록 놀이 3주제의 ko/en 6편을 2026-09-21 동시에 공개했다. 소스는 `scripts/_data/marketing/activity-blogs`, 시더는 `seed-marketing-activity-blogs.mjs`다. 활동 콘텐츠는 `activity:*` 메모를 써서 동화책 CTA로 잘못 해석되지 않게 유지한다.

동화책과 마케팅 기획의 명시적 연결은 [2026-09-22 자동 동기화 작업](tasks/20260922-storybook-marketing-source-sync.md)에 있다. 책 원본은 `mkt_content_sources`, 마케팅 기획은 `mkt_contents`이며 `content_source_id`로 연결한다. 신규 책은 저작 승인 시 등록되고 승인된 책 저장 시 원본 메타만 재동기화한다. 한 원본에 여러 기획을 연결할 수 있으며 카드뉴스·릴스·롱폼·공개 블로그의 책 해석도 이 관계를 우선한다. 운영 DB 마이그레이션·기존 책 복구 실행 여부는 작업 기록에서 확인한다.

## 유지할 결정과 이유

정체성/포지셔닝은 STRATEGY.md, 실행 톤/키워드는 brand-brief가 원본이다. 과거 채널 성과 수치를 현재 값으로 인용하지 않는다.

근거 원본은 [BRIEF의 자료](BRIEF.md)와 [공통 기억](../../handoff/MEMORY.md). 상세 원본을 이 파일에 길게 복제하지 않는다.

## 실패·반증과 검증 범위

자동 동기화 작업에서는 server/client typecheck·production build, 서버 원본 서비스 테스트 2개, 클라이언트 목록/카탈로그 테스트 16개를 통과했다. 운영 Supabase 마이그레이션과 R2 동기화는 실행하지 않았다. 원본 지침의 과거 수치는 당시 결과이며 지금도 맞는지 요청 범위에서 확인한다.

## 다음 행동

구체적인 요청과 기존 진행 작업을 대조하여 task를 재개하거나 만든다. 전체 기능을 불필요하게 다시 검토하지 않는다.

## 작업·과거 기록

- [작업 목록](tasks/README.md)
- [분야별 과거 메모리 검색](../LEGACY-MEMORY.md) — 파일명 기반 분류이며 본문 검토 여부와 구분한다.
