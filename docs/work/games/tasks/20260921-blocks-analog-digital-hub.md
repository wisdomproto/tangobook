...
- 랜덤 한글·영어 블록 항목은 어휘 출처 중 선택된 세계명작의 실제 `storybookId`를 끝까지 보존한다. 플레이어는 정답 단어별 원본 책을 미리 불러오고, 칭찬이 끝날 때 아직 로딩되지 않았으면 한 번 더 직접 불러와 해당 장면을 보여 준다. 연결 책이나 매칭 장면이 없을 때만 바로 다음 문제로 넘어간다.
