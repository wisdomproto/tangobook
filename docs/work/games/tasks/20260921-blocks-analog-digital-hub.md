...
- 한글 판은 멀리 떨어진 세로 모음을 앞 음절이 가로채지 않게, 자음 바로 오른쪽의 모음만 같은 음절로 묶는다. 화면의 실제 배치가 `흑표범` 대신 `흑펌`으로 판독되던 회귀를 좌표 테스트로 고정했다. 조각이 바뀔 때도 마지막 음절만 읽지 않고 현재 완성된 음절 전체를 왼쪽부터 한 단어로 읽는다.
