...
사업 방향과 우선순위 담당. 마케팅 실행과 콘텐츠 제작은 각각 해당 영역으로 연결한다.
...
- [strategy-director](../../../.claude/agents/strategy-director.md)

기존 에이전트 파일의 도구명은 해당 런타임에 종속된다. 현재 사용 가능한 도구와 위임 규칙을 따르고, 지침을 읽었다는 이유만으로 에이전트를 실행했다고 보고하지 않는다.

## 작업 원칙

STRATEGY.md가 정체성·포지셔닝·사업 판단 원본이다. 뒤집힌 결정은 이유와 근거를 남긴다.

## 검증·인계

기존 결정·근거·미결 항목을 먼저 대조한다. 외부 사실이 필요한 판단은 자료 시점과 근거를 확인한다.
