...
| `tv.webp` | ⑤ 「거실 TV 도 화면이 됩니다」 뒤 | 768×432 · **뷰어 화면 합성 · 가장 큼** |
...
| 사진 | seed | quad (좌상·우상·우하·좌하) | 넣은 화면 |
|---|---|---|---|
| hero | `380,530` | `(272,387) (455,419) (505,632) (330,653)` | `/library` 세로 3:4 |
| reading | `840,600` | `(745,430) (1037,517) (955,761) (643,645)` | 뷰어 가로 4:3 |
| siblings | `735,355` | `(617,278) (849,279) (849,433) (617,426)` | 단원 화면 16:10 |
| tv | `778,205` | `(527,46) (1066,36) (1066,385) (527,336)` | 뷰어 **16:9** |
| tracing | — | `(-199,864) (650,698) (1080,795) (400,1023)` | ㄱ 써보기 가로 |

🔴 **quad 를 화면보다 크게 잡지 말 것.** 마스크가 잘라 주니 넘쳐도 안 보일 것 같지만, 넘친
만큼 **UI 가 잘린다**(「그림 짝 찾기」 카드가 반쯤 날아갔다). 모자라면 회색이 남으므로 실제
화면 경계에 딱 맞추는 게 유일하게 옳다.

⚠️ 스크린샷을 찍을 땐 **미리보기 도구가 좌하단에 끼워 넣는 배지**를 잘라낸다(앱 UI 가 아니다).

## 📺 TV 컷 — **쓴다** (2026-08-02)

⚠️ 「TV 앱이 없으니 과장이다」라고 판단했다가 바로 뒤집혔다. **틀린 판단이었다.** 탱고북은
웹앱이고 TV 브라우저에서 열면 그냥 열린다. 사용자가 **실제로 그렇게 쓰고 있다**고 확인해 줬다.
「전용 앱이 없다」와 「그 화면에서 못 쓴다」는 다른 말인데 그걸 섞었다.

오히려 TV 는 이 랜딩의 논지에 정확히 맞는다. 대비축이 「패드를 묶어 파는 방문 판매 vs 우리」이고
히어로가 **「패드도 약정도 없습니다」**라고 말한다 — 집에 있는 화면 아무거나 쓴다는 걸 그림으로
보여 주는 게 TV 컷이다.

🔴 TV 는 **유리가 번들거려 반사가 크다**. 반사가 있는 자리는 마스크에서 빠져 UI 에 구멍이 나므로,
반사 금지를 태블릿 프롬프트보다 더 세게 넣는다.

✅ **완료** — `tv.webp` 가 일곱 장 중 **화면이 가장 큰** 컷이다(사진 폭의 45%). 「크게 보이려면
기기가 아니라 거리」라고 적었는데, 거리도 맞지만 **거실 TV 는 원래 프레임을 크게 먹는다**.
같은 자리의 `bedtime`(태블릿 뒤판만 보이던 컷)은 지웠다 — 화면이 든 컷이 같은 말을 더 잘 한다.
본문에도 한 줄 붙였다: 「앱을 따로 깔지 않습니다 … 태블릿도, 폰도, 거실 TV 도 그대로 화면이
됩니다. 패드를 사야 볼 수 있는 게 아닙니다.」
