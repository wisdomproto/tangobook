...
```
CHARACTER SHEET - dingding-papercut   (all four, one line-up)

[여기에 앵커 전문]

[출력] 가로로 넷을 한 줄에 세운다. 배경은 SHEET WHITE #F7F4EC 단색, 바닥 그림자 없음, 글자 없음.
[인물] 왼쪽부터 GRANDMOTHER (Grandma pig) · MOTHER (Mama pig) · DINGDING (Dingding piglet) ·
  MING (Ming duck). 전신 정면 idle, 넷의 키가 한 화면에서 비교되게 같은 바닥선에 세운다.
🔴 CHECK - the sheet fails if any of these is false:
...
- 🔴 **빨간 종이 관습을 뒤집은 이유**(슬레이트 §악센트): 종이를 빨강으로 두면 악센트가 주조가 되어
  「그 아이만 빨갛다」가 죽는다. 검정으로 오려 흰 종이에 붙이면 붉은 조각 하나가 화면에서 **유일한 색**이 된다.
...
- 🔴 **얼굴도 검정이고, 흰 것은 뚫은 자리뿐**이라 페파형이 산다. 검은 머리에 뚫린 흰 구멍은 화면에서 **가장
  ...
  그것이 이 시리즈의 뼈대(`NOTHING FLOATS`)와 정면으로 부딪힌다.
...
| | 옛 앵커 | 지금 |
|---|---|---|
| `RENDERING` | 「머리는 검은 **테**, 그 안이 **흰 얼굴**, 눈·입은 그 흰 자리에 **안 오리고 남긴 검은 조각**」 | 🔴 **폐기.** 머리도 몸과 같은 검정이고 흰 것은 뚫은 자리뿐 |
| `CHARACTER DESIGN LANGUAGE` | 「머리는 통 검정, 눈·**콧구멍**·입은 뚫은 구멍」 | 유지 + **콧구멍 삭제** · 눈 크기 하한 · 입 폭 상한 |

🔴 **왜 음각이 이긴 것이 아니라 양각이 진 것인가** — 흰 얼굴 안의 검은 눈은 **둘레가 전부 흰색인 조각**이라
한 장에서 떨어져 나간다. `NOTHING FLOATS` 는 이 시리즈의 장식이 아니라 **뼈대**이고(무대 시트 §0),
`Baskets` 시트가 이미 같은 문장(`Every hole must leave the black around it connected`)으로 서 있다.
즉 흰 얼굴 셋은 **매체를 어긴 쪽**이었다.

**해골로 안 읽히는 조건** — 밍이 해골로 보인 것은 음각 때문이 아니라 셋 때문이다.
① **코 구멍**(눈 둘 + 코 자리 구멍 = 두개골이다) → 삭제, 돼지 코·오리 부리는 **양각**(윤곽의 검은 혹).
② **눈이 너무 작다** → 눈 구멍 하나가 머리 폭의 **1/4**.
③ **입이 얼굴을 가로지르는 실금**이라 웃음이 아니라 이빨로 읽혔다 → 입 구멍은 **두 눈 바깥끝 폭 이내**,
실금 금지, 다문 얼굴엔 아예 없다.
④ 시트에서만: 구멍이 **배경 마젠타**를 그대로 내보였다 → 머리 뒤에 **흰 종이 한 장**(`dingding-cast.md`).
쪽 삽화는 종이가 이미 흰색이라 저절로 지켜진다.

🔴 **SCENE 8쪽이 표정을 「구멍 모양」으로 지시하고 있다**(01 p8 · 04 p3 · 08 p10 · 12 p6 · 15 p5 ·
16 p5 · 19 p4 · 20 p10 — 「눈 구멍이 가는 틈으로 좁아졌다」·「입 구멍이 옆으로 길게 벌어졌다」).
그 지시가 **살아 있는 쪽**을 골랐다. 흰 얼굴로 갔으면 여덟 쪽의 지문이 통째로 뜻을 잃는다.

**개체 규격은 앵커가 아니라 `dingding-cast.md`** — 앵커는 그 세계 전체를 말하지 한 사람을 말하지 않는다.
할머니·엄마가 남자로 읽힌 것도 같은 자리다(둘 다 「통바지 + 윗도리」뿐이라 **가를 것이 없었다**).
