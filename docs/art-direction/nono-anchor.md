...
> **신규 민팅**이다. 슬러그 = `nono-tornpaper`.
> (설계서 §6 이 적어 둔 `nono-torncollage` 를 이 슬러그로 대체한다.)

---

## §0. 설계 — 이 라인에서 처음 쓰는 계열

앞의 여덟은 **인쇄 공정**(실크스크린·활판·리소·리노컷)이거나 **손자국**(색연필·크레용·과슈·숯)이었다.
찢은 종이는 둘 다 아니다 — **붙인 것**이고, 열 시리즈 중 유일하게 화면에 **물리적인 층**이 있다.
서가에서 이 시리즈만 두께가 보인다.

### 🔴 겨울 25권이라 「눈을 안 그리는 매체」가 필요했다

시리즈 전체가 겨울이다. 250쪽 중 눈이 없는 쪽이 거의 없고, 눈을 매번 **그리면** 250번 다르게
그리게 된다. 찢은 종이는 그 일을 안 한다 — **흰 종이를 찢어 붙인 자리가 그대로 눈**이고,
보풀보풀한 그 가장자리가 눈 둔덕·지붕 눈·쓸어 모은 자리의 윤곽이다. 겨울 25권에 이보다 싼 규칙이 없다.

그래서 이 앵커에는 **흰 것이 둘**이다. 바탕 종이(붙이지 않은 자리 = 겨울 빛)와 그 위에 붙인
**더 흰 눈 종이**. 둘은 밝기와 **가장자리**로 갈린다 — 바탕은 가장자리가 없고, 눈은 늘 찢긴 테를 가진다.

### 🔴 형태를 칼로 만들지 않는다

이 골격이 미오(07 리노컷)와 정확히 반대다. 리노컷은 **칼이 파낸 자리**가 형태이고, 여기는
**손이 찢은 가장자리**가 형태다. 그래서 이 세계엔 곧은 선이 하나도 없다 — 담도, 지붕도, 계단도
미세하게 흔들린다. 겨울 골목의 모든 것(쌓인 눈·다져진 길·김·재를 뿌린 띠)이 원래 딱딱한
윤곽이 없는 것들이라 매체와 무대가 같은 말을 한다.

셋째 색은 **겹쳐 붙인 자리**에서 나온다. 두 장이 포개진 데는 조금 두껍고 그늘이 져서 저절로 어두워진다 —
잉크를 겹쳐 찍는 대신 종이를 겹쳐 붙일 뿐, 이 라인의 골격은 그대로다.

### 🔴 겨울 하나로 25권 — 그래서 무대 조항이 계절이 아니라 「겨울의 얼굴」을 가른다

브루노(05)는 조항 넷이 봄·여름·가을·겨울이었다. 여기는 넷이 다 겨울이다. 설계서 §2 가 겨울의 얼굴을
**열여덟 가지**로 갈라 놨으므로, 앵커는 그것을 **여덟 조항**으로 묶는다 — 첫눈 / 한파와 얼음 /
눈 언덕과 쓸기 / 겨울 시장 / 긴 밤 / 눈에 묻힘 / 김 서린 창과 고드름 / 해빙.

조항이 바꾸는 것은 **바탕 종이와 흰 종이의 관계 하나**다. 첫눈은 흰 종이가 화면을 통째로 덮고,
한파는 그 밑 회색이 드러나고, 긴 밤은 **바탕 종이만 짙은 잿빛으로 갈리고**, 해빙은 흰 종이가
가장자리부터 벗겨진다. 색은 한 장도 안 늘어난다.

### 🔴 시그니처가 사건이 되는 권이 있다 (12)

초록 셋은 「우리 무리」의 표식인데 **12권은 그 초록 하나가 눈에 묻히는 이야기**다.
SCENE 이 이미 그렇게 쓰여 있다 — p1 은 「화면 어디에도 초록이 하나 없다」이고,
p10 은 「사라졌던 셋째 초록이 등불빛 한가운데로 돌아온다」이다.
🔴 **다른 시리즈에서는 악센트 조항을 어기면 사고지만, 이 권에서는 어기는 것이 그 권의 사건이다.**
앵커가 그걸 예외가 아니라 **조항으로** 갖는다.

같은 이유로 **25권 p10 도 조항이다** — 노노가 제 초록 목도리를 눈사람에게 둘러 주는 착지라,
`THREE:` 를 글자 그대로 지키면 목도리가 두 개가 된다.

### 🔴 얼음의 비침 — 이 라인에서 유일하게 허용하는 반사

01·04·05·06·07 앵커가 전부 `NO REFLECTION` 을 달고 있는데, 06권 SCENE 은 「골목 바닥이 매끈하게
얼어 지붕과 벽이 흐릿하게 비친다」로 시작한다. **금지하면 그 권의 첫 쪽이 없어진다.**

찢은 종이는 거울을 못 만든다 — 그리고 그게 답이다. **비침은 같은 종이 한 장을 아래에 눕혀 붙인 것**이고,
안쪽에 아무 무늬도 없다. 뒤집힌 상이 아니라 **얼룩**이다. 모델이 상하 반전을 못 그린다는 실측
(기획서 §배정 규칙, a45 앵커 교체 건)과도 맞아떨어진다.
...
```
STYLE ANCHOR - nono-tornpaper   (three small friends in an old European alley, all winter / torn
...
펴는 장면과 노노가 나는 장면만 예외」로 못박아 두었는데, 이건 **글 규칙이 아니라 그림 규칙**이다.
모델은 「참새」·「비둘기」를 받으면 손을 지우고 날개를 그린다 — 도도(04)에서 정반대 사고(오리에게
손이 생기는 것)를 막느라 `WINGS:` 를 관통 줄에 올려야 했던 것과 같은 자리다.
🔴 **한 권 안에서 팔과 날개를 섞지 않는다** — 설계서가 14권 초고에서 실제로 걸린 것을 기록해 뒀다.

🔴 **`THREE:` 는 인원 제한이 아니라 화면 지시다.** 셋뿐인 무리는 하나가 빠지면 화면에 **빈자리**가
생기는데, 모델은 그 자리를 다른 아이로 채우려 든다. 설계서가 「다섯이면 넷이 가고 하나가 남는 그림이
되지만, 셋은 하나가 빠지면 놀이가 안 된다」로 쓴 그 대가가 **그림에서 보여야** 이 시리즈가 성립한다.
...
🔴 시트가 나오면 기획서 §2 카드에 붙여넣는다(R2 `comic-assets/nono-plan`, 키 = 캐릭터 key).
회차 페이지의 「🎬 이 화 등장」 스트립이 그 이미지를 읽어 온다.

🔴 **`nono-core.js` 의 STYLE 도 이 앵커와 같이 고친다.**
