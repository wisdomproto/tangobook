# 창작동화 1000 — B-08 앵커 + 삽화 프롬프트

> art-director 산출물 (2026-07-31). 배정 근거 = `changjak-assign-08.md` §1 · §2(b08) · §3.3 · §4-3 · 규격 = `_ANCHOR-SPEC.md`.
> 🔴 **대본 SSOT 는 `docs/changjak-books/b08.md`.** 아래 13컷은 그 SCENE 콘티를 그림 지시로 옮긴 번역본이다. 대본은 한 글자도 안 고쳤다.
> 🔴 **이미지 생성은 여기서 하지 않는다.** 사용자가 GPT 로 굽는다. 작가 실명은 한 글자도 안 들어간다.
> 🔴 **실행 순서**: ① `TheFive`(🔴 **이 책에서 그려지는 물건 전부** — 이 다섯이 곧 눈사람이다) → ② `KidGoat` → ③ `GranGoat` → ④ `FenceCorner` → ⑤ **p1** → ⑥ **p13**(p1 과 같은 구도·같은 거리) → ⑦ 나머지 열한 컷. 🔴 **p1 과 p13 을 나란히 놓고 승인한다** — 같은 자리·같은 프레임에서 **위 칸을 끊는 것이 안 칠한 종이인가, 그려진 몸인가**만 다르면 성공이다.

---

## B-08 §1. 앵커 배정

**권**: `b08` 「눈사람이 두고 간 편지」 (13쪽 · 4~6세 · 주제군 **B 상상·변신** · 엔진 **관찰과 성장** · 무대 동유럽 눈마을 집 마당의 나무 울타리 귀퉁이 · 주인공 아기 염소 + 할머니 염소)

**한 줄**: 🔴 **차가운 흰 종이가 곧 눈이고 곧 눈사람이다 — 눈과 눈사람에는 윤곽선이 한 획도 없다. 이 책에서 그려진 것은 물건 다섯·울타리 세 줄·두 염소뿐이고, 그 목록은 열세 쪽 내내 안 늘어난다.** 앵커 슬러그 `changjak-fivemarks` — **신규 민팅**(배정표 지정, 변경 없음).

**이 권이 그림에 요구하는 것** 넷.

1. 🔴 **눈사람에 닫힌 윤곽이 있으면 실패다.** 첫 렌더 판정문이 이 한 줄이다(배정표 §3.3). 눈사람은 **그려지지 않고 남겨진다.**
2. 🔴 **그러면 눈사람이 어떻게 보이나 — 세 가지로만 보인다.** ①**울타리 가로대가 그 자리에서 끊긴다**(가림) ②발밑에 **평평한 회청 그림자 한 덩이** ③몸에 붙은 **물건 다섯의 배치**. 🔴 **자(尺)는 ①이다** — 끊긴 길이가 곧 키다.
3. 🔴 **자가 둘이고 둘 다 그림에 있어야 한다.** ⓐ나뭇가지 팔 끝이 닿는 **칸**(위→가운데→아래→풀 위→p13 아기 염소 어깨가 위 칸) ⓑ모자를 씌우는 **자세**(발끝 들기→팔만 뻗기→무릎 굽히기→앉아서 집기). 쪽마다 둘 다 컷에 적는다.
4. 🔴 **없어지는 것이 아니라 옮겨지는 것으로 끝나야 한다.** p12 에 놓인 다섯은 **처음부터 그려져 있던 그 다섯**이고, p13 에 그것들이 전부 아기 염소 몸 위에 있다. 그러니 다섯은 **한 번도 새로 그리면 안 되고**, 시트 승인본을 그대로 옮긴다.

**왜 이 공정인가.** 대본이 「눈사람은 말하지 않는 소품」이고 착지가 「손바닥에 옮김」이다. 그러면 그림이 할 수 있는 가장 센 일은 **눈사람을 처음부터 한 획도 안 그리는 것**이다. 그리면 p12 에서 **없어진 것**이 생기고 책이 슬픈 쪽으로 샌다. 안 그리면 p12 에 남는 것은 **원래부터 그려져 있던 다섯**뿐이고, 사라진 것은 **안 칠한 면적**이다. 대본이 박아 둔 안전핀 셋에 그림체가 넷째 핀을 보탠다.

**🔴 형제 권과 갈린 축** (배정표 §1 · §3.3 확정, 첫 렌더 판정 기준).

| 대상 | 갈림 |
|---|---|
| **c01** `changjak-blackice` (겨울 · 흰 것이 안 칠한 종이) | 🔴 **흰 것에 가장자리를 누가 주나.** c01 = 검정 숲 평칠이 흰 것을 둘러싸 **칠한 면 안에서 형태를 갖는다** / b08 = 흰 것을 둘러싸는 것이 **울타리 세 줄과 회청 그림자 한 자락뿐**이라 닫힌 윤곽이 없다. 🔴 **첫 렌더 판정 = 눈사람 윤곽선이 그어져 있나(있으면 실패)** |
| **b06** `changjak-risenpaper` | **흰 것에 부피가 있나.** b06 = 밀어 올린 양각(솟았다 꺼진다) / b08 = **평평한 맨 종이**, 줄어드는 것은 면적이다 |
| **d09** `changjak-pressmark` | **흰 것 밑에 자국이 있나.** d09 = 눌린 홈이 새 눈을 뚫고 비쳐 나온다 / b08 = **눈 밑에 아무것도 없다** |
| **c04** (같은 배치의 겨울 권) | **흰 것이 안 칠한 종이인가 칠한 면인가 · 줄어드나 쌓이나.** c04 = 칠한 회백 얼음면 위에 자국이 쌓인다 / b08 = **안 칠한 종이가 줄어든다** |
| **f06** `changjak-whitenight` | f06 = 옅은 광역 워시(칠해져 있다) + 남긴 연필 겹수 / b08 = **워시도 연필도 0**, 흰 것은 손이 한 번도 안 간 자리 |

**🔴 라인 충돌 확인 — 이번 배치 최대 위험이 이 권이다(§4-3).**

| 대상 | 겹치나 | 왜 |
|---|---|---|
| 호리 **니들펠트** | ✕ (**최대 위험**) | 눈사람·염소·털모자·빨간 목도리가 니들펠트 소재 그 자체다. 🔴 보풀·바늘땀·섬유 엣지가 **한 올만 보여도 호리 라인**이다 → NOT 절 `no wool` · `no stitching` · `no fibre edge` · `no felted fuzz` · `flat paper surface` 를 박고, 털모자·목도리·염소 털을 전부 **획 개수 상한**으로 못박는다(털모자 = 평칠 + 띠 하나, 0 fibre marks) |
| 전래 **점눈이** | ✕ | ① 지지면 = **차가운 흰 종이**(전래는 밝은 크림 = 햇빛) ② 매체 = **평칠 + 마른 붓**(느슨한 색연필 낙서 아님, 획에 떨림이 없다) ③ 🔴 **빨강 1점 규칙 ✕** — 여기 빨강은 점이 아니라 **열세 쪽 내내 같은 한 물건**(목도리)이고 마지막에 주인이 바뀐다 ④ 눈 = 아몬드 + 염소 가로 동공(점눈 아님) |

**🔴 대본 SCENE 처방 5건** — 대본은 고치지 않고 컷에서 분기한다.

| # | 대본 | 문제 | 그림 처방 |
|---|---|---|---|
| 1 | p6 「가까이서 본 눈 표면의 오돌토돌한 질감」 | 질감을 그리면 눈에 마크가 생기고 법이 깨진다 | 🔴 **눈에는 0획.** 가까움은 **크기**로만 낸다 — 단추와 앞발이 화면을 채우면 그것이 클로즈업이다 |
| 2 | p10·p12 「젖은 것들이 전부 빛을 되받아 반짝인다」 | 반짝임 = 다섯째 광원 | 🔴 반짝임 대신 **젖은 자리를 한 값 어둡게**(WET EARTH 한 단). 하이라이트 점·별표·글로우 0 |
| 3 | p2 「모자가 높다는 것이 앵글로」 · p7 「얼굴이 같은 높이」 | 앵글이 자를 대신하면 자가 둘로 갈린다 | 🔴 앵글은 쓰되 🔴 **판정은 언제나 RAIL 과 POSTURE 두 줄**이다. 컷에 두 줄을 반드시 남긴다 |
...
**🔴 매 컷 확인하는 세 줄** — `RAIL:`(나뭇가지 팔 끝이 어느 칸인가 · 가로대가 어디서 끊기나) · `POSTURE:`(아기 염소가 어느 자세인가) · `WHITE:`(안 칠한 종이가 화면의 몇 % 인가 · 아래에서 갈색·초록이 어디까지 올라왔나).

| | p1 | p2 | p3 | p4 | p5 | p6 | p7 | p8 | p9 | p10 | p11 | p12 | p13 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| **RAIL** | 🔴 **위 칸** | 위 칸 | (프레임 밖) | 🔴 **가운데 칸** | 가운데 한 줄 | (없음) | 🔴 **아래 칸** | 셋 다 안 끊긴다 | (없음) | 🔴 **풀 위** | 아래 칸 한 줄 | 셋 다 안 끊긴다 | 🔴 **위 칸 = 아기 염소 어깨** |
| **POSTURE** | 까치발·목에 두르기 | 🔴 **발끝 들기** | 앞발 오목하게 | 위 칸을 짚고 아래를 봄 | 🔴 **팔만 뻗기** | 배에 두 앞발 | 쪼그려 앉기 | 🔴 **무릎 굽히기** | 깊이 숙여 매듭 | 무릎 꿇고 안기 | 제 머리에 모자 | 🔴 **앉아서 집기** | 서서 앞발 모으기 |
| **WHITE** | 60% | 60% | 55% | 50% | 55% | 70% | 45% | 🔴 **35% · 갈색 첫 등장** | 30% | 25% · 🔴 **초록 첫 등장** | 40% | 🔴 **20% · 눈 0** | 20% |
...
## B-08 §3. 캐릭터 시트 (🔴 TheFive 를 가장 먼저)

```
CHARACTER SHEET - TheFive   (bake FIRST, before anything else - this book is these five things)

The five belongings on a plain PAPER #F4F5F6 sheet, alone, no snowman, no animals, no scenery.
🔴 THESE ARE THE ONLY OBJECTS DRAWN IN THE WHOLE BOOK AND THEY NEVER CHANGE. Bake them once and
copy them onto every page.

🔴 PLATE 1 - EACH ONE ALONE, in a row at one size:
  A · a brown knitted hat, HAT BROWN #7A6248, one flat shape with one darker band round the brim,
    slightly floppy at the top. 🔴 0 fibre marks, 0 knit texture, 0 bobble.
  B · a long red scarf, SCARF RED #C0342E, one flat shape with at most 3 fold marks and two short
    fringed ends.
...
🔴 PLATE 3 - LAID DOWN: the same five lying flat on wet grass in that same arrangement, seen from
  ...
  value darker than the grass around it.
Also: the scarf held stretched between two paws, and one button pressed under a paw pad.
0 lettering, 0 numerals. SCENE token: TheFive.
```

```
CHARACTER SHEET - KidGoat   (bake SECOND, attach as @image2)

Same make as the book: flat opaque paint with a clean edge on the bare sheet. 🔴 Do not render soft
fur, wool or felt anywhere on this animal, and do not light her.

BODY: a young goat, small and round-bellied, PAPER-white coat with WET WOOD hooves and a
  WET WOOD #3A332C muzzle line, two short straight horn buds, ears out sideways, a short tail.
  🔴 FUR = AT MOST 6 SHORT MARKS ALONG THE OUTLINE, never a texture, never one hair at a time.
  She wears nothing.
FACE: one dark almond eye a side with a small pale horizontal slit pupil, no eyebrow, a short mouth
  line. 🔴 No blush, no catchlight, no glossy eye, no tears at any point in the book.
🔴 THE POSTURE LADDER IS THE RULER OF THIS BOOK - bake all five in a row at one size, because the
  page is read by which one she is in:
  1 · TIPTOE - up on the very tips of the hind hooves, back arched, both arms stretched straight
    overhead, 🔴 HEELS CLEARLY OFF THE GROUND, mouth open, looking up.
  2 · ARM ONLY - standing square, 🔴 HEELS FLAT ON THE GROUND, back straight, one arm reaching
    forward at shoulder height, the other at her side.
  3 · KNEES BENT - both knees bent and the back rounded over, both arms reaching DOWNWARD, head
    lower than her own shoulders.
  4 · KNEELING - down on both knees, arms wrapped round something low, cheek laid against it, eyes
    shut, a quiet face that is neither smiling nor crying.
  5 · PICKING UP - kneeling, one paw lifting a small thing between the pads, the other paw already
    holding something, head down, eyes wide.
Also: standing with both front paws cupped together at her chest, and one paw held out flat with a
  single drop on the pad. Plain sheet, no scenery. 0 lettering, 0 numerals. SCENE token: KidGoat.
```

```
CHARACTER SHEET - GranGoat   (bake THIRD, attach as @image3)

Same make and same edges as KidGoat - attach the approved KidGoat sheet so the two are made by one
hand and the size between them is settled. Plain bare sheet.

BUILD: an old goat, tall and narrow: a long straight face, a chin beard as at most 5 marks, curved
  horns, bony shoulders, a coat one value greyer than the kid's so 🔴 SHE IS NEVER THE WHITE OF THE
  SNOW. She is about twice the kid's height and perspective must not flip that. 🔴 AT THUMBNAIL SIZE
  SHE IS A TALL NARROW UPRIGHT AND THE KID IS A SMALL ROUND ONE.
CLOTHES: one dark shawl over the shoulders, nothing else. Black boots on one page only.
FACE: one dark almond eye a side with a slit pupil, laugh creases as at most 3 marks. 🔴 She never
  explains anything and never looks sad - she is busy with her own work and glances over.
POSTURES - three at one size: HANGING WASHING (arms up to a line, a grey blanket over one arm,
  head turned back over the shoulder) · PULLING ON A BOOT (seated on a doorstep, one boot half onto
  a hind hoof, eyes on her own foot) · STANDING BY (upright, hands at her sides, only the head
  turned).
Plain sheet, no scenery. 0 lettering, 0 numerals. SCENE token: GranGoat.
```

```
CHARACTER SHEET - FenceCorner   (bake FOURTH, attach as @image4 - the stage and the ruler)

The place, with no animals and no snowman, in the book's make: bare sheet and flat paint.

FROM THE SIDE, EYE LEVEL: a wooden fence corner running across the frame - ONE CORNER POST and
  THREE HORIZONTAL RAILS (top, middle, bottom), all WET WOOD #3A332C, each rail one flat band with
  at most 2 grain marks. Behind and around it, 🔴 NOTHING AT ALL: the sheet is left bare. No house,
  no trees, no village, no sky, no falling snow.
🔴 PLATE 2 - THE RULER, four small versions of the same fence in a row, showing how a snowman that
  is never drawn is read: (a) all three rails BROKEN over a tall width - the bare paper between the
  broken ends is the snowman · (b) only the middle and bottom rails broken · (c) only the bottom
  rail broken · (d) 🔴 all three rails running unbroken end to end, with two sticks lying on the
  ground below the bottom rail. 🔴 In every one of the four, the paper in the gap is completely
  bare - 0 outline, 0 tint, 0 shading.
🔴 PLATE 3 - THE SHADOW: one flat COLD GREY #9FB0BC shape lying on the ground beside the gap, hard
  edged, with 0 gradient; its length is the height of the thing that is not drawn.
Also: the same fence with morning shadow bars falling from the rails, at most 3 flat grey bands.
0 lettering, 0 numerals. SCENE token: FenceCorner.
```
...
각 컷 = `STYLE ANCHOR + @image1(TheFive) + @image2(KidGoat) + @image3(GranGoat) + @image4(FenceCorner) + 아래 블록`.
🔴 **p1 → p13 순으로 먼저** 굽는다. 나머지 열한 컷은 그 둘 사이에서 자를 읽는 것뿐이다.

### p1 — 이거 두르고 있어 🔴 무대·법 고정 · 먼저 굽는다

```
CAMERA: wide, eye level, from the side. 🔴 The kid and the snowman stand at the same depth, side by
  side, so their heights compare directly. Fix this framing - p13 repeats it exactly.
...
FINISH: 2 (the kid + the red scarf). 0 lettering, 0 numerals.
```
...
WHITE: about 60%. Two pairs of small hoof prints are drawn in the snow as 🔴 flat COLD GREY marks,
  at most 4 in all - they are the only marks ever allowed on the snow, and they are shadow, not
  outline. Everything else is bare.
FINISH: 2 (the tiptoeing kid + the tilted hat). 0 lettering, 0 numerals.
```

### p3 — 차가워!

```
CAMERA: medium wide, eye level, the frame split by the eaves line into roof above and yard below.
...
WHITE: about 55%. 🔴 The one page where extra props are allowed, and only these: one washing line,
  ...
  eaves the snow is pitted by drips as at most 6 small COLD GREY marks.
FINISH: 2 (the cupped paw with the drop + the grandmother at the line). 0 lettering, 0 numerals.
```

### p4 — 어제는 위 칸이었는데

```
CAMERA: medium, eye level, straight from the side 🔴 so that all three rails are visible end to
  end. This is the measuring page.
...
FINISH: 2 (her paw on the empty top rail + the stick tip on the middle rail). 0 lettering.
```

### p5 — 팔만 쭉 뻗어

```
CAMERA: medium close-up, eye level - 🔴 the low angle of p2 has flattened out. The two faces are at
...
WHITE: about 55%. The head and shoulders of the snowman are bare paper carrying the hat, two
  ...
  only edge is where a drawn thing lies across it. Everything outside is left empty.
FINISH: 2 (the outstretched arm + the hat). The snow surface is not shiny: 0 sparkle, 0 stipple.
...
```
CAMERA: close-up over the shoulder - only the kid's two front paws and a belly's worth of bare
...
WHITE: about 70% - the emptiest page in the book. 🔴 On the bare paper there is ONE MARK ONLY: a
  ...
  texture, 0 stipple, 0 shading anywhere else. One end of the red scarf hangs down beside the paws.
FINISH: 2 (the pressing paw with the button + the groove). 0 lettering, 0 numerals.
```

### p7 — 이제 눈이 딱 맞네

```
CAMERA: medium, 🔴 LOW angle, dropped so that the two faces sit at the same height.
...
WHITE: about 45%. The yard is cracking: at most 5 flat HAT BROWN #7A6248 patches of wet earth show
  ...
  shadow lies from the gap to the left.
FINISH: 2 (the crouching kid + the stick tip on the bottom rail). 0 lettering, 0 numerals.
```

### p8 — 마당이 질척하구나 🔴 갈색이 처음 들어오는 쪽

```
CAMERA: medium wide, 🔴 HIGH angle looking slightly down - the exact opposite of p2.
...
WHITE: about 35%. 🔴 WET EARTH ENTERS THE BOOK HERE: the bottom third of the frame is HAT BROWN
  ...
  shape. A ring of standing water round the mound, one flat COLD GREY shape. One pair of boots.
FINISH: 2 (the stooping kid + the low mound with the hat). 0 lettering, 0 numerals.
```
...
WHITE: about 30%. Above the scarf, a bare mound with the buttons and stone; below, wet brown ground
  ...
  one value darker, with a faint red stain bleeding into the brown beneath it - the only place in
  the book where two colours touch.
FINISH: 2 (the two paws on the knot + the darkened scarf end). The red is at its strongest here and
  everything else is grey-brown. 0 lettering, 0 numerals.
```

### p10 — 안아도 돼?

```
CAMERA: medium, eye level, slightly off the front.
...
WHITE: about 25%. 🔴 GRASS ENTERS THE BOOK HERE: a few blades of GRASS #6E8A52 beside the sticks,
  ...
  are shown 🔴 one value darker, never by sparkle or highlight.
FINISH: 2 (the kneeling kid + the two sticks on the grass). 0 lettering, 0 numerals.
```
...
SUBJECT: KidGoat at the centre with the brown hat pulled down onto her own head, 🔴 THE BRIM DOWN
...
FINISH: 2 (her face under the brim + the one remaining button). This page sits one step brighter
  than its neighbours; do it with less drawn, not with light. 0 lettering, 0 numerals.
```

### p12 — 하나씩 주워요

```
CAMERA: 🔴 HIGH angle straight down over the ground, so that the arrangement reads as a shape.
...
WHITE: 🔴 20%, AND NONE OF IT IS SNOW - there is not one patch of snow left in the frame. The
  ...
  unchanged from TheFive plate 3: two buttons at the top, the stone below them, the two sticks out
  to the sides, the red scarf coiled at the bottom - 🔴 and the grass directly beneath that
  arrangement is pressed flat and one value darker, an exact snowman-shaped patch.
FINISH: 2 (the lifting paw with the button + the flattened patch). Low sun: 🔴 each blade is edged
  by leaving a hair of bare paper along it, not by adding white paint. 0 lettering, 0 numerals.
```

### p13 — 여기 다 있어 🔴 p1 과 같은 구도 · 먼저 굽는다
