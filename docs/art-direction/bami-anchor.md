...
Style: opaque gouache on warm dark-taupe paper, a picture book read aloud to children of four to six.
  ...
  bare paper; an owl's back, a wooden wall and a guest's fur are paper left alone. LIGHT IS PAINT: a
  thing is bright because an opaque patch was put on it, flat, matte, one pass, hard cut edge. No wash,
  no fade, no glow - a lamp does not radiate, it lays a shape of paint on what it reaches and stops.
  Past the reach of the light nothing is painted and nothing is drawn.

RENDERING (finish hierarchy): one measure, HOW FAR THE LIGHT REACHES, gives all three finishes.
...
PALETTE (only these): PAPER #75695F warm taupe - the dark, the unlit, the body of every owl, the cat, the
  ...
  guest's coat, a flower, the sky or water. 🔴 IT IS ON EVERY PAGE BUT NOT ALWAYS ON BAMI, and its place is
  the weather and the hour: round the neck (cold) · knotted at the belly (warm) · on its peg or a bunk post
  (asleep, or being washed, or not yet learnt) · laid flat along a table · one end out of a quilt. Where a
  new scarf is held open beside the worn one there are two teals and both are his; that is the one page in
  the series with two.
...
STAGE CLAUSES (one small train that is the family's home, the station it stands at, and the one view
  ...
  🔴 A CAR SEEN FROM INSIDE - the wall is paper and a lamp is the only light: ONE LAMP patch on the table,
    ...
    🔴 A FROZEN PANE IS THIS CLAUSE TURNED OVER and is the only window that is: wherever the script calls the
    glass frozen, the pane is ONE solid FROST slab with no hint of the view through it, and the view exists
    only inside the round breathed HOLES cut in that slab, each edge falling in one clean line, no blur, no
    crystal pattern. Used with the snow-field view and nowhere else.
  🔴 RUNNING = ONE TILT. While the train runs, every hung thing - lamp, strap, the scarf on its peg, a cup
    on its hook - leans the same way at ONE ANGLE FOR THE WHOLE VOLUME, 5 to 15 degrees - the only book that varies its
    angle is the one whose subject is the bend of the track, and it steps 7, 8, 10, 12, 15 - and what lies loose
  ...
  DINING CAR - the stove IRON with a LAMP mouth, its light stopping at the near table and the cat; table
    one LAMP plank; lunchbox one FROST rectangle with one IRON lid
    line, its three compartments read by what is in them and never by colour; cup one FROST shape; the window
  ...
  SLEEPING CAR - two bunks, paper, one LAMP plank each; the round window a FROST disc by day, paper by
    ...
    dry-dragged strokes - a heap of foam is those same 5 strokes overlapped into one mounded outline and
    never a sixth, and foam covering a body is not strokes at all but ONE broad flat FROST patch out of
    which only the face disc and beak come; towel FROST; toothbrush one IRON mark.
  ...
  🔴 A BIG STATION, INDOORS (wherever the script puts the train's stop under a roof, the small-station
    ...
    FROST clock disc with two IRON hands sits at the very TOP of the page, and the child is small under it.
    The crowd is legs and bodies only, cropped by the frame's top edge, FACES NEVER DRAWN - a stand of IRON
    uprights, each with one LAMP patch where a lamp reaches it; the way out to the platform is ONE bright
    rectangle at the far end. The floor is paper and only what stands under a lamp is painted.
  🔴 THE KINDERGARTEN CLASSROOM (the one room in the series that is neither train nor station) - wall paper,
    ONE window a FROST rectangle with the sun coming in as ONE patch in the window's exact shape on the
    floor, the near half of the room lit and THE INNER HALF FLAT IRON SILHOUETTES with nothing inside them:
    the other children have no species and no marks. 🔴 ITS WINDOW AND A CAR'S WINDOW ARE THE SAME WINDOW
    SEEN FROM THE TWO SIDES - the train is out of that window, small, on its platform line below the hill,
    engine at the right; the two pages face each other and the camera must be able to swap them.
...
CHARACTER DESIGN LANGUAGE: 🔴 THE OUTLINE CARRIES THE FEELING - NOT THE EYES. An owl says what it feels
  ...
  it is tall · startled = drawn up thin and tall, half its easy width, feathers flat · curious = the face
  disc tilted off the body's vertical, up to a right angle, body unchanged · pleased = one straight IRON
  lid line across the top of both eyes, body round · cold or cross = puffed wider than tall.
  ...
  NABI the cat: four legs, no clothing, cat proportions, claws never out, and 🔴 NEVER STANDING - lying,
  sitting or stretched on every page, the lowest and longest figure there is; her eyes are two LAMP
  dots, the smallest LAMP marks in the book, shut whenever Ppippi is within reach.
  PPIPPI the sparrow: thumb-sized against Bami, 🔴 NEVER ON THE GROUND AND NEVER BELOW ANOTHER FIGURE'S
  FEET - on a wide page it is the highest figure in frame, in the air or on the highest edge; on a close
  page it perches on the highest thing inside that frame, a sill, a chimney or a cup's rim; chatter is the beak open, nothing drawn coming out.
  🔴 GUESTS: one party a volume, one sheet a species, and A GUEST IS KNOWN BY ITS LUGGAGE - one bag, case,
  basket or bundle touching it on every page it is on - EXCEPT where the script makes empty hands that
  guest's own mark, and then nothing is invented for it on any page of that book. Nobody in the cast carries
  a guest's luggage; Bami's own knotted cloth bundle is not a guest sign and is the one thing the cast owns.
  The stationmaster's flag is his luggage. A guest's coat is FROST, LAMP or IRON, never the accent. Guests
...
```
REACH:   the paper is the dark; a thing is painted only as far as the light reaches it, in one flat hard-edged patch
LIGHT:   three paints and the one teal - LAMP warm, FROST cool, IRON dark - and fog is the one dry-dragged stroke
OUTLINE: the body's width and the tilt of the face carry the feeling, the eyes never change size, and the one teal is on the page - on Bami or in its place
```

🔴 **컷에 붙일 때는 그 쪽 자리의 `STAGE CLAUSES` 한 조항(칸 하나 · 통로 · 지붕 · 바깥 · 역 · 큰 역 · 교실 중 하나) +
그 권의 창밖 조항 한 값 + 시간대 조항만 붙인다.** 자리 조항이 열하나(칸 다섯·통로·지붕·바깥·역·큰 역·교실)인 것은
25권이 한 기차를 나눠 쓰기 때문이고, **한 쪽에 동시에 쓰이지 않는다.** 🔴 그림으로 굽는 것은 조항이 아니라
`bami-stages.md` §1 의 **자리 시트 아홉 장**이다 — 조항은 컷에 붙는 문장이고, 시트는 그 자리를 한 번 정해 두는 판이다. 「기차는 집」·「안에서 본 칸」·「달림 = 기울기」 세 조항은 자리가 아니라
...
🔴 **개체 규격은 `bami-cast.md` 가 SSOT**(아직 없다 — 이 시리즈의 다음 일). 아래는 다섯을 한 장에 굽는 단체
...
SHEET LAYOUT: one sheet of bare dark-taupe paper - 🔴 THE GROUND IS NOT PAINTED AND THERE IS NO SCENERY,
  and there is no lamp: every figure is lit from the top left by day, so each has ONE FROST patch on its
  upper-left side and paper on the rest. Five figures stand on a single ground line at their true relative
  heights: Dad tall and narrow with cap and tufts, Mom wide and low with apron and tufts, Bami between
  them at Mom's shoulder with no tufts and the teal scarf, Nabi lying along the ground line, and Ppippi
  perched on the top edge of the sheet. Each owl is drawn THREE times - standing front, three-quarter
  walking with one foot lifted and holding a thing, and the head and body outline alone, large enough
  that the width of the body against its height is clear. Feet are drawn in every one of them, so the
  one-foot rule is fixed before any prop exists.
🔴 TEST ON THE THREE LARGE OWL OUTLINES: paint the eyes out entirely and the feeling must still be readable
  from the width of the body and the tilt of the face alone. If it is not, the eye was doing the work and
  the sheet is wrong.
🔴 ONE EXTRA ROW AT THE FOOT OF THE SHEET: the same five again, side by side, filled solid in flat IRON
  with 0 interior marks and no teal at all. THREE THINGS ARE READ OFF THAT ROW AND THE SHEET IS WRONG IF
  ANY ONE FAILS: the three owls are told apart by width and by points (two points tall, two points wide,
  no points small), NOT by the cap or the apron; Nabi is the only shape longer than it is tall; and
  Ppippi is the only shape not touching the ground line.
🔴 A SECOND SMALL ROW: Bami's whole outline five times at thumbnail size - at ease, startled, curious,
  pleased, cross - WITH THE EYES PAINTED OUT. The five must still be told apart from the outline alone.
  Nothing else on the sheet: no props beyond the three worn things, no scenery, no shadow, and no
...
## §2. 왜 이 공정인가 — 그리고 왜 `boyd-flashlight` 인가

### §2-a. 🔴 과슈를 고르되 **종이를 어둡게** — 이 라인에서 처음 뒤집는 골격

사용자 후보는 둘(과슈 / 색연필+수채 밑칠)이었다. **둘 다 그대로는 못 쓴다** — 시리즈 등록부에 이미 있다.

| 후보 | 이미 쓰는 시리즈 | 겹치는 겹 |
|---|---|---|
| 과슈(크림 종이) | **04 `dodo-gouachemill`** | 불투명 · 붓이 못 미쳐 남는 종이 테 · CHALK |
| 불투명 덧얹기 | **16 `kota-mineral`** | 불투명 · 종이 안 남김 · 패스 수가 눈금 |
| 색연필 | **03 `mei-pencilslope`** · 전래동화 **점눈이(2호)** | 색연필 + 밝은 종이 |
| 수채 밑칠 | **17 `moya-daylight`** · 세계명작 수채동화풍 | 젖은 종이 · 종이가 빛 |

색연필+수채는 **세 라인**과 닿고(mei · 점눈이 · moya), 과슈는 둘(dodo · kota)과 닿는다. 과슈 쪽이 가깝고,
**닿는 겹을 하나로 뒤집을 수 있는 축이 있었다** — 종이다.

🔴 **이 라인 열일곱 앵커는 예외 없이 「칠하지 않은 종이가 빛」이다**(§8 골격: 「가장 어두운 것은 만들어진 것이지
갖고 온 게 아니다」). 코타만 종이를 남기지 않는데, 그것도 「밝은 것도 칠한다」이지 종이가 어둡지는 않다.
이 시리즈는 **종이가 어둠**이고 **빛이 만들어진 것**이다 — 골격의 부호를 뒤집었다. 뒤집은 이유는 취향이
아니라 **내용**이다:

- 사용자가 부른 장면 셋(난로 불빛 · 김 서린 창 · 눈 오는 플랫폼의 등불)이 **전부 「어둠 위의 빛」**이다.
  「저녁 등불 장면이 많다」는 지시까지 합치면 25권의 절반 이상이 그렇다. 밝은 종이에 어둠을 칠해서 그 장면을
  만들면 **가장 큰 면적(밤)이 가장 많이 칠한 것**이 되어 §2.7(획의 절약)이 첫 쪽부터 무너진다.
  종이가 어둠이면 **밤은 0획**이고 등불만 칠한다 — 덜 그리기가 광원의 결과로 따라온다(단권 g88 이 검정 판에서
  확인한 것, `verified-references` §7.10).
- 과슈는 **밝은 것을 어두운 위에 불투명하게 얹을 수 있는** 유일한 수성 매체다(dodo 의 CHALK 조항이 같은
  성질을 「튀는 것」에만 썼다). 여기서는 그 성질이 **화면의 전부**다 — 등불·눈·안개·김·달·햇빛 전부가
  「어두운 위에 얹은 밝음」이다. 매체의 유일한 성질이 시리즈의 유일한 사건과 같으면 그게 필연이다.
- **눈금이 하나로 떨어진다** — 「빛이 어디까지 닿았나」. 모야의 「종이가 얼마나 젖었나」, 코타의 「몇 번
  갈아 얹었나」와 같은 층의 한 변수이고, 세 마감이 그 하나에서 나온다(§1 `RENDERING`).

⚠️ **「보기만 해도 기분 좋은」과 어두운 종이가 부딪히나** — 종이를 검정이 아니라 **따뜻한 타우프 중간톤**
(#75695F)으로 잡았다. 등불 크림이 이 종이 위에서 가장 넓은 칠이 되므로 화면의 온도는 **가장 많이 칠한
색**이 정하고, 그건 LAMP 다. 검정 판(g88 #16181C)과의 거리는 §3.

### §2-b. `boyd-flashlight` — 표에 없던 것 셋

| | |
|---|---|
| id | **`boyd-flashlight`** · BolognaRagazzi 2015 Fiction 위너 · C6 검정 바탕 위 과슈 |
| 카탈로그 `medium` | 「검정 바탕 위 회색 과슈 + 손전등 빛 부분만 **컷아웃처럼** 밝게 오려냄」 |
| 카탈로그 `character` | 「아이는 회색 실루엣 + 흰 점 눈 하나. **빛 밖에선 형태만 남는다**」 |
| 가져온 축 | 🔴 **빛이 마감 위계를 정한다**(공정) + **빛 밖에서 남는 것은 윤곽뿐**(캐릭터 → 윤곽이 표정을 진다) |

🔴 **표만 보고 정하지 않고 표지를 받아서 봤다**(1,645×1,853). 표에 없던 것 셋:

1. 🔴 **빛의 가장자리가 직선이다.** 손전등 빛이 원뿔이 아니라 **자로 그은 쐐기**이고, 그 선이 나무·새·땅을
   가리지 않고 **통과해 자른다** — 나뭇가지 하나가 빛 안에서는 초록 잎, 밖에서는 회색 선이다. 「컷아웃」이
   비유가 아니라 **직선 한 줄**이었다. → `TIME OF DAY` 의 「lamp's reach is a hard-edged cone or disc cut
   straight through whatever it crosses」가 여기서 나왔다. AI 실패 모드(글로우)를 막는 조항이 형용사가 아니라
   **기하**가 된 자리.
2. 🔴 **빛 밖에서 사물은 사라지지 않고 「회색 선」으로 남는다.** 카탈로그는 「형태만 남는다」로 적었는데, 실제는
   **가는 회색 선으로 다 그려져 있다**(풀·잎·아이의 옷 주름까지). 우리 앵커는 이걸 **안 가져왔다** — 4~6세에게
   「읽기 쉬운」이 우선이라 빛 밖은 **0획 또는 IRON 실루엣 하나**로 더 비웠다(§2.7). 가져온 것은 「빛 안에서만
   색과 마감이 생긴다」는 위계 하나다.
3. 🔴 **빛 안에 올빼미가 있다.** 표지의 유일한 채색 동물이 **올빼미**이고, 노란 눈 두 개 + 얼굴 원반이 그
   빛 안에서 가장 먼저 읽힌다. 우연이지만 「이 참조로 올빼미 캐스트를 세운다」에 근거가 실재한다 —
   **얼굴 원반이 곧 빛을 받는 자리**라는 앵커의 조항(`THE FACE DISC IS THE ONE THING… PAINTED`)이 참조 그림에
   그대로 있다.

- ⚠️ **안 가져온 것** — 검정 종이(우리는 중간톤 타우프 §2-a) · 빛 안의 다색(연두·빨강 — 우리는 LAMP/FROST
  둘) · 빛 밖의 회색 선묘(위 2). 가져온 것은 위계와 직선 가장자리, 그리고 캐릭터 축의 방향뿐이다.
- 🔴 **C6 는 단권 앵커 일곱(a05·g01·g02·b10·h01·a47·d12)이 이미 `boyd-flashlight` 을 딛고 있다.** 시리즈에서는
  처음이고, 단권들과의 거리는 §3 의 g88 행이 대신 말한다(g88 이 그중 가장 가깝다).

### §2-c. 🔴 의인화 등급 판단 — 서서 걷고, **잡는 것은 발**
...
- 🔴 **옷은 「한 사람에 하나」**(모야와 같은 절제): 아빠 모자 · 엄마 앞치마 · 밤이 목도리. 깃털 위에 소매·바지를
  입히지 않는다. 대본에 **「손」「팔」「소매」를 쓰지 말 것** — 「발로 집었다」「날개를 펼쳤다」로.
- 🔴 **어른 표지 = 귀깃(ear tufts) 두 점.** 밤이만 없다 → 통째로 메워도 「뾰족한 점이 없는 작은 것」이 밤이다.
  키로 안 가른다(§2.6 「어른은 비율이 아니라 기호」).
...
이 무대가 **늘 지니는 색**: 등불 노랑(LAMP) · 눈·안개·김 흰색(FROST) · 쇠·레일 검정(IRON) · 나무·벽 타우프(종이).
계절이 더하는 것 = 봄 꽃(FROST) · 가을 잎(LAMP) — 팔레트 안에서 해결했다.
...
| 초록 | 09 nono · 신호등 초록 |
| 보라 · 자홍 · 복숭아 | 07 mio · 17 moya · 16 kota |
...
못)이 계절·시간대를 한 번 더 읽힌다 — 겨울엔 목에, 여름엔 배에 묶고, 자는 쪽엔 못에 걸린다. 악센트가
「그 권에서 변하는 것」이 되는 §2.9 의 방식(자리 변형).

---

## §3. 🔴 dodo(04)·kota(16)·g88(단권)·bruno/yuki(종이가 빛)와 갈린 자리

| 축 | `dodo-gouachemill`(04) | `kota-mineral`(16) | `g88`(단권, 검정 판) | **`bami-lamplight`(18)** |
|---|---|---|---|---|
| **종이** | 크림 = 빛 · 하늘 · 밀가루 | 진흙색, **칠한 것**, 맨 종이 0 | 검정 #16181C = 어둠 | **타우프 #75695F = 어둠. 중간톤이라 등불 크림이 가장 넓은 칠** |
| **기계** | 반복 개수 상한 | 갈기(패스 수) | 밝은 면적의 누적 | **빛이 어디까지 닿았나(1/2/3단계)** |
| **가장자리** | 붓이 못 미쳐 남은 **종이 테** | 잘린 가장자리, 결은 안에만 | 하드에지 | **빛의 가장자리 = 직선/원뿔로 자름. 종이 테 없음** |
| **선** | 250쪽에 윤곽선 0 | 인물에만 잉크 윤곽 | — | **IRON 은 윤곽이 아니라 「빛 안의 마크」. 윤곽 0** |
| **흰 것** | CHALK = 튀는 것에만 | 조개흰 = 김·수건, 순백 0 | 담색빛 | **FROST = 빛 자체(하늘·눈·안개·달·천)** |
| **부드러운 것** | 없음(하드에지만) | 김 = 가장 고운 갈기 | — | **안개·김·서린 창 = 마른 붓 스컴블 하나뿐** |
| **계절** | 자리별(안개·밤·눈은 권 지정) | 없음 | 없음 | **넷 — 「찬 칠이 어디에 얼마나」** |
| **표정** | 부리 각도 + 전신 개그 | 눈가 띠 각도 | — | **몸의 폭 + 얼굴 원반의 기울기** |
| **악센트** | 리본 파랑 | 복숭아 수건 | 놋빛 | **청록 목도리(자리 셋)** |
| **의인화** | 이족 · 날개 · 천 넷 | 이족 · 옷 · 손 | — | **이족 · 날개는 날개 · 잡는 건 발 · 한 사람에 옷 하나** |

- **bruno(05) · yuki(14) · moya(17)** 와의 관계는 한 줄이다 — 셋은 「안 칠한 종이가 빛」이고 여기는
  **「안 칠한 종이가 어둠」**이다. 같은 부호가 한 겹도 없다.
- **g88 이 가장 가깝다**(검정 판 + 빛은 오려낸 하드에지 + `boyd-flashlight`). 갈라 놓은 것 = 종이 명도(검정
  vs 중간톤 타우프) · 팔레트(놋빛·저녁주황·깃털분홍 vs LAMP/FROST 둘) · 캐릭터 축(g88 은 새끼 생쥐 단권이라
  캐릭터 축이 없다) · **등불이 아니라 태양도 같은 규칙으로 그린다**(g88 은 밤만). 단권 하나와 시리즈 하나가
  같은 참조를 딛는 것은 허용 범위이고(`_SERIES-ANCHORS` 「실명은 표에만」), 프롬프트 문면은 겹치지 않는다.
- 🔴 **기계 대조**(7낱말 단위 겹침, 모야 §3 과 같은 스크립트) — 첫 초안이 **모야 73 · 코타 17** 이었다. 뜻이
  달라도 가락이 같았다(`THE PAPER IS THE LIGHT` ↔ `THE PAPER IS THE DARK` · `HOW TO FIND A CHILD ON A
  FINISHED PAGE` · `IS THE ONE ACCENT IN TWENTY-FIVE BOOKS` · `the dial of this book is`). 골격 문장을 전부
  다시 써서 **모야 2 · 코타 0 · 나머지 0~1**(남은 1 은 전부 `CANVAS: 16:9 double-page spread. NO` 정형구).
  ⚠️ 모야가 적어 둔 그대로다 — 「뜻이 달라도 가락이 같으면 형제로 읽힌다」. 대조는 눈이 아니라 스크립트로.

**라인 충돌**(내 지시의 필수 점검) — 호리(니들펠트 입체)·전래(점눈이 = 밝은 크림 + 색연필)·세계명작
수채동화풍(투명·그라데이션·밝은 바탕)과 매체·종이·캐릭터 언어 세 축 모두 무관하다. 🔴 단 **점눈이와
「검은 점 눈 둘」이 닿는다** — 갈라 놓은 것 = 종이가 어둡고, 눈이 점이 아니라 **부리 길이만 한 원반**이며,
표정을 눈이 아니라 윤곽이 진다. `NOT` 의 「no glow or bloom around a lamp」가 **세계명작 수채와의 충돌을 막는
유일한 조항**이므로 지우지 말 것.
...
| 검사 | 어디에 | 판정 |
|---|---|---|
| **인물을 찾는다** | 앵커 `RENDERING` 끝 한 줄 | 화면에서 **빛 받은 면 안에 마크가 있는 것은 인물뿐**이다. 벤치 판·벽 띠 안에 획이 생겼으면 틀렸다 (매 쪽) |
| **빛의 가장자리를 만진다** | 관통 줄 `REACH:` | 등불 주위를 확대해 **칠과 종이의 경계가 한 줄로 떨어지는가.** 번져 있으면 글로우다 (매 쪽) |
| **안개를 뒤집는다** | `RENDERING` 스컴블 조항 | 안개 자리에서 **종이가 획 사이로 보이는가.** 매끈한 회색이면 섞은 것이다 (매 쪽) |
| **눈을 지운다** | 시트 `TEST ON THE THREE LARGE OWL OUTLINES` + 발치 둘째 줄 | 놀람·궁금·기쁨이 **폭과 기울기만으로** 갈리는가 (🔴 한 쪽으로는 실행 불가 → 시트) |
| **통째로 메운다** | 시트 발치 실루엣 줄 | 올빼미 셋이 **모자·앞치마 말고 점(귀깃)과 폭으로** 갈리는가 · 나비만 가로로 길고 · 삐삐만 바닥선에 안 닿는가 |

🔴 **왜 「모자·앞치마 말고」를 덧붙였나** — 모자·앞치마는 IRON 으로 메우면 사라진다(mio 의 「내부 마크는 정체가
될 수 없다」 §8 발견①). 귀깃은 **윤곽에 붙은 점**이라 메워도 남는다. 옷으로 가르는 시리즈가 아니라는 뜻이다.
...
| **어둠** | `no page is ever fully dark: at least one lit patch on every page` — 어두운 종이 시리즈라 이 조항이 없으면 「무서운 책」이 된다 |

🔴 **먹이 관계를 자세로 막은 이유** — 「고양이는 참새를 안 쫓는다」라고 쓰면 모델은 **안 쫓는 고양이가 참새를
보는 그림**을 그린다(눈이 곧 위협이다). 나비의 「서지 않는다 + 눈을 감는다」는 자세 규칙이라 정의상 쫓을 수
없다 — 모야가 포식자를 마감 3단계에 실은 것과 같은 방식(§7.32 ⑥), 여기서는 **자세**에 실었다.
...
|---|---|---|---|---|
| `bami-lamplight` **(수리본, 2026-09-04)** | **16,380** | 2,080 | 🔜 재측정 | 🔜 재측정 |
| (수리 전) `bami-lamplight` 기차 | 13,664 | 2,080 | **10,327** | 10,678 |
...
🔴 **2026-09-04 수리로 +2,716**(13,664 → 16,380). 늘어난 것은 문장이 아니라 **없던 자리 둘**(큰 역 실내 · 유치원
교실 = 신고 1)과 **거짓이던 관통 줄 넷의 조건절**(목도리 · 손님 짐 · 삐삐 높이 · 각도 단위)이다. 🔴 **컷 실효는
다시 세야 한다** — 늘어난 자리 조항은 컷에 동시에 안 실리므로 실효 증가분은 전문 증가분보다 훨씬 작지만, 규칙
조항(안에서 본 칸 · 기울기)과 팔레트가 같이 늘어 옛 10,327 은 이제 맞는 값이 아니다.

- **다음에 줄어들 자리** = `bami-stages.md` §1 의 자리 시트 아홉 장이 구워지면 자리 조항 열하나가 「그 시트를 보라」로 얇아진다. 창밖 일곱 값은
...
3. ✅ **`bami-stages.md`** — **자리 시트 아홉 장 완료(2026-09-04)**: DiningCar · SleepingCar · WashCorner ·
   EngineCab · Passage · Roof · Outside · Station(`Station/Big`·`Station/Crossing` 포함) · Kindergarten.
   🔴 §1-a 에 **기차 배치도**(칸 순서 · 통로 셋 · 바깥 문이 통로에만 있다는 것)를 넣었다 — 그게 없으면 스무 권이
   스무 대의 기차를 그린다. `node packages/client/scripts/check-stage-tokens.mjs bami` = **0**.
   (옛 계획) 칸 다섯 + 통로·지붕·바깥·역 = 아홉 자리의 시트와
