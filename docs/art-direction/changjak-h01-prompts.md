...
### 🔴 C5 둘째를 여는 근거 — a97 과 공정이 다르다 (§7.5 조건)

| 축 | **a97** (C5 첫째) | **h01** |
|---|---|---|
| 공정 | 같은 도장을 **화면 안에서 반복해 찍어** 저정보 필드를 만든다(스텐실·스탬프) | **손으로 그린 장식 테두리 + 그 안 불투명 평칠.** 반복되는 것은 도장이 아니라 **프레임 자체**다 |
| 반복의 단위 | 🔴 **한 화면 안**(라벤더 수천 그루) | 🔴 **쪽과 쪽 사이**(같은 테두리 열두 번) |
| 종이 | 차가운 회백 #E9E6E4 | 탁한 따뜻 버프 #C9BBA4 |
| 악센트 | 흰 거품 ↔ 노란 꽃가루가 **교대** | **순백 알갱이(매 쪽) + 창 파랑(p12 단 한 번)** |
| 밀도 | 밭이 화면 80%, 반복으로 채운다 | **테두리는 촘촘, 안은 평칠** — 위계가 바깥/안으로 갈린다 |
...
6. 🔴 **근접 3권과 나란히 놓고 본다** — ① **e01**(같은 프랑스 빵집): 하나는 테두리 있는 꽉 찬 버프 책, 하나는 테두리 없는 텅 빈 흰 책인가. 닮아 보이면 h01 의 테두리 폭을 올린다. ② **a97**(C5 첫째): 반복이 「화면 안 ↔ 쪽 사이」로 갈렸나. ③ **b09**(같은 공방 무대): 「나뭇결이 비치는 글레이즈 ↔ 종이 위 불투명 평칠」로 갈렸나.
</content>
