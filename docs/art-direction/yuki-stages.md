...
| 시트 | = 후보 | 왜 하나인가 |
|---|---|---|
| `Yard` | 마당 · 집 마당 · 마당 한복판 · 마당과 마루 밑 · 마루 앞 · 마루 끝 · 마루 · 방문 앞 · 감나무 밑 · 마당 한쪽 항아리 자리 | 마루에서 마당 흙이 보이고(12 p4) 마당에서 마루 끝선이 보인다(19 p1). **한 화면 안이다** |
...
| 쪽 | 자리 | SPOT | 획 예산 | 이어짐 |
|---|---|---|---|---|
| p1 | `BackOfHouse` | 나무 밑 | **겨울 = 바닥 안 칠함** | `Firewood` 상태 1 |
| p2~p6 | `BackOfHouse` | 나무 밑 / 스택 앞 | 겨울 | 상태 1 → 2 |
| p7~p9 | `BackOfHouse` | 스택 앞 | 겨울 | 🔴 상태 3 — **획이 줄어야 한다** |
| p10 | `Yard` | B 마루 | 겨울 = 2획 | 벽 윗줄 + 마루 끝선만 |

🔴 **p7~p9 에서 화면이 비어야 한다.** 무너진 더미를 「어질러진 것」으로 그리면 획이 늘어 이 시리즈의
겨울 규칙과 정면으로 부딪힌다.
