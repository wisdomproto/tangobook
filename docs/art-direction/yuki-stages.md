...
> art-director 산출물 (2026-08-16). 시리즈 14 `yuki-inkwash` · 25권 250쪽.
> 🔴 앵커 SSOT = `yuki-anchor.md`. 이 문서는 그 그림체로 그릴 대상을 확정한다 — 앵커를 안 고친다.
> 후보 = `docs/changjak-books/yuki/_stages.json`(자리 17 · 사물 44) · 대본 = `_scenes.json`
...
## §2. 사물 시트 — 30장 (고유 41에서 노이즈 11 제외)

| 토큰 | 사물 | 권·쪽 | 비고 |
|---|---|---|---|
| `Jars` | 항아리 넷 | 24 (8쪽) | 🔴 `Yard` 고정 부품이기도 하다 |
| `Firewood` | 장작더미 | 23 (7쪽) | 쌓인 상태 / 무너진 상태 |
| `Quilt` | 이불 | 04 (7쪽) · 18 | |
| `Broom` | 빗자루 둘 | 17 (6쪽) | 두 자루 크기가 달라야 한다 |
| `Basket` | 바구니 | 03(6) · 15(3) | 2권 공유 |
| `Bowls` | 그릇 · 밥그릇 | 14(5) · 10(3) · 07(3) | 3권 공유 |
| `Mat` | 돗자리 | 07 (5쪽) | |
| `Acorns` | 도토리 | 14 (5쪽) | |
| `Scarf` | 주홍 목도리 | 21(3) · 18 | 🔴 **유일한 색** — 캐릭터 시트 소관, 여기선 벗어 놓은 상태만 |
| … | (나머지 21장) | | |
...
| 쪽 | 자리 | SPOT | 획 예산 | 이어짐 |
|---|---|---|---|---|
| p1~p2 | `BackOfHouse` | A 나무 밑 | **겨울 = 바닥 안 칠함** | `Firewood` 상태 1 |
| p3~p4 | `BackOfHouse` | B 더미 앞 | 겨울 | 상태 1 → 2 (팔 위 넷 → 여덟) |
| p5 | `BackOfHouse` | C 눈길 | 겨울 | 두 몸 말고는 종이 그대로 |
| 🔴 p6~p8 | **`Yard`** | 문턱 앞 | 겨울 = 2획 | 🔴 상태 3 — **획이 줄어야 한다** |
