# 창작동화 1000 — G-06 앵커 + 삽화 프롬프트

> 대본 SSOT 는 `docs/changjak-books/g06.md`. 대본은 한 글자도 안 고친다.
> 배정 근거는 `changjak-assign-08b.md` §1·§2(g06)·§3.2·§3.4 — 앵커는 **e07 `changjak-tautline` 재사용**(변주).
> 🔴 실행 순서: ① 시트 3장 → ② p2(굵기 기준판) · p9(가는 획 기준판) 승인 → ③ 나머지 열한 컷
...
주제군 G · 용기·두려움 / 엔진 **관찰과 성장** / 무대 북유럽 숲, 가문비나무 한 그루의 줄기 바깥(카메라가 이 나무를 안 벗어난다) / 새끼 다람쥐 + 순록 / **13스프레드**

## G-06 §1. 앵커 배정

**권**: `제일 높은 가지` (g06 · 13쪽 · 4~6세)
**클러스터**: C2 · **슬러그**: `changjak-tautline` — 🔴 **재사용(변주)**. 공정 문단은 e07 것 그대로이고 바꾼 것은 **팔레트 hex 넷 · 관통 줄 셋 · 방향 한 줄**뿐이다. 앵커 원본(수상작 참조 그림)도 e07 것을 물려받는다.

**한 줄**: 평칠 위에 붓이 지나간 자리가 **화면당 정확히 한 군데**다. 그 굵은 한 획이 딛는 가지이고, **획이 곧 이야기다.**

**🔴 방향 한 줄 (원본 e07 과 갈린 축)**
> **획의 어느 값이 변하나 · 획이 어느 방향인가.** e07 = **가로**로 화면을 가르고 계기판이 **장력**(곧음↔구불구불) / **g06 = 세로에 가깝게 올라가고 계기판이 굵기다.** 획은 쪽마다 가늘어지기만 하고(3.0 → 0.5 머리통) 마지막에 딱 한 번 굵은 데로 돌아온다.

**🔴 형제 권과 갈린 축 (첫 렌더에서 세어진다)**

| 짝 | 갈린 축 | g06 의 값 |
|---|---|---|
| **f11** `risenpaper` (같은 배치 · 둘 다 「기다리는 동안 재기」) | 🔴 **이 책에 붓획이 몇 개인가** | f11 = 거의 0 · **g06 = 정확히 1**. 흰 것이 지지면인 쪽 ↔ 흰 것이 없는 쪽 |
| **g88** (둘 다 오르는 책) | **무엇을 세나** | g88 = 계단 칸을 **더한다**(눈금이 있다) · **g06 = 가지가 가늘어지는 것을 잰다.** 🔴 층·칸·마디를 만들지 마라 |
| **e07** `tautline` 원본 | 획의 방향과 변하는 값 | 위 방향 한 줄 |
| **e09** (같은 북유럽 숲) | 카메라가 안인가 밖인가 | e09 = 나무 **구멍 안**(사방이 막힘) · **g06 = 줄기 바깥**(사방이 트임). 한 번도 안으로 안 들어간다 |
| **a97** (라벤더를 송이마다 그려 온 전례) | 반복 단위를 그리나 | 🔴 **바늘잎은 한 잎도 안 그린다** — 가문비 초록은 평칠 한 판이다 |

### 이 권이 그림에 요구하는 것

1. 🔴 **높이를 잴 눈금이 없다.** 그래서 계기판을 **획의 굵기**로 옮겼고, 쪽마다 **다람쥐 머리통 단위**로 못박았다(`BOUGH:` 3.0 · 3.0 · 2.5 · 2.0 · 1.8 · 1.5 · 1.2 · 0.8 · 0.6 · 0.5 → p12 에 2.5).
2. 🔴 **굵기는 앞발이 닿는 네 쪽에서만 잰다**(대본 plant) — p2 못 감김 · p5 딱 맞물림 · p9 감기고 남음 · p11 몸을 못 버팀. 그 네 쪽은 **같은 거리·같은 클로즈업**으로 굽는다.
3. 🔴 **순록이 거리 측정기다.** 프레임에서 한 번도 사라지지 않고 쪽마다 작아진다 — 값을 스프레드 높이 대비로 적었다(`DEER:` 0.35 → 0.03).
4. 🔴 **높이가 무서움으로 새면 안 된다**(대본 note) — 오를수록 **밝아진다**. 초록 판이 줄고 흰 하늘이 는다. 어둡게·차갑게·아득하게 만들지 마라.
5. 🔴 **아래를 보는 것이 착지다.** p1~p11 은 고개도 앞발도 **한 번도 아래를 안 향한다**(`DOWN: no`). p12 에서 같은 앞발이 그대로 아래를 가리킨다.
6. 🔴 **제일 높은 가지는 끝까지 프레임 밖 위에 있다.** p13 에서도 안 보인다 — 화면 위 가장자리에 **그 가는 가지 끝만** 걸린다.
7. 🔴 **글자·숫자 없음.** 열세 컷 전부에 반복해서 적었다.

### 대본 SCENE 처방표

| 대본 문구 | 컷에서 옮기는 법 |
|---|---|
| p2 「뒤로 잎 그늘이 부드럽게 풀려 있고」 | 흐림 없음 — **뒤에 마감을 안 준다**(초록 평칠 한 판, 안에 0개) |
| p3·p9 「흐리게 걸린다 / 흐리게 겹친다」 | 겹친 가지는 **평칠 한 색면**, 획이 아니다(획은 화면당 1) |
| p13 「밑동은 부드럽게 풀린다」 | 밑동에 **마감을 안 준다**(색면 그대로, 가장자리는 하드 엣지) |
| p7 「흩날리는 잎만 흐릿하고」 | 날리는 잎 = **평칠 조각 최대 9개**, 모션 블러 0 |

### 밀도 배급

**`FINISHED THINGS PER PAGE = 2` · `DENSITY RATION = p1 · p13`.** 사건이 「획 하나가 가늘어지는 것」이라 열한 쪽은 조용해야 하고, 기준 셋(옹이·둥지·그루터기)을 **세는 두 쪽에서만** 마감을 5개까지 푼다.

### 🔴 라인 충돌 확인

| 대상 | 겹치나 | 왜 |
|---|---|---|
| 호리 **니들펠트** | ✕ | 평칠 포스터 판 · 윤곽 0 · 털 획 0 (펠트는 섬유 가장자리가 정체다). NOT 에 `no fur`·`no fibre`·`no stitching` |
| 전래동화 **점눈이** | ✕ | 점눈 2 는 같으나 **선이 0**(점눈이는 실선 입·낙서 선이 정체) · 빨강 1점 규칙 미사용 |
...
2. **p2·p5·p9 「껍질의 세로결 · 마른 송진 방울 · 얇고 매끈한 껍질」** — 결을 그리면 화면에 획이 수천 개가 된다. 🔴 껍질은 **평칠**이고, 세로결은 **획의 마른 가장자리**(붓이 마른 자리)로만 읽힌다.
3. **p9 「검은 실루엣에 가깝게」** — 검정을 새로 들이지 않는다. 흰 하늘 위의 `#6B4530` 획이 이미 이 책에서 가장 어두운 것이다.
...
Style: picture book for ages 4-6. One old spruce in a northern forest, seen from
outside the trunk, one clear late-summer morning. Flat opaque poster-plates with
NO outlines, and laid on top of them exactly ONE loaded brush stroke per spread.
That stroke is the branch itself - an object, never a contour.

RENDERING (finish hierarchy): every shape is one flat colour, hard-edged, 0
shading, 0 gradient, 0 texture, 0 outline. FOLIAGE = ONE unbroken green plane: 0
needles, 0 needle clusters, 0 twigs drawn - the crown is a colour, not a pattern.
Sky = one flat pale plane, 0 clouds, 0 birds. Bark = flat colour, 0 grooves, 0
scales; the trunk carries at most 3 flat cut shapes (knot, nest, stump) and
nothing else. BRUSH STROKE COUNT PER SPREAD = 1: one dark-brown bristle-loaded
pass, edges frayed where the brush ran dry; the branch reads as swellings and
...
PALETTE: spruce green #3E5244 (one plane, ~60% of the frame at p1 and ~5% at
p10) / bark brown #6B4530 (the single stroke and the trunk plate - the darkest
thing on every page) / pale sky #DCE3DE (the sky, and also the lichen ground, the
dry stump, the nest and the reindeer's coat - 🔴 the reindeer is the same value
as the sky, which is why it stops reading as it shrinks) / squirrel rust #D8A23A.
🔴 Rust appears ONLY on the squirrel's body and on ONE pine cone (p6 and p13, at
most 1 cone per page). 0 other colours mixed in. No black, no blue.

CHARACTER DESIGN LANGUAGE: 🔴 BOTH ANIMALS ARE PLAIN FOUR-LEGGED ANIMALS - 0
clothing, 0 props, 0 fingers, natural posture. The squirrel rises onto its hind
legs only when a forepaw is doing something. Eyes = 2 flat dots, 0 whites, 0
lashes. The reindeer is one flat pale shape with antlers as a flat cut
silhouette; its posture never changes except nose down (p6) and head up (p12,
p13). Nobody speaks with hands - there are none.

CANVAS: 16:9 double-page spread, horizontal. NO letters, numbers, signs or
lettering of any kind anywhere.

NOT (rendering only): no outline around any shape; no airbrush, gradients, gloss
or 3D shading; no drawn needles, bark grain, fur or fibre; no second brush
stroke.
```

### 🔴 이 앵커의 관통 줄 셋 (매 컷 반복 확인)

**A — `BOUGH:`** 이 쪽의 유일한 붓획 = 딛는 가지. 굵기(다람쥐 **머리통** 단위) · 방향 · 그 위에 무엇이 걸치나.

| p1 | p2 | p3 | p4 | p5 | p6 | p7 | p8 | p9 | p10 | p11 | p12 | p13 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 3.0 줄기(수직) | 🔴 3.0 (두 앞발 못 감김) | 3.0 줄기 | 2.5 | 🔴 2.0 (앞발 끝 딱 맞물림) | 1.8 | 1.5 (기움) | 1.2 | 🔴 0.8 (한 앞발에 남게 감김) | 0.6 | 🔴 0.5 → 활처럼 휨 | 2.5 (굵은 데로 돌아옴) | 3.0 (위에서 봄) |

🔴 **획은 늘 세로에 가깝다** — 화면 세로축에서 최대 30° 안. 유일한 예외가 p7(바람에 사선)과 p11(활처럼 휜 곡선)이다.
🔴 **제일 높은 가지는 열세 쪽 내내 프레임 밖 위**다. 가리킨 방향 끝에는 늘 **또 한 겹**이 걸리고, 그 위는 잘린다.

**B — `DEER:`** 순록이 화면 어디에 있고 몇 할인가(스프레드 높이 대비 어깨높이). 🔴 **한 쪽도 빠지지 않는다.**

| p1 | p2 | p3 | p4 | p5 | p6 | p7 | p8 | p9 | p10 | p11 | p12 | p13 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 🔴 0.35 (같은 바닥·같은 깊이) | 프레임 밖(대본) | 0.12 등·뿔만 | 밖 | 밖 | 0.08 코와 뿔 | 밖 | 밖 | 🔴 0.02 갈색 점 | 밖 | 밖 | 0.05 올려다봄 | 🔴 0.03 솔방울만 |

⚠️ 대본이 「순록은 프레임 밖(아래)」이라 못박은 쪽(p2·p4·p5·p7·p8·p10·p11)은 **넣지 않는다** — 넣으면 클로즈업이 무너진다.

**C — `DOWN:`** 다람쥐의 고개·앞발이 아래를 향하나.

| p1~p11 | p12 | p13 |
|---|---|---|
| 🔴 **no** — 늘 위 아니면 앞. 눈·코·앞발 어느 것도 아래를 안 본다 | 🔴 **yes** — 같은 앞발이 각도만 뒤집혀 아래를 가리킨다 | yes — 턱을 얹고 내려다본다. 🔴 화면 어디에도 위를 보는 것이 없다 |

---

## G-06 §3. 캐릭터 시트 (🔴 이것부터 굽는다 · 장면 금지)

### 시트 1 — SquirrelKit

```
CHARACTER SHEET - SquirrelKit   (bake this FIRST)
Flat poster-plate style, 0 outlines, 0 shading - see anchor changjak-tautline.

A young red squirrel. Body = ONE flat rust shape #D8A23A, small and round-
backed. Tail = one flat shape as long as the body, 0 hair strokes, 0 fringe -
its edge is a clean cut curve. Ear tufts = 2 short flat spikes. Muzzle = one
wedge, nose = 1 dark dot. Eyes = 2 dark dots, 0 whites, 0 lashes, 0 catchlights.
0 collar, 0 clothing, 0 props, 🔴 0 fingers - the forepaw is one flat mitt shape
that can wrap, grip and point.
🔴 THE HEAD IS THE RULER OF THIS BOOK - bake it clean from the side so its width
can be laid against a branch. Silhouette test: a rust comma with a tail as big
as itself.
REFERENCE SHEET, each drawn big: (a) standing on the hind legs, one forepaw
stretched straight overhead pointing, head thrown back, tail up (b) belly on a
branch, BOTH forepaws stretched round it and NOT meeting - the gap one head
wide (c) belly on a branch, both forepaws meeting exactly, claw tips touching
(d) ONE forepaw wrapped right round a thin branch with the toes overlapping (e)
flat on a branch, all four sets of claws dug in, legs splayed wide, fur pressed
one way (f) sitting on a thick branch, hind legs folded, head bent DOWN, one
forepaw pointing DOWN (g) belly on a thick branch, chin laid on it, both
forepaws hanging over the front, tail curled over the back.
Flat pale #DCE3DE background. No text.
```

### 시트 2 — Reindeer

```
CHARACTER SHEET - Reindeer   (bake SECOND)
Flat poster-plate style, 0 outlines, 0 shading - see anchor changjak-tautline.

An adult reindeer. Body = ONE flat pale shape #DCE3DE, the same value as the
sky, with a slightly darker flat #9AA79C saddle over the back so it still reads
against the green plane. Throat mane = one flat pale shape hanging under the
neck. Antlers = ONE flat cut silhouette, at most 6 tines in total, 0 texture.
Eyes = 2 dark dots. Muzzle = one wedge. 0 fur strokes, 0 clothing, 0 harness,
0 bell, natural four-legged posture.
🔴 IT IS THE DISTANCE METER OF THE BOOK, so bake it at four sizes, side by side
on one line, all in the SAME posture, shoulder height marked: 0.35 / 0.12 / 0.08
/ 0.02 of a page height. At 0.02 it is a single rust-free brown-grey blob with
0 legs readable; at 0.03 it is exactly the size of a pine cone - 🔴 bake ONE
PINE CONE #D8A23A beside it at that size for comparison.
THREE POSTURES: (a) standing four-square, head half raised, chewing (b) nose
lowered to the ground, neck long, antlers tipped forward (c) neck thrown back,
head up, mouth open, pale throat mane showing bright.
Flat green #3E5244 background. No text.
```

### 시트 3 — TreeKit 🔴 **이 시트가 책을 지배한다**

```
CHARACTER SHEET - TreeKit   (bake THIRD - not a scene)
...
strokes: a pale #DCE3DE KNOT shaped like an eye, ringed by a flat darker collar ·
an EMPTY NEST of flat #DCE3DE lodged in a fork, with one flat feather and 0 eggs,
0 birds · a BROKEN BRANCH STUMP, its dry end a pale #DCE3DE disc broad enough for
...
🔴 NO TREE HOLLOW, NO ENTRANCE, NO INTERIOR anywhere on this sheet.
No text.
```
...
각 컷 = `STYLE ANCHOR + @image1(SquirrelKit) + @image2(Reindeer) + @image3(TreeKit) + 아래 블록`.
🔴 **굽는 순서 = p2(굵기 기준판) → p9(가는 획 기준판) → p1(기준 셋 확정) → 나머지 열.** p5·p11 은 p2 승인본을 함께 붙인다 — 네 쪽이 같은 자세, 같은 거리다.

### p1 — "제일 높은 가지, 저기야!" 🔴 기준 셋 심음 · 밀도 배급 쪽
...
SUBJECT: LOWER CENTRE, small - SquirrelKit on its hind legs, one forepaw
stretched straight up pointing off the top of the frame, tail up (sheet posture
a).
...
this distance - p5, p9 and p11 repeat it exactly.
...
SUBJECT: CENTRE - SquirrelKit clinging belly-flat to the trunk, all four sets of
claws in, halted one hand above the knot, mouth open, tail hanging past the
knot's rim.
...
### p4 — "제일 높은 가지, 저기야!" (두 번째)

```
BOUGH: one #6B4530 pass 2.5 heads wide, near-vertical-to-diagonal, crossing the
lower third of the frame - the branch he now stands on. 🔴 ABOVE IT, ONE MORE
BRANCH crosses the frame as a FLAT colour shape (not a stroke), and above that a
third, and the top is cut off. The layers say the tree keeps going.
DEER: out of frame (script).
DOWN: no. Head thrown back, forepaw straight overhead - 🔴 the same posture as
p1, only the place has moved up.
CAMERA: medium wide, low angle from just below his feet.
SUBJECT: LOWER LEFT - SquirrelKit up on its hind legs on the branch, one forepaw
...
side.
DEER: out of frame (script).
DOWN: no. Head turned sideways to the nest, never down.
CAMERA: medium close-up, eye level, exactly p2's distance.
SUBJECT: LEFT - SquirrelKit belly on the branch, both forepaws meeting round it,
claw tips touching, hind claws hooked under, head turned right (sheet posture c).
...
head stays up; ears turn back, eyes stay on the branch above.
CAMERA: wide, slightly above eye level, tall composition split top and bottom.
SUBJECT: UPPER - SquirrelKit on all fours on the branch, head up, one forepaw
...
```
BOUGH: one #6B4530 pass 1.5 heads wide, 🔴 THE ONE PAGE WHERE THE STROKE LEANS -
a diagonal across the frame, the camera tipped with it. Still one pass, still
dry-frayed, 0 second stroke.
DEER: out of frame (script).
DOWN: no. Eyes half shut, looking along the branch ahead and up.
CAMERA: medium, tilted eye level, the whole frame leaning one way.
...
TONE: the crown opens as it leans, so the page gets BRIGHTER, not darker - more
pale plane than p6. GRADE: four-legged, 0 clothing, 0 fingers.

```

### p8 — "제일 높은 가지, 저기야!" (세 번째)

```
BOUGH: one #6B4530 pass 1.2 heads wide, near-vertical, and above it TWO more
branch shapes in flat colour, thinner than any below. 🔴 The pointed-at branch is
cut off by the top of the frame.
DEER: out of frame (script).
DOWN: no. Head thrown back, forepaw overhead - 🔴 the same posture a third time,
the place moved up again.
CAMERA: medium wide, low angle.
SUBJECT: LOWER CENTRE - SquirrelKit standing with both hind feet on the dry end
...
```
BOUGH: one #6B4530 pass 0.6 heads wide under his feet, and 🔴 ONE MORE PASS-
WIDTH SHAPE ABOVE IT in flat colour, the last thing a body could reach. Above
that, the trunk tip tapers off and 🔴 the highest branch is still outside the
top of the frame.
DEER: out of frame (script).
...
DEER: out of frame (script).
DOWN: no - 🔴 his body hangs low but his eyes are still on the paws above him.
Head has not turned down yet.
CAMERA: medium, eye level, exactly p2's distance.
SUBJECT: CENTRE - SquirrelKit with both forepaws laid on the bending branch and
...
```
BOUGH: one #6B4530 pass 2.5 heads wide, near-horizontal, thick and steady - 🔴
...
angle is turned over. Ears forward.
CAMERA: medium, eye level.
SUBJECT: LEFT - SquirrelKit sitting on the thick branch, hind legs folded, head
...
diagonal. GRADE: four-legged, 0 clothing, 0 fingers.

```

### p13 — "조금만 더 보고 갈게!" 🔴 밀도 배급 쪽 · p1 의 정반대
...
CAMERA: wide, high angle over the squirrel's shoulder, looking down the trunk.
SUBJECT: NEAR LOWER EDGE - SquirrelKit belly on the thick branch, chin on it,
