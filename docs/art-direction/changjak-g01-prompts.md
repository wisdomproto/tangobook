...
**한 줄**: 🔴 **어둠은 안 칠한 자리가 아니라 겹쳐 붙인 자리다.** 같은 회청 질감판을 한 겹 더 얹을 때마다 값이 한 단 내려가고 아래가 덜 보인다. 침대 밑은 이 책에서 겹이 가장 많은 곳이라 **끝까지 판독되지 않고**, 빛은 판이 비켜 간 **0겹**이다. 앵커 슬러그 `changjak-g01` — **신규 민팅** (🔴 **C9 둘째**, f05 와 다른 계열 = 질감판 겹치기 ②).
...
### 🔴 C9 둘째를 여는 근거 — f05 와 계열이 다르다 (§7.5 조건: 「②질감판 겹치기 계열로」)

| 축 | **f05** (C9 첫째) | **g01** |
|---|---|---|
| 종이 | 손 인쇄 **무늬** 종이 여러 장(타일·격자·나무결) | 🔴 **무늬가 없는 단색 질감판** — 스폰지·롤러 그레인만. 무늬가 한 점도 없다 |
| 공정 | **오려 붙인다**(가위 엣지가 정체) | **겹쳐 붙인다**(겹의 개수가 정체) |
| 세는 것 | 같은 종이에서 오린 **같은 도형의 개수** | 🔴 **겹의 수** = 어둠의 깊이 |
| 온도·채도 | 따뜻한 부엌, 유일 채도색 = 치즈 노랑 | 차가운 밤, 유일 채도색 = 호박 |
| 선 | 크레용 굵은 윤곽 | 🔴 **가는 마른 연필 윤곽**, 가시만 짧은 획 다발 |

🔴 §2.11·§2.14 대로 **오리기가 개수를 강제한다**는 원리는 같지만, **세는 대상이 다르다**(도형 ↔ 겹). 그래서 두 권은 같은 클러스터에서 각자 앵커가 된다(전례 = C3 셋이 「새기나/찍나」로, C6 넷이 「밝은 종이 가법 / 검정 판 가법」으로 갈렸다 — **클러스터 라벨보다 공정이 정체를 정한다**, §2.13).
...
## G-01 §3. 시트 4장 (🔴 LayerKit 을 가장 먼저 굽는다 — §2.14)
...
6. 🔴 **근접 3권과 나란히 놓고 본다** — ① **g88**(같은 G 군 어둠 책): 하나는 **검정 책**, 하나는 **흰 책**인가. 어둠이 「비어 있음 ↔ 쌓여 있음」으로 갈렸나. 닮아 보이면 g01 의 낮 쪽(p2) 비중과 겹 엣지 가시성을 더 올린다. ② **f05**(C9 첫째): 무늬가 있나 없나 — g01 에 무늬 종이가 한 장이라도 들어오면 두 권이 같은 재료가 된다. ③ **g10**(같은 G 군): 「딥펜 떨림 선의 아침 ↔ 선이 거의 없는 밤」으로 갈렸나.
</content>
