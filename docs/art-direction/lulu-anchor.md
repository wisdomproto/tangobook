...
CHARACTER DESIGN LANGUAGE: the donkeys are built from the same broad fields as the world - two or
  ...
  never five. Backs and heads are PLATE MIX, chests and bellies are DRY EARTH, so every donkey is
  built the same way. Eyes are two solid dark dots set wide apart; a soft dark muzzle; the mouth is
  ONE curve; above each eye ONE short eyebrow stroke. 🔴 FEELING IS CARRIED BY THE MOUTH CURVE, THE
  TWO EYEBROWS AND THE TWO LONG EARS - the eyes only ever open or close and their size NEVER changes.
  ...
  the world · MAMA a donkey, a full head over Lulu, an apron, no blue · NINO a donkey, tall and
  narrow, chest thrown out, a STRAW HAT that is his silhouette (volume 01 leaves it up a tree and it
  stays there to the last page), no blue · ROSA a donkey, the roundest adult, a SKIRT and an APRON
  WITH A DEEP POCKET, always in motion with the skirt swinging behind her, no blue · BEPPO a donkey,
  the biggest and heaviest, a WAISTCOAT and a CROOKED STICK always in one hand, no blue · THE THREE
...
NOT: no airbrush, gradient, glow or 3D render / no white ink or white paint - white is wiped / no
  soft edge anywhere except the plate mark / no drawn outline / no purple on anything, including the
  grape juice / no blue on any character but Lulu / no shoes on anybody / no fourth walking cousin /
  no reflection or mirrored image in water / no second printing pass / no lettering.
```
...
SHEET LAYOUT: one warm white sheet, the family standing in a row on a single ground line at their true
  relative heights - Beppo the biggest and heaviest, Nino tall and narrow, Rosa the roundest adult,
  Mama a full head over Lulu, Lulu the smallest standing figure. Each of these five is drawn THREE
