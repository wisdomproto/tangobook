...
CHARACTER DESIGN LANGUAGE: ducks are built from the same flat sweeps as the world - two or three
  ...
  OVERLAP; chests and bellies are WHEAT; bills and webbed feet are WHEAT. Eyes are two solid dark
  dots set wide apart; the mouth is the line where the bill closes; above each eye ONE short eyebrow
  stroke. 🔴 FEELING IS CARRIED BY THE BILL ANGLE AND THE TWO EYEBROW STROKES - the eyes only ever
...
NOT: no airbrush, gradient, glow or 3D render / no watercolour wash, bleed or transparency / no soft
  or feathered edge / no outline of any kind / no ripple, glint or sparkle on water / no reflection
  or mirrored image in water / no brush stroke that reaches every outline / no white used as a
  highlight on a form / no hands, fingers or sleeves on a duck / no lettering.
```
