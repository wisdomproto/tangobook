# 창작동화 1000 — F-08 앵커 + 삽화 프롬프트

> art-director 산출물 (2026-07-31). 배정 = `changjak-assign-31.md` §1 · §3.3 · 규격 = `verified-references.md` §2.16 · §2.16.1 · §2.16.2 · §2.16.3.
> 🔴 **대본 SSOT 는 `docs/changjak-books/f08.md`.** 아래 12컷은 그 SCENE 을 그림 지시로 옮긴 번역본이다. 대본은 한 글자도 안 고쳤다.
> 🔴 **이미지 생성은 여기서 하지 않는다.** 🔴 **작가 실명 0.**
> 🔴 **실행 순서**: ① **TubWater**(투명 ↔ 불투명 · 네 색이 안 섞인 채 나란히) → ② CabinRoom → ③ BearCub · GranBear → ④ **p1**(나뭇결이 다 보이는 쪽 = 이 책의 눈금) → ⑤ **p8**(넷이 아직 따로 있는 쪽 = 최대치) → ⑥ **p12**(나뭇결이 하나도 안 보이는 쪽) → ⑦ 그 셋을 ref 로 나머지. 🔴 **p1 이 확정되기 전에 다른 컷을 굽지 마라** — 잃을 것이 없으면 착지가 성립하지 않는다.

---

## F-08 §1. 앵커 배정

**권**: `f08` 「목욕은 왜 해야 해」 (12쪽 · 4~6세 · 주제군 **F 집·가족의 작은 사건** · 엔진 **누적·반복** · 무대 슬로베니아 숲집 방 하나 · 주인공 아기 곰 + 할머니 곰)

**한 줄**: **한 통의 물에 색을 하나씩 푼다.** 넷이 각자 가장자리를 지킨 채 나란히 있다가, 저으면 섞여 **불투명**해지고 통 바닥 나뭇결이 안 보인다. 앵커 슬러그 `changjak-tubwater` — **신규 민팅**(배정표 지정, 변경 없음).

**이 권이 그림에 요구하는 것** 셋.

1. 🔴 **넷이 서로 구별돼야 한다 — 그런데 배경은 좁아야 한다.** 대본 note 가 「이 권은 2~4색 제한 팔레트를 못 쓴다」고 명시한 유일한 권이다. 해법은 팔레트를 넓히는 것이 아니라 **색이 있을 수 있는 자리를 하나로 줄이는 것**이다 — 🔴 **채도색은 통 안에만 존재한다.** 방·난로·마루·옷이 전부 옅은 나무빛 워시 한 색이라, 넷이 다 들어와도 화면에 색을 가진 것은 여전히 **물 하나**다.
2. 🔴 **착지가 「투명 ↔ 불투명」이라 매체가 그것을 실제로 할 수 있어야 한다.** 수채는 겹치면 밝아지지 않고 **탁해진다** — 네 안료가 실제로 교차하면 침전이 생겨 밑이 안 비친다. 그래서 「통 바닥 나뭇결이 안 보인다」가 밝기 조절이 아니라 **안료의 사실**이 되고, 흰 물감을 팔레트에서 없애면 그것 말고 불투명해질 방법이 없다(§2.16.3 의 판정 질문 — 「절대 하면 안 되는 일(흰색으로 덮기)을 **할 수 없게** 만드는 매체가 있나」).
3. 🔴 **넷이 「아직 안 섞인 것」을 열한 쪽 동안 보여야 한다.** 젖은 물감에 다음 색을 떨어뜨리면 서로 **밀면서 제 가장자리를 만든다**(wet-in-wet 의 실제 성질). 이 성질이 없으면 p8(넷이 따로 있는 쪽)이 안 그려지고, p10 의 섞임도 값이 없다.

**후보 3.**

① **C8 · 한 그릇 안에 색을 순차 투입(신규 앵커)** — 요구 1~3 을 매체 하나가 동시에 푼다. 배정표가 준 공정 그대로다.
② **C4 평면 색면**(누적 엔진 1순위, §2.11) — **탈락**: 넷을 세는 데는 최강이지만 🔴 **평칠에는 「섞임」이 없다.** 하드에지 색면 넷을 겹치면 다섯째 색면이 될 뿐, 「가장자리가 휘어져 서로 끌려 들어간다」(p10)와 「밑이 안 비친다」(p12)가 원천적으로 안 나온다. 이 권의 누적은 개수가 아니라 **농도**다.
③ **C9 오려 붙이기** — **탈락**: 같은 이유에 더해 F 군 1순위이지만 물이 소재라 종이가 젖은 것을 표현할 수 없고, F 군에는 이미 콜라주가 배정돼 있다.

**🔴 C8 이웃 셋과의 분리 — 첫 렌더에서 즉시 판정되는 축은 「색이 화면 어디에 있나」다.**

| | 공정 | 색이 있는 자리 | 이 권과 갈리는 자리 |
|---|---|---|---|
| c02 | 번지게 → 마른 뒤 선을 얹는다 | 화면 전체(무채 회록) | **선이 있다** ↔ 여기는 선이 워시 가장자리뿐 |
| c10 | 기울여 흘린다, 중력 한 방향 | 화면 전체(흙탕 갈색) | 물감이 **종이 전체를 흐른다** ↔ 여기는 **통 안에 갇혀 있다** |
| f06 | 옅은 광역 워시, 어두운 데가 0 | 화면 전체(백야) | 값이 안 내려간다 ↔ 여기는 **탁해져 값이 내려간다** |
| **f08** | **한 그릇에 하나씩 푼다** | 🔴 **통 안에만** | — |

🔴 **h08(유리의 온도)과도 이미 갈라 뒀다**(배정표 §3.3) — h08 은 색이 **사라지며 투명해지고**(빼기), f08 은 색이 **쌓여 불투명해진다**(더하기).

**🔴 라인 충돌**: 호리 니들펠트 ✕(종이·물감뿐) · 점눈이 ✕(① 종이색이 크림에 가까우나 **그것이 목욕통의 맨 나무**이고 그림의 지지면 역할이 다르다 ② 얼굴 = 그린 눈 + **별개 눈썹** ③ 매체 = 색연필 낙서 ↔ **투명 워시, 선이 거의 없다** ④ 🔴 **빨강 1점 규칙을 안 쓴다** — 이 권의 붉음은 물 안에서만 존재하고 **인물·소품에 빨강이 한 점도 없다**).

**🔴 대본 SCENE 처방 3건** — 대본은 고치지 않고 컷에서 분기한다(§2.16.4 표).

| # | 대본 | 문제 | 그림 처방 |
|---|---|---|---|
| 1 | p2 「나머지는 **부드럽게 풀리도록**」 · p11 「배경은 **깊게 풀린다**」 | 워시에 초점이 없다 | 🔴 뒤로 미는 것은 **코트 수를 1로 줄이고 선을 0 으로** 하는 것뿐. 「부드럽게 풀린 배경」 = **한 겹만 칠하고 아무 선도 안 긋는 배경** |
...
**🔴 컷 관통 줄 3개** — 컷마다 `WATER:` `GRAIN:` `STAINS:` 를 먼저 읽는다.
...
```
STYLE ANCHOR - changjak-tubwater   (a bear cub and its grandmother / a Slovenian forest cabin)
...
## F-08 §3. 캐릭터 시트 (🔴 TubWater 를 가장 먼저)

```
MATERIAL SHEET - TubWater   (bake FIRST - this book is this one test board)

A test board, no characters. Watercolour on cold-pressed cream paper #F0E7D8.
...
C · 🔴 STIRRED: the same pool after a stir - the four crossed into ONE muddy #6B5238, OPAQUE, and
  🔴 THE GRAIN LINES UNDERNEATH HAVE GONE. Put B and C at the same size side by side. 🔴 The
  difference must read as SEDIMENT, not as darkness - do not simply make C darker.
D · WHAT FLOATS: on the surface of C, at most 9 small marks - grit specks, torn leaf scraps, tiny
  seeds, round yellow beads. 🔴 The colours mixed; the bits did not.
E · DRY AND WET: a patch of brown fur with dried mud on it - dull, flat, no shine - beside the same
  ochre loose in water, bright and spreading. One pigment, two states.
No characters, no room, no lettering or numerals. SCENE token: TubWater.
```

```
SET SHEET - CabinRoom   (bake SECOND - the room, no animals in it)

Same make as the book. Do not render it richly just because nobody is in it: 🔴 THE WHOLE ROOM IS
...
🔴 THE TUB, THREE STATES, drawn large: (i) empty and dry - the inner wall and floor boards are BARE
  ...
  the water stood.
🔴 A TRAIL OF DRY OCHRE FOOTPRINTS crosses the floor from the doorway to the tub.
0 lettering, 0 numerals, no animals. SCENE token: CabinRoom.
```

```
CHARACTER SHEET - BearCub   (bake THIRD - the stain ladder is the book)

Same make: watercolour on cream paper, no drawn line where a wash edge will do. Plain paper behind.

BODY: a young bear cub, warm brown #7C5B3E in 2-3 washes, 0 hairs drawn. Round ears, short muzzle,
  small forepaws used as hands. 🔴 IT WEARS NOTHING ON ANY PAGE. It is a third of the
  grandmother's height.
FACE: a small drawn eye with a SEPARATE short brow stroke above each. No catchlight, no dot-eyes.
🔴 THE STAIN LADDER - the same standing figure FIVE times in a row, same size, same pose:
  (4) dried MUD OCHRE on both feet and between the toes; GRASS GREEN smeared on flank and belly
  with two real grass blades stuck in it; BERRY RED inside both forepaws and round the mouth;
  sticky HONEY YELLOW on the nose tip and one cheek. (3) feet clean. (2) flank clean too. (1) only
  the nose tip left. (0) 🔴 not one mark anywhere, the coat one even brown.
  🔴 ALL FOUR STAINS ARE DRY, DULL AND FLAT HERE - the bright versions of these colours exist only
  in the water. No wet shine on the coat at any step.
SHEET: both forepaws hooked over a tub rim on tiptoe, one hind leg lifted with the toes just
  entering water / belly on the rim, sliding forward, one hind foot off the floor / sitting in the
  tub rubbing its own flank with one forepaw / neck stretched out, dipping only the tip of its nose,
  both forepaws braced on the rim / sunk to the shoulders, both forepaws circling the water / both
  forepaws cupped and lifted to its own eyes with water running out between them.
Plain paper, no room. SCENE token: BearCub.
```

```
CHARACTER SHEET - GranBear   (bake FOURTH)

Same make - attach the approved BearCub sheet so the two read as one hand.

BODY: an old bear, three times the cub's height, heavy and slow. A fleece waistcoat and an apron,
  both in the PALE WOOD WASH and 🔴 carrying none of the four colours. Broad muzzle, a heavy brow
  ridge that does the acting, two small dark eyes.
🔴 FIXED GRADE, twelve pages: she heats the water, pours it, snatches her feet back when it spills
  and holds a towel open at the end. SHE NEVER TELLS THE CUB TO WASH ANYTHING, never points at a
  dirty place, never leans in to scrub, never says be careful. She speaks once, and it is not an
  instruction. Draw her doing nothing much and looking pleased about it.
SHEET: bent over, tipping a big iron kettle with both forepaws, eyes only on the spout / sitting on
  a low stool with knitting on her knees, glancing sideways, eye corners creased / on the stool with
  both hind feet snatched up high and the knitting held clear, eyes round / standing, holding a big
  towel open with both forepaws, head tilted toward the tub.
Plain paper, no room. SCENE token: GranBear.
```
...
각 컷 = `STYLE ANCHOR + @image1(TubWater) + @image2(CabinRoom) + @image3(BearCub) + 아래 블록`. 할머니가 나오는 쪽(p1·p2·p4·p9·p12)은 `@image4(GranBear)` 를 더 붙인다.
🔴 **p1 → p8 → p12 순으로 먼저** 굽는다(나뭇결의 최대치 · 넷이 따로 있는 최대치 · 나뭇결 0).

### p1 — 통 속 물이 아주 맑아요 🔴 눈금을 심는 쪽 · 가장 먼저

```
BAKE THIS PAGE FIRST. It sets what the last page loses.
CAMERA: wide, child's eye level, the whole room in one frame - LEFT the doorway and the trail of
  prints, CENTRE the tub with the cub beside it, RIGHT the stove. Evening, the sun just gone.
SUBJECT: centre, BearCub standing on two legs beside the tub, both forepaws clasped in front of its
  belly, shoulders up, lower lip pushed out, one hind foot tapping the boards, one round ear folded
  back, eyes down into the water. 🔴 ALL FOUR STAINS ARE ON IT AND ALL FOUR ARE VISIBLE IN THIS ONE
  FRAME - ladder step (4). Right, GranBear bent over, carrying the iron kettle toward the tub with
...
FINISH: 2 (the cub + the water in the tub). 🔴 The water is the brightest and emptiest thing in the
  ...
  from the doorway to its feet. Room = one pale wood wash, stove mouth thumbnail-sized. 0 lettering.
```

### p2 — 발끝을 쑥 넣었어요

```
CAMERA: medium close-up, LOW angle from floor height looking up past the tub rim. RIGHT, the cub's
  upper body hooked over the rim; LOWER LEFT, the foot going in.
SUBJECT: right, BearCub with both forepaws hooked over the rim, one hind leg lifted high and bent,
  🔴 THE TOES OF THAT FOOT JUST BREAKING THE SURFACE - the other foot still flat on the floor,
  knee folded up. Eyes half shut, muzzle wrinkled and the mouth corners already going up because it
...
FINISH: 2 (the entering foot + the water). 🔴 The subject is the point where the toes touch. Say so
  by giving the rest of the water ONE coat and no line at all - do not blur or soften anything.
  Steam off the kettle spout. The room behind = fewer strokes, same brightness. 0 lettering.
```

### p3 — 발 둘레가 누레져요 🔴 첫 색

```
CAMERA: close-up, HIGH angle looking straight down at the surface. CENTRE, two feet under water
  with a cloud spreading round them; TOP EDGE, the cub's face looking in.
SUBJECT: top edge, BearCub bent right over the rim with both forepaws gripping it, nose almost
...
```
CAMERA: medium wide, child's eye level, from the side. RIGHT, the cub sliding into the tub; LEFT,
  GranBear on the low stool.
SUBJECT: right, BearCub with both forepaws braced on the rim and 🔴 ITS BELLY LAID ON THE RIM,
...
FINISH: 2 (the sliding body + the water it is entering). 🔴 The right half of the frame is busy and
  ...
  0 firelight on the floor. 0 lettering, 0 numerals.
```

### p5 — 물이 초록빛으로 변해요 🔴 두 색이 나란히

```
ATTACH THE APPROVED PAGE 3 RENDER - the two clouds must be comparable.
CAMERA: medium close-up, over the cub's own shoulder, looking down at its belly and the surface.
  FRONT UPPER, its shoulder and chin; CENTRE, the submerged belly and the rubbing forepaw.
SUBJECT: centre, BearCub sitting in the tub, upper body folded forward, 🔴 ONE FOREPAW RUBBING ITS
...
### p6 — 붉은 것이 실처럼 풀려 나와요 🔴 세 색

```
CAMERA: close-up, at surface level from the side, so half the frame is under water and half above.
  CENTRE, both forepaws swinging under the surface; TOP, the lower half of the cub's face.
SUBJECT: centre, BearCub sitting in the tub with 🔴 BOTH FOREPAWS IN THE WATER SWINGING HARD SIDE
...
### p7 — 코끝만 콕 담갔어요 🔴 네 색이 다 들어온다

```
CAMERA: extreme close-up, child's eye level. CENTRE, the tip of the nose touching the water; ABOVE,
  the bridge of the muzzle and one eye.
SUBJECT: centre, BearCub with its neck stretched right out and its body leaning forward, 🔴 ONLY
  THE VERY TIP OF ITS NOSE PRESSED ONTO THE SURFACE, both forepaws gripping the rim with the arms
...
### p8 — 넷이 아직 따로따로 있어요 🔴 최대치 · 두 번째로 굽는다

```
BAKE THIS PAGE SECOND. It is the maximum: four colours in one tub, none of them mixed. If they
  blend here, page 10 and page 12 have nothing to do.
CAMERA: extreme close-up, HIGH angle straight down; the surface fills the whole frame. TOP EDGE,
  only the cub's muzzle and two eyes.
...
STAINS: 0.
FINISH: 2 (the four-coloured surface, counted as one + the two eyes at the top). 🔴 The water is
  flat calm - the pages either side are noisy and this one is still. 0 lettering, 0 numerals.
```

### p9 — 풍덩 뛰어들었어요

```
CAMERA: wide, LOW angle from floor height looking up at the tub. CENTRE, the cub going under;
  LOWER RIGHT, water going over the rim onto the floor and GranBear's feet snatched back.
SUBJECT: centre, BearCub 🔴 SITTING RIGHT DOWN INTO THE WATER WITH ITS KNEES FOLDED AND ITS BODY
  BALLED UP - in to the shoulders, only its head above the surface, both forepaws flung up, eyes
...
FINISH: 2 (the sinking body + the water going over the rim). 🔴 Things fly UP on one side and pull
  ...
  wall; the spill spreads over the dry ochre prints. Window fully dark. 0 lettering.
```

### p10 — 색들이 서로 끌려가요 🔴 섞임

```
ATTACH THE APPROVED PAGE 8 RENDER - the four that were separate are these same four.
CAMERA: medium close-up, HIGH angle looking obliquely down at the surface. CENTRE, the turning
  water with both forepaws in it; TOP, the cub's face.
SUBJECT: centre, BearCub sunk to the shoulders with 🔴 BOTH FOREPAWS UNDER THE WATER SWEEPING ONE
  BIG CIRCLE, arms opened front and back, upper body leaning the way the water turns, the water
  dragging after its paw tips. Top, the head bent down with the eyes following the turn, mouth wide
  open shouting, wet ears stuck to the cheeks.
WATER: 🔴 THE MIXING PAGE. One big whirl; the four are pulled out long from their edges and drawn
...
FINISH: 2 (the two sweeping forepaws + the turning water). 🔴 EVERY SHAPE IN THE FRAME WINDS TOWARD
  ...
  side of the tub. 0 lettering, 0 numerals.
```

### p11 — 손안에 색이 딱 하나예요

```
CAMERA: close-up, slightly LOW angle. CENTRE, two cupped forepaws holding water; ABOVE, the cub's
  face bent right down to look into them.
SUBJECT: centre, BearCub with 🔴 BOTH FOREPAWS CUPPED INTO A BOWL AND LIFTED UP TO ITS OWN EYES,
  elbows resting on the tub rim, the paw edges parted a little so water leaks out. Above, the head
  bent with both eyes brought close to the water it holds, eyes wide, mouth open shouting, a drop
  hanging on the wet nose, ears up.
WATER: 🔴 ONE COLOUR AND ONE ONLY - muddy #6B5238, OPAQUE, in the cupped paws; not one of the four
...
FINISH: 2 (the cupped forepaws + the water in them). 🔴 Push the background back by giving it ONE
  ...
  highlight painted on them. 0 lettering, 0 numerals.
```

### p12 — 바닥 나뭇결이 하나도 안 보여요 🔴 착지 · 세 번째로 굽는다

```
BAKE THIS PAGE THIRD. 🔴 ATTACH THE APPROVED PAGE 1 RENDER - same room, same tub, same brightness.
CAMERA: medium, child's eye level at rim height, from the side. LEFT, the cub with its chin on the
  rim; CENTRE, the water; BACK RIGHT, GranBear with a towel and the dark window.
SUBJECT: left, BearCub sitting in the tub with its lower jaw laid on the wooden rim and both
  forepaws side by side on top of it, wet coat flat to the body, a drop off one ear, eyes down,
  mouth just open asking, eyelids half down. Back right, GranBear holding a big towel open, head tilted
...
FINISH: 2 (the cub's face + the water). Two dried rings on the inner wall; a wide spill mark on the
  floor with the ochre prints washed away inside it. Window dark, stove mouth thumbnail-sized.
  0 lettering.
```
...
| # | 볼 것 | 실패 시 처방 |
|---|---|---|
| 1 | 🔴 **통 밖에 채도색이 있나.** 초록 난로·주황 불빛·붉은 천·노란 등불이 한 점이라도 있으면 이 권의 해법(넷을 쓰면서 화면은 좁게)이 통째로 무너진다 | 장면을 고치지 말고 `CabinRoom` 을 다시 굽는다. 특히 **난로 불빛이 방으로 번졌나** — 아궁이 입은 손톱만 하고 바닥에 빛이 안 떨어진다 |
...
부수: 🔴 **할머니가 가르쳤나** — 손가락을 들거나 몸을 굽혀 문질러 주면 그 순간 생활동화 45편이 된다. `GranBear` 의 네 포즈를 다시 굽는다.
부수: **아기 곰이 뭘 입었나** — 열두 쪽 내내 알몸이다(목욕이니까). 수건·앞치마가 걸쳐지면 그 컷을 다시.
부수: 🔴 **c02·c10·f06 과 나란히 본다** — 같은 C8 넷이라 판정 = 「이 책만 색이 **통 안에만** 있나」.
