...
```
--- p1 — 뱃등을 타고 내려온다 ---
MOSS: one horizontal #7A9A3E line along the top face of the third step, full width of the lower third; the only line that crosses the picture - step joins are short ink lines that stop at the stair.
FOOT: both webs on the black hull, sliding; the steps are in frame.
TAR: 🔴 a thumbnail-sized lump of solid #1C1A18 under the LEFT web, exactly the black of the hull it came off. In the picture from page one though the text names it only at p8. Do not hide or enlarge it.
CAMERA wide, slightly high oblique onto steps and water. / SUBJECT upper left, Duckling in POSTURE 1 coming down the curved back of the upturned boat, breast low, both webs shoved ahead; behind him two rows of footprints run up the hull. Lower right, Old duck POSTURE 1, back turned. / SETTING the boat keel-up left of the steps, its bottom one opaque flat black plane, straight edges, 0 gloss; the steps; far bank = 5 flat house silhouettes, 0 windows. Water = one flat green, 0 marks. / FINISH Duckling full; the hull half; rest flat fill and outline. Water opaque - nothing below it. No letters or numbers, no boat name. Plain ducks, no hands. / TONE early morning, cool; the one strong contrast is green water against black hull.
```
...
CAMERA medium, from the side of the stair at a low eye level right at the waterline, so the four steps read as layers. / SUBJECT right, Duckling in POSTURE 2 on the second step, both feet planted, neck stretched forward and down over the water, bill closed. Left on the water, Old duck POSTURE 2, bill open, calling, one web pushing back under the surface - THAT WEB IS UNFILLED INK OUTLINE with the green reading through it, the only thing under the water here. / SETTING four stone steps; the boat out of frame except one corner upper left. Water = one flat green, 0 ring marks. / FINISH Duckling full; the moss band half; rest flat. 🔴 Water opaque - no bottom, no sunk steps, no reflection. No letters or numbers. Plain ducks, no hands. / TONE moss green and dry stone grey split the picture along one horizontal; that line stays at this height for the rest of the book.
```
...
```
--- p3 — 셋째 단은 미끌미끌 ---
MOSS: the line is close here and fills the lower half at full width - still one line, still horizontal, still the only thing crossing the picture.
...
```
--- p4 — 첨벙 ---
MOSS: the line runs behind him at full width, close under his breast; the step is within one duckling-length and IN FRAME - that is what keeps this page from being the ending.
FOOT: not on stone. Both webs under the surface, touching nothing.
TAR: under the left web, submerged - 🔴 it stays SOLID BLACK under the water while the feet do not. The one filled thing below the surface.
...
CAMERA high angle, looking down almost straight onto the water. / SUBJECT centre, Duckling in POSTURE 5, tilted onto one side, head turned back to his own tail. 🔴 Under the surface: the RIGHT web is unfilled ink outline, fully spread and pushed back; the LEFT web is unfilled outline too but its three toes show TWO stuck together by the solid black lump, and it is not moving. Both webs are at the same depth in the same frame, so it is a direct comparison. / SETTING one white #F4F2EA line coiling once around him - the start of the mark the next page counts; upper corner, the moss fragment. Nothing else. / FINISH Duckling full; the two webs half; rest flat. 🔴 Top-down does NOT license a visible bottom - one flat opaque green edge to edge. No letters or numbers. Plain ducks. / TONE one green field from above; the single coil line is the only white.
```
...
```
--- p13 — 여기는 발이 안 닿아요 ---
MOSS: only a small fragment far off at the TOP with the steps - still horizontal, still the same green. 🔴 Far enough that no stone reaches the lower half.
FOOT: 🔴 not on stone, and THERE IS NO STONE ANYWHERE IN THE LOWER HALF. Twelve pages had something to stand on within reach; this one does not. That absence is the ending.
TAR: one small flat black shape still floating in the upper half.
CAMERA wide, high angle obliquely down, a boat-length out from the steps. / SUBJECT centre, Duckling in POSTURE 11, floating still, head turned slightly toward the old duck. 🔴 Under the surface both webs are unfilled ink outline, opening and closing slowly, touching nothing. Beside him Old duck POSTURE 5, level, facing the same way, webs likewise unfilled outline. / SETTING far at the top the small steps; 🔴 the LOWER HALF OF THE FRAME IS FLAT GREEN AND NOTHING ELSE - 0 marks, 0 steps, 0 bottom, 0 rings. No downward darkening, no gradient; it is empty because nothing is drawn there. / FINISH the two birds full; rest flat fill and outline. No letters or numbers. Plain ducks, no hands. / TONE the two bodies are the brightest things left; mostly one green plane with nothing in it.
```
