...
## 10. deck 의 전제 — Phase 1 트랙션 위에서 오픈하는 라운드 (2026-05-29 추가)

이 deck 은 **맨바닥에서 외치는 자료가 아니다.** Phase 1(부모 모집)이 어느 정도 검증된 **뒤에** 오픈한다고 가정하고 만든다.

- 오픈 게이트(예시): **유저 3,000명 · 평균 체류 10분+** 수준의 초기 트랙션 달성 후.
- 그 트랙션이 안 나오면 어떤 숫자·주장도 신뢰받지 못하고 "억지 주장"만 된다 → deck 자체가 의미 없음. 즉 Phase 1 KPI 는 **희망이 아니라 라운드 오픈의 전제 조건**.
- 따라서 모든 슬라이드(재무·flywheel·milestone)는 "Phase 1 성과라는 바닥 위에 쌓는 것"으로 읽혀야 한다. Phase 2~3·매출 추정은 **이미 검증된 부모 베이스를 디딤돌로** 전개.
- phased-gtm 슬라이드에서 Phase 1 → 라운드 오픈 트리거(달성 지표)를 암시/명시하면 일관성이 산다.
...
- [ ] (§10) Phase 1 트랙션(유저 3천·체류 10분+)을 **이미 달성한 전제** 위에서 쌓는 톤인가? 맨바닥 억지 주장 아닌가?
...
- 2026-05-29 — §10 추가 (deck 전제: Phase 1 트랙션 ~유저 3천·체류 10분+ 달성 후 오픈하는 라운드. 맨바닥 억지 주장 X) + §05 marketing 슬라이드 삭제(phased-gtm 으로 통합, 16→15장)
