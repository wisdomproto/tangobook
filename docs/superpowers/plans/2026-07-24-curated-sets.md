...
## 후속 조정 (2026-07-24 밤, 사용자 피드백 — 계획 밖 · 반영 완료)

계획대로 배포한 뒤 받은 피드백 4건. 모두 `PlaylistLibrarySection` + `LibraryPage` 렌더 위치만 손댔고 파생 로직(`buildCategoryBundles`)은 무변경.

- [x] **내 세트 + 묶음 한 행 통합** — 「내가 만든 세트」 소제목 행과 전체폭 빈상태 CTA 제거. 순서 = ➕ 이어재생 만들기(맨 앞, 코랄 카드, 로그인만) → 내 세트 → 카테고리 묶음.
- [x] **펼침 = 하나의 박스** — 테두리·배경을 헤더 버튼에서 `<section>` 컨테이너로 이동, 행에 내부 패딩.
- [x] **위치를 검색/필터보다 위로** — `LibraryPage` 의 `<PlaylistLibrarySection />` 한 줄 이동.
- [x] **기본 접힘으로 회귀** — Task 4 의 "기본 펼침"을 되돌림(책 목록이 먼저 보이게). 테스트는 `openSection()` 헬퍼로 펼친 뒤 검증하도록 갱신, 11개 통과.
