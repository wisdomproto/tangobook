...
> **상태 (2026-09-14)**: ✅ 라이브 — 명작 48 · 전래 40 · 자연관찰 101권 = 285장 · 칸 1,933개. 씬은 AI 자동 생성이 아니라 **작업판(`/hidden-object-plan.html`)에서 GPT 로 그려 붙이고**, 칸은 격자판을 눈으로 읽어 `hidden-object-hotspots.json` → 링커로 넣는다. 설계와 달라진 점: 한 낱말에 칸 여럿(`[[…],[…]]`) · 층(배경 0/사물 1/몸 부분 2) · 낱말 음원 concat. 현행은 루트 CLAUDE.md 가 SSOT.
