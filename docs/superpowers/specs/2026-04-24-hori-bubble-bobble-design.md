...
- 가변 점프: `Space` 누른 시간에 따라 `velocity.y` 조정 (원작의 짧은 탭/긴 홀드 구분). Phaser arcade에는 내장 지원 없음 → `update()` 루프에서 "점프 중 Space 릴리즈 & velocity.y < -150" 조건에 `velocity.y = -150`로 클램프하여 구현
...
**타이머 규칙 (원작 충실):**
- `floatTimer`: 거품 생성 시 0부터 시작, 8초 도달 시 자동 팡 (적 없으면 소멸, 적 있으면 적 탈출)
- `trapTimer`: 적 트랩 **순간** 별도로 0부터 시작, 5초 도달 시 적 탈출. `floatTimer`와 독립.
- 예: 거품 7초 시점에 적 트랩되면 → `trapTimer` 0 시작 · 8초 시점에 `floatTimer` 만료로 거품 팡 · 이 경우 적은 **과일 변환 (팡이 먼저이므로)**. `trapTimer`는 미완료 상태로 폐기.
- 즉 `floatTimer`가 항상 우선. `trapTimer`는 최대 5초 보장이 아니라 "5초 이내 플레이어가 안 터뜨리면 적 탈출" 조건 카운트다운.
- TRAPPED 마지막 1초 (trapTimer 4~5초): 빨강 tint + shake (탈출 직전 경고)
...
- 일반 과일(🍎🍓🍇🍊🍒)과 **별개 스폰 타입**. 적이 과일화되는 대신 레벨 중 1~2마리의 적이 **EXTEND 글자 과일** (E·X·T·E·N·D 중 하나) 로 변환됨 (레벨당 무작위 1~2개 보장)
- 수집 시 HUD 상단 좌측 E/X/T/E/N/D 슬롯에 해당 글자 점등
- 6글자 전부 모으면 +1 목숨 + 특별 팡파르, 슬롯 전체 리셋 (같은 레벨 내 반복 가능)
- 새 플레이 세션(리트라이) 시작 시 슬롯은 유지 (세션 전체 누적)
- 원작 아이덴티티의 핵심 요소
...
두 단계 연출 (원작과 동일):
- **제한시간 180초 (레벨별 조정 가능)**
- **[경고] 90초 남음**: BGM rate 1.2배 + HUD 중앙 "HURRY UP!" 팝업 (2초 표시)
- **[Blubba 스폰] 0초 도달**: 화면 상단에서 **Baron von Blubba 오마주 유령** 하나 등장. 빠르게 플레이어 추적, 무적, 지울 수 없음. 적 모두 처치 전까지 지속. — 원작 충실 요소
