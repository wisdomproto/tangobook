...
> 상태: **구현 완료 · 라이브** · 작성 2026-07-13 · 갱신 2026-07-17
>
> 진행: 명작 144 + 자연 101 렌더·등록 완료, YouTube 자동발행 244개 예약(07-17~12-06).
> **생활동화 45권**(2026-07-17): 메타 45권 집필(`_data/longform-meta.json` 149→194) + 렌더 진행 중
> → 남은 것은 `schedule-longform-youtube.ts --life-only --apply`.
>
> 🔴 설계 대비 바뀐 것: **메타는 Gemini 자동생성이 아니라 Claude 직접 집필**(§목표의 "Gemini로 자동
> 생성"은 폐기 — 파이프라인 전체가 Gemini-free). 메타가 없으면 폴백(제목=책 title 그대로·설명
> 빈칸·태그 1개)이라 **신규 시리즈는 렌더와 메타 집필이 세트**다.
> 🔴 쇼츠 발행은 이 롱폼 경로와 **별개**(`publishYouTube`) — 제목·설명·태그 규칙이 달라 따로 손봐야
> 한다(2026-07-17 fix: 선두 번호 제거 + 롱폼 메타 재사용). → memory `saenghwal-reels-redesign-2026-07-17`
