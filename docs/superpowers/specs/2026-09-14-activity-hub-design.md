...
2026-09-14 · 상태: 설계 승인(사용자 「ㄱㄱ」) · 스펙 리뷰 1회 반영
...
- 🔴 `/worksheet`(허브) → `/activity` · `/worksheet/hangul` → `/activity/hangul/kr-h1-u01` · `/worksheet/english` → `/activity/english/en-b1-u01` **301**.
  ...
  `PublicNav`·랜딩의 `/worksheet` 링크도 `/activity` 로. 인쇄물 파일(`/worksheet/ko_phonics.html`·`en_phonics.html`)은 그대로.
...
| 숨은그림 | 씬 + 찾을 낱말 체크리스트 | 책을 받아 `buildHiddenObjectSceneData(book, undefined, key)` → `HiddenObjectPlayer({ storybookId, gameData, onComplete, onBack })` |
...
- `buildHiddenObjectSceneData` 에 세 번째 인자 `sceneKey?` — 주면 그 씬만(없으면 지금처럼 무작위). 찾을 것 < 2 필터는 그대로.
- `ColoringPlayer` 에 선택 prop `onDone?: () => void` — 내부 `done` 이 true 가 되는 순간 한 번 부른다. 「다음 장」은 플레이어가 아니라 **CTA 옆 「다음 도안 →」 링크**(같은 갈래 다음 항목 주소)로 한다.
...
`packages/server/scripts/build-activity-catalog.mjs` 가 R2 를 읽어 **`packages/client/public/activity/` 에 두 파일**을 굽고 **커밋한다**
...
- `hidden-object.json` — **공개 책**(`isPublic !== false`)의 top-level `hiddenObjectScenes` 중 찾을 것(이름 중복 제거) **2개 이상**.
  항목: `{ key, bookId, bookTitle(bookDisplayTitle), category, sceneImageUrl, words:[라벨], blurb(parentGuide 요약 앞 120자) }`.
...
- 서버 SSR: `path.join(clientDist, 'activity/*.json')` 을 첫 요청에 읽어 메모리에 둔다(프로세스 수명 캐시 — 파일은 배포 때만 바뀐다).
  개발(`dist` 없음)은 `packages/client/public/activity/` 로 폴백.
- 클라: `fetch('/activity/coloring.json')` 등(TanStack Query, `staleTime` 길게).
- sitemap/IndexNow: `packages/client/public/activity/*.json`.

## 4. SEO

- 서버 `services/seo-activity.service.ts` + `app.ts` 라우트 5개. 기존 `sendSeo`/`AboutSeo`(`{redirect}` 지원) 형식:
  `<title>` · description · canonical · og(도안/씬 이미지) · h1 · **본문**(blurb + 활동 안내 2~3문장 + 찾을 낱말/배우는 글자 목록) ·
