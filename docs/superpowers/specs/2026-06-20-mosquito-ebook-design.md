...
- **개정(2026-06-20, 페르시아어 추가 + R2 일원화)**: **페르시아어(fa) 추가** — 원본 3개국어 HTML(`OneDrive/…/모기의_항변_3개국어.html`, `.block.fa`)에서 공식 나레이션 31p + 이미지글자 추출. `EbookLang`에 `fa`, `EBOOK_LANGS`/`EBOOK_LANG_LABEL`(`فارسی`)/`RTL_LANGS` 추가, `EbookOverlay.text`는 `Partial`(fa 는 샘플 1~10p 오버레이만, 나머지 ko 폴백). 자막·오버레이 **RTL**(`direction:rtl`) + 페르시아어 폰트(Vazirmatn/Tahoma). fa TTS 는 Gemini(`language:'fa'`)로 2~10p 생성 → **R2** `tts/v1/fa/`. ⚠️ mp4 렌더는 Linux Chromium 에 페르시아어 폰트 없어 Vazirmatn 번들 필요(웹은 OK). **R2 일원화**: 표지·4페이지를 로컬 public 에서 **R2 로 이전**(`img/v3/page-01.webp`·`img/v2/page-04.webp`) — R2 업로드는 권한 문제였을 뿐 기술 제약 아님(명시 지시 시 통과). 이제 모든 이미지·오디오가 R2.
