...
| 단계 | 상태 | 결과 |
| --- | --- | --- |
| ① Review 층 | ✅ | 한글 7단원(`kr-h1-r1`…) + 영어 Book1·2 8단원(`en-b1-r1`…). |
| ② 리스닝 변별 | ✅ | 학습 단원 = `word-listen-choose`(한글 31 + 영어 Book 1). 복습 = 듣고 음절·듣고 단어 2종. |
| ③ book3~5 plan | ✅ | 23유닛 개방(2026-07-31). **배우기 = 낱말가족 배우기**(`WordFamilyLearnActivity`, 2026-08-01 — 처음엔 듣고 고르기 퀴즈였으나 §4 대로 Learn=Listen and repeat 으로 교정) + 써보기 + 게임 3종. |
| ④ Test | ⬜ | 정답률이 남는 활동으로 설계해야 부모 리포트와 이어진다. |
| 콘텐츠 | 🔜 | ABC 나무 기획서 + 단어 카드 프롬프트 388장 완성. **대본 39편·삽화·연동 미착수.** |

### 복습이 자리 잡기까지 — 두 번 뒤집혔다

1. 처음엔 **3종**(다시 듣기·짝 찾기·글자 쓰기)이었고 전부 기존 컴포넌트 재사용이었다. → 사용자: **"너무 심심하다."** 이 문서 §1 이 이미 "복습은 형식을 바꿔야 한다"고 적어놨는데 재사용을 앞세워 정작 축약판을 만들었다.
2. **게임만 남기고**(익히기 제거) 길 따라가기·뒤집기 짝맞추기를 새로 넣었다.
3. 그러자 이번엔 **소리로 판단하는 활동이 하나도 없어져** 듣고 음절·듣고 단어를 다시 넣었다. 단, 학습 단원의 그림 3지선다와 겹치지 않게 **보기를 글자·낱말 텍스트 4개**로 바꿔서.

최종 6종 = 길따라가기 · 뒤집기 · **듣고 음절** · 짝찾기 · **듣고 단어** · 글자쓰기.
🔴 **듣기 둘을 붙여 놓지 않는다** — 같은 화면(🔊 + 보기 4개)이 연달아 나오면 한 활동을 두 번 하는 걸로 느낀다.
